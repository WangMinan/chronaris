...
if ENABLE_TORCH_RUNTIME_TESTS:
    ...
    class AlignmentPreviewPipelineTest(unittest.TestCase):
        def test_alignment_preview_config_rejects_invalid_normalization_mode(self) -> None:
            with self.assertRaises(ValueError):
                AlignmentPreviewConfig(input_normalization_mode="invalid")

        def test_alignment_preview_config_rejects_invalid_physics_mode(self) -> None:
            with self.assertRaises(ValueError):
                AlignmentPreviewConfig(physics_constraint_mode="invalid")
        ...
        def test_alignment_preview_pipeline_runs_train_validation_test_loop(self) -> None:
            ...
            for metrics in (*result.train_history, *result.validation_history, result.test_metrics):
                ...
                self.assertGreaterEqual(metrics.vehicle_physics, 0.0)
                self.assertGreaterEqual(metrics.physiology_physics, 0.0)
                self.assertGreaterEqual(metrics.physics_total, 0.0)
        ...
        def test_alignment_preview_pipeline_supports_zscore_train_normalization(self) -> None:
            ...
            result = pipeline.run(samples)
            self.assertEqual(result.test_metrics.sample_count, 5)
            self.assertGreaterEqual(result.test_metrics.total, 0.0)

        def test_alignment_preview_pipeline_supports_stage_f_min_physics_constraints(self) -> None:
            samples = tuple(_sample(index) for index in range(25))
            pipeline = AlignmentPreviewPipeline(
                config=AlignmentPreviewConfig(
                    prototype_config=AlignmentPrototypeConfig(
...
# ---- merged from test_alignment_experiment_pipeline.py ----
import os
import sys
from pathlib import Path
import unittest
