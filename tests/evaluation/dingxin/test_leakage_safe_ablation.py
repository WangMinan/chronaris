"""Tests for task evaluation leakage-safe private proxy ablation."""
...
from chronaris.evaluation.dingxin.pipelines.leakage_audit import (  # noqa: E402
    LabelFeatureLeakageError,
    audit_label_feature_overlap,
)
from chronaris.evaluation.dingxin.pipelines.leakage_safe_ablation import (  # noqa: E402
    MAX_T3_DISTRIBUTION_ROWS,
    StageILeakageSafeAblationConfig,
    _compact_t3_similarity_distribution,
    run_task_eval_leakage_safe_ablation,
)

_HELPER_SPEC = importlib.util.spec_from_file_location(
    "task_eval_leakage_safe_helpers",
...
class StageILeakageSafeAblationTest(unittest.TestCase):
    ...
    def test_leakage_safe_ablation_writes_smoke_outputs_and_aggregates_seeds(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            result = run_task_eval_leakage_safe_ablation(
                StageILeakageSafeAblationConfig(
    ...
    def test_same_seed_reproduces_summary_rows(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            root = Path(temp_dir)
            e_manifest, f_manifest = _write_pair(root)
            first = run_task_eval_leakage_safe_ablation(
                StageILeakageSafeAblationConfig(
            ...
            second = run_task_eval_leakage_safe_ablation(
                StageILeakageSafeAblationConfig(
