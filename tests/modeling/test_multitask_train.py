"""Tests for task evaluation multitask training and thesis-task builders."""
...
from chronaris.modeling.training.multitask_train import (  # noqa: E402
...
if _HELPER_SPEC is None or _HELPER_SPEC.loader is None:  # pragma: no cover - import guard
    raise RuntimeError("failed to load synthetic feature export helper")
    ...
    def test_real_task_builder_separates_thesis_weak_label_tasks(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            records = load_aligned_private_records(
                e_run_manifest_path=str(root / "feature-export-e" / "run_manifest.json"),
                f_run_manifest_path=str(f_manifest),
            )

            payload = build_task_eval_real_task_payload(records)
    ...
    def test_multitask_train_runs_end_to_end_and_writes_artifacts(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            result = run_task_eval_multitask_train(
                StageIMultitaskTrainConfig(
                    run_id="task-eval-multitask-smoke",
                ...
                source_summary={"source": "synthetic_feature_export_records"},
            )
            ...
            self.assertEqual(summary["source_summary"]["source"], "synthetic_feature_export_records")
