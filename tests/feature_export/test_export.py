"""feature export export and partial-data tests."""
...
from chronaris.feature_export.profile import StageHProfileResolver, StageHSortieProfile, StageHViewProfile
...
from chronaris.feature_export.export import (
    StageHExportConfig,
...
class StageHExportConfigTest(unittest.TestCase):
    def test_preview_profile_resolves_default_point_limit_guard(self) -> None:
        ...
        self.assertEqual(config.point_limit_note, "preview query guard, not a feature export closure standard")

    def test_validation_profile_can_run_without_point_caps(self) -> None:
        config = StageHExportConfig(
            run_id="feature-export-validation",
            sortie_ids=("sortie-1",),
    ...
    def test_report_renders_warn_diagnostic_reasons(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            report = render_feature_export_report(
                config=config,
...
class StageHExportPipelineTest(unittest.TestCase):
    def test_pipeline_writes_feature_export_artifacts_and_report(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            tmp = Path(tmpdir)
            ...
            config = StageHExportConfig(
                run_id="feature-export-test",
                sortie_ids=(profile.sortie_id,),
                output_root=tmp / "artifacts" / "feature_export",
                report_path=tmp / "docs" / "reports" / "feature-export-test.md",
                preview_point_limit_per_measurement=500,
            ...
            self.assertEqual(
                ...
                "preview query guard, not a feature export closure standard",
            )
            ...
            feature_run = load_feature_export_feature_run(run_manifest_path)
    ...
    def test_dual_pilot_sortie_exports_two_views(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            config = StageHExportConfig(
                run_id="feature-export-dual",
                sortie_ids=(profile.sortie_id,),
                output_root=tmp / "artifacts" / "feature_export",
                report_path=tmp / "docs" / "reports" / "feature-export-dual.md",
            )
            pipeline = StageHExportPipeline(
                config=config,
                profile_resolver=_FakeProfileResolver((profile,)),
    ...
    def test_feature_bundle_uses_fixed_keys_without_stage_g(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            profile = _profile("20251005_四01_ACT-4_云_J20_22#01", (10033,))
            config = StageHExportConfig(
                run_id="feature-export-no-g",
                sortie_ids=(profile.sortie_id,),
                output_root=tmp / "artifacts" / "feature_export",
                report_path=tmp / "docs" / "reports" / "feature-export-no-g.md",
    ...
    def test_pipeline_records_backbone_inference_lineage_in_manifests(self) -> None:
                ...
                *,
                ...
                return StageHViewExecutionResult(
                    ...
                    backbone_checkpoint_metadata={
                        "backbone_run_id": "task-eval-backbone-smoke",
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            profile = _profile("20251005_四01_ACT-4_云_J20_22#01", (10033,))
            config = StageHExportConfig(
                run_id="feature-export-backbone-infer",
                sortie_ids=(profile.sortie_id,),
                output_root=tmp / "artifacts" / "feature_export",
                report_path=tmp / "docs" / "reports" / "feature-export-backbone-infer.md",
                checkpoint_path="/tmp/alignment_backbone_checkpoint.pt",
                backbone_run_id="task-eval-backbone-smoke",
                inference_only=True,
                causal_fusion_enabled=False,
            ...
            feature_run = load_feature_export_feature_run(result.run_manifest_path)
...
class PartialDataBuilderTest(unittest.TestCase):
    def test_seed_partial_data_entry_is_concrete(self) -> None:
        seed_path = SRC.parent / "configs" / "partial-data" / "feature-export-seed-v1.jsonl"
    ...
    def test_validation_pipeline_writes_three_views_and_built_partial_sidecar(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            config = StageHExportConfig(
                run_id="feature-export-validation-test",
                sortie_ids=(single.sortie_id, dual.sortie_id),
                output_root=tmp / "artifacts" / "feature_export",
                report_path=tmp / "docs" / "reports" / "feature-export-validation-test.md",
                export_profile="validation",
                partial_data_entries=(partial_entry,),
            )
            pipeline = StageHExportPipeline(
