"""Stage I public-opt regression aggregation tests."""
...
_HELPER_SPEC = importlib.util.spec_from_file_location(
    "stage_i_public_opt_helpers",
    Path(__file__).resolve().with_name("test_stage_i_public_opt.py"),
)
if _HELPER_SPEC is None or _HELPER_SPEC.loader is None:  # pragma: no cover - import guard
    raise RuntimeError("failed to load Stage I public-opt helpers")
...
class StageIPublicOptAggregationTest(unittest.TestCase):
    ...
    def test_torch_session_mean_broadcast_makes_final_predictions_constant_within_session(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            root = Path(temp_dir)
