"""Tests for task evaluation LLM preprocessing context generation."""
...
class StageILLMPreprocessingTest(unittest.TestCase):
    def test_mock_provider_writes_context_and_downstream_summaries(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            result = run_task_eval_llm_preprocessing(
                StageILLMPreprocessingConfig(
                    run_id="task-eval-llm-preprocessing-test",
                    mode="build-and-evaluate",
                    artifact_root=str(root / "artifacts"),
                    report_root=str(root / "reports"),
                    provider_name="mock",
                    model="mock-task-eval-llm",
            ...
            self.assertEqual(harness_summary["remaining_invalid_task_count"], 0)
    ...
    def test_provider_failure_is_recorded_as_expected_failure(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            result = run_task_eval_llm_preprocessing(
                StageILLMPreprocessingConfig(
                    run_id="task-eval-llm-preprocessing-failure-test",
                    artifact_root=str(root / "artifacts"),
                    report_root=str(root / "reports"),
    ...
    def test_schema_repair_retries_invalid_local_output(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            result = run_task_eval_llm_preprocessing(
                StageILLMPreprocessingConfig(
                    run_id="task-eval-llm-preprocessing-repair-test",
                    artifact_root=str(root / "artifacts"),
                    report_root=str(root / "reports"),
                    provider_name="mock",
                    model="mock-task-eval-llm",
    ...
    def test_large_payloads_are_sliced_and_merged_locally(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            result = run_task_eval_llm_preprocessing(
                StageILLMPreprocessingConfig(
                    run_id="task-eval-llm-preprocessing-slicing-test",
                    mode="build-and-evaluate",
                    artifact_root=str(root / "artifacts"),
                    report_root=str(root / "reports"),
                    provider_name="mock",
            ...
            self.assertEqual(summary["harness_summary"]["remaining_invalid_task_count"], 0)
    ...
    def test_downstream_helpers_keep_llm_context_optional_and_whitelisted(self) -> None:
        ...
        context = {
            "run_id": "llm-run",
            "schema_version": "task_eval_llm_preprocessing_context.v1",
...
def _write_sources(root: Path) -> dict[str, Path]:
    ...
    entries = _task_entries()
    dump_task_eval_private_task_entries(entries, path=task_manifest)
