"""task evaluation support aggregation tests."""
...
from chronaris.evidence.support import (  # noqa: E402
    StageISupportConfig,
    run_task_eval_support,
)


class StageISupportTest(unittest.TestCase):
    def test_run_task_eval_support_writes_summary_matrix_and_reports(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            temp_root = Path(temp_dir)
            sources = _write_support_sources(temp_root)
            result = run_task_eval_support(
                StageISupportConfig(
                    run_id="task-eval-support-test",
                    ...
                    feature_export_run_manifest_path=str(sources["feature_export"]),
            ...
            self.assertIn("feature_export_export", set(matrix["variant"]))
            ...
            self.assertIn("feature export Export Stability", alignment_report)
...
class StageIMidtermEvidenceTest(unittest.TestCase):
    def test_run_task_eval_midterm_evidence_writes_manifest_metrics_figures_and_audit(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            temp_root = Path(temp_dir)
            sources = _write_midterm_sources(temp_root)
            result = run_task_eval_midterm_evidence(
                StageIMidtermEvidenceConfig(
                    run_id="task-eval-midterm-test",
...
def _write_support_sources(temp_root: Path) -> dict[str, Path]:
    ...
    feature_export = temp_root / "feature_export_run_manifest.json"
...
def _write_midterm_sources(temp_root: Path) -> dict[str, Path]:
    ...
    report_root = workspace_root / "docs" / "reports" / "task_eval"
    ...
    (workspace_root / "docs" / "README.md").write_text(
        "task-eval-public-mainline-history-fixture.md\n",
        encoding="utf-8",
    )
    (report_root / "README.md").write_text("task_eval report index\n", encoding="utf-8")
    (report_root / "task-eval-closure-2026-04-30.md").write_text("keep\n", encoding="utf-8")
    (report_root / "task-eval-public-mainline-history-fixture.md").write_text(
        "old mainline\n",
        encoding="utf-8",
    )
    (report_root / "task-eval-public-opt-history-fixture.md").write_text(
        "old public opt\n",
        encoding="utf-8",
    )
    unused_module_root = workspace_root / "src" / "chronaris" / "pipelines" / "task_eval"
    unused_module_root.mkdir(parents=True, exist_ok=True)
    (unused_module_root / "__init__.py").write_text("", encoding="utf-8")
    (unused_module_root / "task_eval_unused_candidate.py").write_text(
        "VALUE = 1\n",
        encoding="utf-8",
    )
    ...
    support_result = run_task_eval_support(
        StageISupportConfig(
            ...
            feature_export_run_manifest_path=str(support_sources["feature_export"]),
    ...
    private_midterm_summary.write_text(
        json.dumps(
            {
                "run_id": "20260504T120000Z-task-eval-private-opt-package",
                "target_variant_name": "chronaris_opt",
    ...
    uab_fairness_summary.write_text(
        json.dumps(
            {
                "run_id": "20260509T010000Z-task-eval-uab-fairness",
                "subjective": {
                    "groups": {
                        "n_back": {"rmse": 4.7000},
