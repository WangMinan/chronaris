"""Key-condition anchor selection over frozen feature export assets."""
...
from chronaris.evidence.case_study import (
    ...
    build_task_eval_case_study_results,
)
...
def run_task_eval_anchor(
    config: StageIAnchorConfig,
) -> StageIAnchorRunResult:
    """Select stable key-condition anchors from frozen feature export views."""

    LOGGER.info(
        "task_eval_anchor start run_id=%s feature_export_run_manifest=%s private_summary=%s",
        config.run_id,
    ...
    LOGGER.info(
        "task_eval_anchor selected views=%d anchors=%d verdict_filter=%s",
    ...
    report_path = report_root / f"task-eval-anchor-{config.run_id}.md"
    ...
    summary = {
        ...
        "source_paths": {
            "feature_export_run_manifest_path": config.feature_export_run_manifest_path,
    ...
    report_path.write_text(
        render_task_eval_anchor_report(summary) + "\n",
        encoding="utf-8",
    )
    LOGGER.info(
        "task_eval_anchor finished run_id=%s manifest_path=%s report_path=%s",
        config.run_id,
        manifest_path,
        report_path,
    )
...
def render_task_eval_anchor_report(summary: Mapping[str, object]) -> str:
    """Render the markdown anchor report."""

    lines = [
        f"# task evaluation Anchor Report - {summary['run_id']}",
        "",
        f"- generated_at_utc: `{summary['generated_at_utc']}`",
        f"- feature_export_run_manifest_path: `{summary['source_paths']['feature_export_run_manifest_path']}`",
