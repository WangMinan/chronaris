"""P38 thesis protocol freeze builder for task evaluation evidence."""
...
DEFAULT_P34_ROOT = (
    REPO_ROOT
    / "docs/artifacts/runs"
    / "20260702T-task-eval-task-heads-optimization-r3-confirm20"
)
DEFAULT_P35_ROOT = (
    REPO_ROOT
    / "docs/artifacts/runs"
    / "20260702T-task-eval-stream-role-fusion-r4-v3-confirm20"
)
DEFAULT_P36_ROOT = (
    REPO_ROOT
    / "docs/artifacts/runs"
    / "20260702T-task-eval-optimized-reevaluation-r4-v3-confirm20"
)
DEFAULT_P36_SUMMARY_ROOT = (
    REPO_ROOT
    / "docs/artifacts/runs"
    / "20260702T-task-eval-optimized-model-summary-r4-v3-confirm20"
)
DEFAULT_P37_ROOT = (
    REPO_ROOT
    / "docs/artifacts/runs"
    / "20260702T-task-eval-metric-calibration-r1"
)
...
def build_task_eval_thesis_protocol(
    config: StageIThesisProtocolConfig,
) -> StageIThesisProtocolResult:
    """Build the P38 thesis protocol package from existing task evaluation artifacts."""
    ...
    report_path = _resolve_path(config.report_root) / f"task-eval-thesis-protocol-{config.run_id}.md"
...
def _experiment_registry(roots: Mapping[str, Path]) -> pd.DataFrame:
    rows = [
        _registry_row(
            ...
            / "docs/artifacts/runs/task-eval-private-thirdparty-comparison-20260702T-task-eval-private-thirdparty-comparison-gpuopt-r1.md",
            claim_boundary="Private feature export proxy-task third-party comparison; not expert truth.",
        ),
        _registry_row(
            stage="P31",
            ...
            report_path=REPO_ROOT
            / "docs/artifacts/runs/task-eval-public-fusion-ablation-20260702T-task-eval-public-fusion-ablation-gpuopt-r1.md",
        ...
        _registry_row(
            ...
            report_path=REPO_ROOT
            / "docs/artifacts/runs/task-eval-cross-evidence-matrix-20260702T-task-eval-cross-evidence-matrix-gpuopt-r1.md",
        ...
        _registry_row(
            ...
            report_path=REPO_ROOT
            / "docs/artifacts/runs/task-eval-task-aware-heads-20260702T-task-eval-task-heads-optimization-r3-confirm20.md",
        ...
        _registry_row(
            ...
            report_path=REPO_ROOT
            / "docs/artifacts/runs/task-eval-stream-role-aware-fusion-20260702T-task-eval-stream-role-fusion-r4-v3-confirm20.md",
        ...
        _registry_row(
            ...
            report_path=REPO_ROOT
            / "docs/artifacts/runs/task-eval-optimized-chronaris-reevaluation-20260702T-task-eval-optimized-reevaluation-r4-v3-confirm20.md",
        ...
        _registry_row(
            ...
            report_path=REPO_ROOT
            / "docs/artifacts/runs/task-eval-optimized-model-summary-20260702T-task-eval-optimized-model-summary-r4-v3-confirm20.md",
        ...
        _registry_row(
            ...
            primary_result_path=roots["P37"] / "metric_calibration_summary.json",
            report_path=REPO_ROOT
            / "docs/artifacts/runs/task-eval-metric-calibration-20260702T-task-eval-metric-calibration-r1.md",
            claim_boundary="Terminal polish accepts T1/public route only; T3 rejected and public remains context proxy.",
        ),
    ]
...
def _p36_summary_rows(root: Path) -> list[dict[str, object]]:
    ...
    for record in pd.read_csv(path).to_dict(orient="records"):
        ...
        if scope == "private_feature_export":
...
def _p37_private_rows(root: Path) -> list[dict[str, object]]:
    ...
    t3_path = root / "t3_metric_calibration_metrics.csv"
    ...
    rows = [
        {
            ...
            "evidence_quadrant": "private_model_comparison",
            "dataset_role": "private_real_dual_stream",
            "allowed_claim": "Use P30/P34/P35/P37 private rows as T1/T2/T3 proxy-task evidence on existing feature export dual-stream samples.",
            ...
            "artifact_path": str(roots["P37"] / "metric_calibration_summary.json"),
        },
        {
        ...
        {
            "claim_id": "p37_metric_calibration_scope",
            "evidence_quadrant": "private_model_comparison + public_model_comparison",
            "dataset_role": "mixed_private_public",
        ...
        },
        {
            ...
            "artifact_path": str(
                REPO_ROOT
                / "docs/artifacts/runs/2026-06-14_llm-preprocessing-comparison/llm_comparison_summary.json"
            ),
        },
    ]
    return pd.DataFrame(rows)
...
def _render_report(
    ...
    lines = [
        f"# task evaluation Thesis Protocol Freeze - {summary['run_id']}",
        "",
        "## 摘要",
        "",
...
def _status_from_stage(stage: str, root: Path) -> str:
    ...
    if stage == "P37":
        return _status_from_json(root / "metric_calibration_summary.json")
...
def _resume_command(config: StageIThesisProtocolConfig, roots: Mapping[str, Path]) -> str:
    parts = [
        PYTHON,
        str(REPO_ROOT / "scripts/task_eval/evidence/build_thesis_protocol.py"),
        "--run-id",
        config.run_id,
        "--p30-root",
        str(roots["P30"]),
