"""Data contracts for task evaluation thesis-facing tables and figures."""
    ...
    return [
        {
            ...
            "detail_cn": "复用当前 feature export 到 task evaluation 的联合训练权重，不重跑训练。",
...
def build_rigid_body_rotation_rows(
    ...
    for axis in ("pitch", "roll", "yaw"):
        for field_type, key in (("angle", axis), ("rate", f"{axis}_rate")):
            feature_export_count = len(_as_list(feature_candidates.get(key)))
            ...
            rows.append(
                {
                    ...
                    "feature_export_candidate_count": feature_export_count,
                    "mysql_candidate_count": mysql_count,
                    "available": bool(feature_export_count and mysql_count),
                    "availability_label": "available" if feature_export_count and mysql_count else "missing",
                    "translation_vertical_enabled": True,
                    "rotation_status": rotation.get("rotation_status"),
                    "rotation_disabled_reason": rotation.get("rotation_reading"),
...
def build_public_transfer_rows(
    ...
    return [
        ...
        {
            "segment_order": 2,
            "segment_id": "private_feature_export_weak_label",
            "segment_title_cn": "鼎新弱监督主线",
            "segment_title": "Dingxin feature export weak-label mainline",
