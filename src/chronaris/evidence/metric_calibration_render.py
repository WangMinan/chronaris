"""Rendering helpers for metric calibration artifacts."""
...
def render_figures(run_root: Path, tables: Mapping[str, Path]) -> dict[str, str]:
    ...
    _metric_bar(_safe_read(tables["t3_metric_calibration_metrics_csv"]), "mrr", paths["fig_p37_t3_retrieval_leaderboard"])
...
def render_report(
    ...
    return "\n".join(
        [
            f"# task evaluation Optimized Metric Calibration - {summary['run_id']}",
            "",
            "## Executive Summary",
            ...
            "",
            "## Retrieval Task Metric Calibration",
            f"- metrics: `{summary['t3_metric_calibration_metrics_csv']}`",
