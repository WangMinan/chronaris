"""Phase 2 case-study pipeline over frozen feature export assets."""
...
from chronaris.features import StageICaseStudyViewInput, load_task_eval_case_study_run


@dataclass(frozen=True, slots=True)
class StageICaseStudyConfig:
    """Frozen configuration for the task evaluation Phase 2 run."""

    run_id: str
    feature_export_run_manifest_path: str
    output_root: str
    report_path: str
    top_k_windows: int = 5
    device: str = "auto"
    ...
    def to_dict(self) -> dict[str, object]:
        return {
            "run_id": self.run_id,
            "feature_export_run_manifest_path": self.feature_export_run_manifest_path,
...
def build_task_eval_case_study_results(
    run_input,
    *,
    top_k_windows: int,
    device: str = "auto",
...
def run_task_eval_case_study(
    config: StageICaseStudyConfig,
) -> StageICaseStudyRunResult:
    """Run the fixed Phase 2 case-study family over a feature export run."""

    run_input = load_task_eval_case_study_run(config.feature_export_run_manifest_path)
    view_results, pilot_comparisons = build_task_eval_case_study_results(
        run_input,
...
def write_task_eval_case_study_report(
    ...
    report_path.write_text(render_task_eval_case_study_report(result) + "\n", encoding="utf-8")
    return str(report_path)


def render_task_eval_case_study_report(result: StageICaseStudyRunResult) -> str:
    """Render the Chinese Phase 2 report."""

    lines = [
        f"# task evaluation Phase 2 Case Study - {result.config.run_id}",
        "",
        f"- generated at UTC: `{result.generated_at_utc}`",
        f"- feature export run manifest: `{result.config.feature_export_run_manifest_path}`",
        ...
        "## View Summary",
        "",
        "- `hidden-vs-projection cosine` 使用 fused L2 norm profile 的 cosine；原因是当前 feature export 导出的 hidden fused 为 `96` 维，而 projection rerun baseline 为 `48` 维，不能直接做逐向量余弦。",
        "",
