"""Markdown reporting helpers for task evaluation support summaries."""
...
def render_task_eval_alignment_support_report(summary: Mapping[str, object]) -> str:
    ...
    feature_export = alignment["feature_export_export"]
    return "\n".join(
        [
            f"# task evaluation Alignment Support - {summary['run_id']}",
            "",
            ...
            "",
            "## feature export Export Stability",
            "",
            f"- sortie_count: `{feature_export['sortie_count']}`",
            f"- generated_view_count: `{feature_export['generated_view_count']}`",
            f"- view verdict counts: `{feature_export['view_verdict_counts']}`",
            f"- partial_data_entry_count: `{feature_export['partial_data_entry_count']}`",
            f"- partial_data_built_entry_count: `{feature_export['partial_data_built_entry_count']}`",
            "",
            "## 对齐结论",
            "",
            ...
            "3. `feature export` 已把对齐结果稳定导出为 3 个双流 view，可直接被下游与 case-study 消费。",
...
def render_task_eval_causal_support_report(summary: Mapping[str, object]) -> str:
    ...
    lines = [
        f"# task evaluation Causal Support - {summary['run_id']}",
        "",
        f"- generated_at_utc: `{summary['generated_at_utc']}`",
        f"- artifact_root: `{summary['artifact_root']}`",
...
def render_task_eval_ablation_support_report(summary: Mapping[str, object]) -> str:
    matrix_rows = summary["main_ablation_rows"]
    overview_plot_path = summary.get("overview_plot_path")
    lines = [
        f"# task evaluation Ablation Support - {summary['run_id']}",
        "",
        f"- generated_at_utc: `{summary['generated_at_utc']}`",
        f"- artifact_root: `{summary['artifact_root']}`",
