...
def run_task_eval_private_component_ablation(
    ...
    with open_task_eval_run_observer(
        ...
        stage_name="task_eval_private_component_ablation",
        logger=LOGGER,
        initial_progress={
        ...
        return _run_task_eval_private_component_ablation_observed(
            config=config,
            run_root=run_root,
            progress=progress,
        )
...
def _run_task_eval_private_component_ablation_observed(
    *,
    config: StageIPrivateComponentAblationConfig,
    run_root: Path,
    ...
    report_path = report_root / f"task-eval-private-component-ablation-{config.run_id}.md"
    report_path.write_text(
        render_task_eval_private_component_ablation_report(summary) + "\n",
        encoding="utf-8",
    )
    progress.finish(
...
def render_task_eval_private_component_ablation_report(summary: Mapping[str, object]) -> str:
    lines = [
        f"# task evaluation Private Proxy Component Ablation - {summary['run_id']}",
        "",
