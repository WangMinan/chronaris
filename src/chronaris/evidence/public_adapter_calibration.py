"""Public adapter and calibration summary builder for task evaluation."""
...
DEFAULT_UAB_SKLEARN_SUMMARY_PATHS = (
    "docs/artifacts/runs/20260506T121000Z-task-eval-public-opt-uab/public_opt_summary.json",
    "docs/artifacts/runs/20260508T125651Z-task-eval-public-opt-uab-robust-prior-r1/public_opt_summary.json",
)
DEFAULT_NASA_SUMMARY_PATHS = (
    "docs/artifacts/runs/2026-05-06_public-opt-nasa-round1/public_opt_summary.json",
)
DEFAULT_UAB_TORCH_SUMMARY_PATHS = (
    "docs/artifacts/runs/20260506T165558Z-task-eval-public-opt-uab-torch/public_opt_torch_summary.json",
    "docs/artifacts/runs/20260508T090700Z-task-eval-public-opt-uab-heat-specialist-r1/public_opt_torch_summary.json",
)
...
def run_task_eval_public_adapter_calibration(
    ...
    with open_task_eval_run_observer(
        ...
        stage_name="task_eval_public_adapter_calibration",
        logger=LOGGER,
        initial_progress={
        ...
        return _run_task_eval_public_adapter_calibration_observed(
            config=config,
            run_root=run_root,
            progress=progress,
        )
...
def _run_task_eval_public_adapter_calibration_observed(
    *,
    config: StageIPublicAdapterCalibrationConfig,
    run_root: Path,
    ...
    report_path = report_root / f"task-eval-public-adapter-calibration-{config.run_id}.md"
    report_path.write_text(
        render_task_eval_public_adapter_calibration_report(summary) + "\n",
        encoding="utf-8",
    )
    progress.finish(
...
def render_task_eval_public_adapter_calibration_report(summary: Mapping[str, object]) -> str:
    lines = [
        f"# task evaluation Public Adapter Calibration - {summary['run_id']}",
        "",
...
def _extract_uab_torch_rows(*, payload: Mapping[str, object], source_path: str) -> list[dict[str, object]]:
    rows: list[dict[str, object]] = []
    for subset_id, metrics in (payload.get("selected_result", {}) or {}).get("groups", {}).items():
        selection = (
            (payload.get("selected_result", {}) or {})
