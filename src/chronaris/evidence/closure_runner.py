"""Unified task evaluation evidence runner for thesis-facing closure tasks."""
...
DEFAULT_EXISTING_OUTPUTS = {
    "rigid_body": {
        "summary_path": "docs/artifacts/runs/2026-06-07_rigid-body-diagnostics/rigid_body_ablation_summary.json",
        "report_path": "docs/artifacts/runs/task-eval-rigid-body-20260607T-task-eval-rigid-body-r2.md",
        "evidence_layer": "rigid_body_support",
    },
    "semantic": {
        "summary_path": "docs/artifacts/runs/2026-06-07_semantic-support/support_summary.json",
        "report_path": "docs/artifacts/runs/task-eval-causal-support-20260607T-task-eval-support-semantic-r2.md",
        "evidence_layer": "semantic_support",
    },
    "runtime": {
        "summary_path": "docs/artifacts/runs/2026-06-07_runtime-inference-service/runtime_inference_summary.json",
        "report_path": "docs/artifacts/runs/task-eval-runtime-inference-20260607T-task-eval-runtime-service-r2.md",
...
class StageIEvidenceRunnerConfig:
    """Configuration for one unified task evaluation evidence closure run."""
...
class StageIEvidenceRunnerRunResult:
    """Artifacts written by one task evaluation evidence closure run."""
...
def run_task_eval_evidence_closure(
    ...
    with open_task_eval_run_observer(
        ...
        stage_name="task_eval_evidence_runner",
        logger=LOGGER,
        initial_progress={
        ...
        return _run_task_eval_evidence_closure_observed(
            config=config,
            run_root=run_root,
            progress=progress,
        )
...
def _run_task_eval_evidence_closure_observed(
    *,
    config: StageIEvidenceRunnerConfig,
    run_root: Path,
    ...
    report_path = Path(config.report_root) / f"task-eval-evidence-closure-{config.run_id}.md"
    ...
    report_path.write_text(
        render_task_eval_evidence_manifest_report(manifest) + "\n",
        encoding="utf-8",
    )
...
def render_task_eval_evidence_manifest_report(manifest: Mapping[str, object]) -> str:
    lines = [
        f"# task evaluation Evidence Closure - {manifest['run_id']}",
        "",
    ...
    *,
    ...
    if task_name == "multitask":
        ...
        command = [
            ...
            "scripts/task_eval/evidence/run_weak_label_sweep.py",
            "--run-id",
        ...
        if config.skip_heavy:
            command.extend(
                [
                    "--sample-source",
                    "feature_export_window_stats_proxy",
    ...
    if task_name == "private_proxy":
        command = [
            config.python_executable,
            "scripts/task_eval/evidence/run_private_component_ablation.py",
            "--run-id",
    ...
    if task_name == "public_adapter":
        calibration_command = [
            config.python_executable,
            "scripts/task_eval/evidence/run_public_adapter_calibration.py",
            "--run-id",
        ...
        transfer_command = [
            config.python_executable,
            "scripts/task_eval/evidence/build_public_transfer_boundary.py",
            "--run-id",
    ...
    if task_name == "rotation":
        command = [
            config.python_executable,
            "scripts/task_eval/evidence/run_rigid_body_rotation_audit.py",
            "--run-id",
