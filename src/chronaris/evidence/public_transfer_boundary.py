...
DEFAULT_CALIBRATION_SUMMARY_PATH = (
    "docs/artifacts/runs/"
    "20260607T-task-eval-public-adapter-calibration-r1/public_adapter_calibration_summary.json"
)
DEFAULT_PUBLIC_MAINLINE_SUMMARY_PATH = (
    "docs/artifacts/runs/"
    "2026-05-08_public-mainline-uab-robust-prior-r1/public_mainline_summary.json"
)
DEFAULT_MULTITASK_SUMMARY_PATH = (
    "docs/artifacts/runs/2026-06-07_dingxin-multitask-real-closure/"
...
def run_task_eval_public_transfer_boundary(
    ...
    with open_task_eval_run_observer(
        ...
        stage_name="task_eval_public_transfer_boundary",
        logger=LOGGER,
        initial_progress={"artifact_root": str(run_root)},
    ) as progress:
        return _run_task_eval_public_transfer_boundary_observed(
            config=config,
            run_root=run_root,
            progress=progress,
        )
...
def _run_task_eval_public_transfer_boundary_observed(
    *,
    config: StageIPublicTransferBoundaryConfig,
    run_root: Path,
    ...
    data_boundary_rows = [
        {
            ...
            "time_reference": "feature export unified timeline with sortie/pilot/view ids",
    ...
    task_boundary_rows = [
        {
            ...
            "note": "weak labels built from private feature export aligned windows",
        },
        {
            "layer": "private_proxy",
            "task_scope": "T1/T2/T3",
    ...
    report_path = report_root / f"task-eval-public-transfer-boundary-{config.run_id}.md"
    report_path.write_text(
        render_task_eval_public_transfer_boundary_report(summary) + "\n",
        encoding="utf-8",
    )
...
def render_task_eval_public_transfer_boundary_report(summary: Mapping[str, object]) -> str:
    lines = [
        f"# task evaluation Public Transfer Boundary - {summary['run_id']}",
        "",
