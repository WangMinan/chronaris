"""Artifact-writing and report helpers for feature export export."""
...
def render_feature_export_report(
    *,
    ...
    """Render the human-readable feature export report that links back to artifacts."""

    lines = [
        f"# feature export Export v1 - {config.run_id}",
        "",
        f"- export version: `{config.export_version}`",
        f"- export profile: `{config.export_profile}`",
        f"- export mode: `{run_manifest.export_mode}`",
        ...
        f"- inference only: `{config.inference_only}`",
        f"- checkpoint path: `{config.checkpoint_path}`",
        f"- backbone run id: `{config.backbone_run_id}`",
...
def build_run_config_summary(config: StageHExportConfig) -> dict[str, object]:
    return {
        ...
        "backbone_run_id": config.backbone_run_id,
