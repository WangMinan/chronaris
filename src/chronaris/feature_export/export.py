"""feature export multi-sortie export pipeline and artifact packaging."""
...
from chronaris.feature_export.export_helpers import (
...
def _default_feature_export_preview_config() -> AlignmentPreviewConfig:
...
class StageHExportConfig:
    """Frozen feature export export settings for one run."""

    run_id: str
    sortie_ids: tuple[str, ...]
    output_root: str | Path = "docs/artifacts/runs"
    report_path: str | Path = "docs/artifacts/feature_export/feature-export-v1.md"
    export_version: str = "feature-export-v1"
    ...
    preview_config: AlignmentPreviewConfig = field(default_factory=_default_feature_export_preview_config)
    ...
    def point_limit_note(self) -> str:
        ...
        if self.export_profile == "preview":
            return "preview query guard, not a feature export closure standard"
...
class StageHViewManifest:
    """One exported view package inside a feature export run."""
...
class StageHSortieManifest:
    """One sortie-level feature export manifest."""
...
class StageHRunManifest:
    """Run-level feature export manifest spanning all sorties and partial-data outputs."""
...
class StageHViewExecutionResult:
    """In-memory result of exporting one feature export view before serialization."""
...
class StageHExportRunResult:
    """Terminal feature export run outputs."""
...
class AlignmentStageHViewRunner:
    """Default feature export executor based on the frozen Stage E/F/G preview stack."""
...
class StageHExportPipeline:
    """Write feature export run/sortie/view assets plus the partial-data sidecar."""
