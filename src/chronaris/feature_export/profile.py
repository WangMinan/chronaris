"""feature export multi-sortie export profile resolution."""
...
class StageHViewProfile:
    """Resolved one-pilot export view for a feature export sortie."""
...
class StageHSortieProfile:
    """Resolved metadata and stream scope for one feature export sortie."""
...
class StageHProfileResolver:
    """Resolve feature export export profiles from MySQL and Influx facts."""
    ...
    def resolve(self, locator: SortieLocator) -> StageHSortieProfile:
        """Resolve one sortie into the exact feature export export contract."""
        ...
        if self.require_model_physiology_measurements and (
            ...
            raise ValueError(
                f"feature export sortie {flight_task.sortie_number} is missing required physiology "
        ...
        if not vehicle_measurements:
            raise ValueError(
                f"feature export sortie {flight_task.sortie_number} has no "
    ...
    def resolve_many(self, sortie_ids: Sequence[str]) -> tuple[StageHSortieProfile, ...]:
        """Resolve a stable sequence of feature export profiles."""
