"""Reporting and plotting helpers for task evaluation baselines."""
...
def render_task_eval_baseline_report(
    *,
    ...
    """Render one markdown report for a task evaluation baseline run."""

    dataset_id = str(dataset_summary["dataset_id"])
    generated_at = datetime.now(timezone.utc).strftime("%Y-%m-%d %H:%M:%S UTC")
    title = "# task evaluation NASA CSM Attention Baseline" if dataset_id == NASA_DATASET_ID else "# task evaluation UAB Baseline"
    lines = [
        title,
        "",
        f"- 生成时间：{generated_at}",
...
def render_uab_baseline_report(
    *,
    ...
    return render_task_eval_baseline_report(
...
def write_dataset_diagnostic_plots(
    *,
    ...
    plot_paths["dataset_subset_counts"] = save_bar_plot(
        ...
        title="task evaluation subset counts",
        ylabel="window count",
    )
