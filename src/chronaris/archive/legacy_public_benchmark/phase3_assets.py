"""Artifact and summary helpers for task evaluation Phase 3 closure."""
...
from chronaris.dataset import StageIDatasetSummary, StageITaskEntry, dump_task_eval_summary, dump_task_eval_task_entries
from chronaris.features import StageIFeatureTableResult
from chronaris.archive.legacy_public_benchmark.baseline import StageIBaselineArtifacts, render_task_eval_baseline_report


def build_task_eval_dataset_summary(
...
def write_prepared_dataset(
    *,
    ...
    dump_task_eval_task_entries(entries, path=artifact_root / "task_manifest.jsonl")
    ...
    dump_task_eval_summary(summary, path=artifact_root / "dataset_summary.json")
...
def write_local_baseline_report(
    *,
    ...
    report_path.write_text(
        render_task_eval_baseline_report(
...
def load_task_eval_baseline_artifacts(
    artifact_root: Path,
    *,
...
def read_task_eval_dataset_summary(artifact_root: Path) -> StageIDatasetSummary:
