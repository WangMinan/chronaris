"""task evaluation Phase 3 orchestration and closure helpers."""
...
from chronaris.archive.legacy_public_benchmark.baseline import run_task_eval_baselines, write_baseline_artifacts
from chronaris.archive.legacy_public_benchmark.phase3_assets import (
    build_task_eval_dataset_summary,
    build_uab_session_window_comparison,
    load_task_eval_baseline_artifacts,
    read_task_eval_dataset_summary,
    summarize_artifact_bundle,
    write_local_baseline_report,
    write_prepared_dataset,
)
from chronaris.archive.legacy_public_benchmark.phase3_reporting import (
...
def run_task_eval_phase3(config: StageIPhase3Config) -> StageIPhase3RunResult:
    ...
    uab_summary = build_task_eval_dataset_summary(uab_entries, uab_feature_result)
    write_prepared_dataset(
    ...
    uab_baselines = run_task_eval_baselines(
    ...
    nasa_summary = build_task_eval_dataset_summary(nasa_entries, nasa_feature_result)
    write_prepared_dataset(
    ...
    nasa_baselines = run_task_eval_baselines(
...
def compose_task_eval_phase3_closure(
    *,
    ...
    uab_artifact_root = artifact_root / "uab_window"
    nasa_artifact_root = artifact_root / "nasa_attention"
    uab_summary = read_task_eval_dataset_summary(uab_artifact_root)
    nasa_summary = read_task_eval_dataset_summary(nasa_artifact_root)
    uab_baselines = load_task_eval_baseline_artifacts(uab_artifact_root, require_subjective=True)
    nasa_baselines = load_task_eval_baseline_artifacts(nasa_artifact_root)
    uab_report_path = write_local_baseline_report(
        artifact_root=uab_artifact_root,
        dataset_summary=uab_summary,
        baselines=uab_baselines,
...
def _write_closure_outputs(
    ...
    closure_report_path = artifact_root / "task_eval_phase3_closure_report.md"
    closure_report_path.write_text(render_task_eval_phase3_report(closure_summary) + "\n", encoding="utf-8")
