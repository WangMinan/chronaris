"""Contracts and loaders for task evaluation sequence-based deep baselines."""
...
class StageISequenceEntry:
    """One sequence sample prepared for task evaluation deep baselines."""
...
def dump_task_eval_sequence_entries(
...
def load_task_eval_sequence_entries(path: str | Path) -> tuple[StageISequenceEntry, ...]:
    rows = [
        json.loads(line)
...
def dump_task_eval_sequence_summary(
...
def load_task_eval_sequence_summary(path: str | Path) -> StageISequenceDatasetSummary:
...
def save_task_eval_sequence_bundle(
...
def load_task_eval_sequence_bundle(path: str | Path) -> StageISequenceBundle:
    with np.load(Path(path), allow_pickle=False) as payload:
        ...
        if missing:
            raise ValueError(
                "task evaluation sequence bundle is missing keys: "
                + ", ".join(missing)
            )
...
def _validate_sequence_entry_row(row: Mapping[str, object]) -> None:
    ...
    if missing:
        raise ValueError(
            "task evaluation sequence entry is missing required fields: "
            + ", ".join(missing)
        )
