"""Contracts and JSON helpers for task evaluation public-dataset tasks."""
...
class StageITaskEntry:
    """One task evaluation task sample backed by a public-dataset recording."""
...
class StageIDatasetSummary:
    """Compact machine-readable summary for one prepared task evaluation dataset."""
...
def dump_task_eval_task_entries(
    entries: Sequence[StageITaskEntry],
    *,
    path: str | Path,
) -> None:
    """Write task evaluation task entries as UTF-8 JSON Lines."""
...
def load_task_eval_task_entries(path: str | Path) -> tuple[StageITaskEntry, ...]:
    """Load task evaluation task entries from JSONL.
...
def dump_task_eval_summary(summary: StageIDatasetSummary, *, path: str | Path) -> None:
    """Write a task evaluation dataset summary as JSON."""
