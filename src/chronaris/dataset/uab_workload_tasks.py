"""UAB public-dataset adapter for task evaluation task manifests."""
...
from chronaris.dataset.task_contracts import StageITaskEntry, isoformat_utc
...
class UABPreparedTaskSet:
    """All UAB task evaluation entries plus helper metadata."""
...
def build_uab_task_entries(
    ...
    *,
    ...
    """Build the task evaluation task manifest for the local UAB dataset."""

    if profile not in {SESSION_V1, WINDOW_V2}:
        raise ValueError(f"unsupported UAB task evaluation profile: {profile}")
