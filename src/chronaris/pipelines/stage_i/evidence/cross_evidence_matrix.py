"""P32 Stage I private/public cross-evidence matrix builder."""
...
DEFAULT_ARTIFACT_ROOT = "docs/artifacts/assets/stage_i_cross_evidence_matrix"
DEFAULT_REPORT_ROOT = "docs/artifacts/stage_i"
DEFAULT_PUBLIC_FUSION_REFRESH_ROOT = "docs/artifacts/assets/stage_i_public_fusion_refresh/20260701T-stage-i-public-fusion-refresh-r1"
...
class StageICrossEvidenceMatrixConfig:
    ...
    private_ablation_root: str = "docs/artifacts/assets/stage_i_private_leakage_safe_ablation/20260619T-stage-i-leakage-safe-ablation-r2"
    public_model_comparison_root: str = "docs/artifacts/assets/stage_i_public_model_comparison/20260701T-stage-i-public-model-comparison-r1"
...
def build_stage_i_cross_evidence_matrix(
    ...
    report_path = _resolve_path(config.report_root) / f"stage-i-cross-evidence-matrix-{config.run_id}.md"
    ...
    for row in long_frame.to_dict(orient="records"):
        ...
        rows.append(
            _matrix_row(
                ...
                wording_boundary="T1/T2/T3 proxy tasks on private real dual-stream Stage H data",
            )
        )
    return rows
...
def _plot_quadrants(matrix: pd.DataFrame, path: str) -> None:
    ...
    axis.set_title("Stage I cross-evidence matrix row coverage")
...
def _render_summary_md(run_id: str, matrix: pd.DataFrame) -> str:
    ...
    return "\n".join(
        [
            f"# Stage I Cross-evidence Summary - {run_id}",
            "",
            "Stage I now contains a cross-evidence matrix covering private real dual-stream comparison, private leakage-safe component ablation, public model comparison and public fusion ablation.",
            "",
            "Quadrant row counts:",
            "",
...
def _render_report(summary: Mapping[str, object], matrix: pd.DataFrame) -> str:
    lines = [
        f"# Stage I Cross-evidence Matrix - {summary['run_id']}",
        "",
        "## Executive Summary",
        "",
        "Stage I now contains a cross-evidence matrix covering private real dual-stream comparison, private leakage-safe component ablation, public model comparison and public fusion ablation. "
        "The private Dingxin/Stage H branch evaluates Chronaris against MulT and ContiFormer under the same leakage-safe task protocol; the public NASA/UAB branch decomposes the P28 public fusion refresh result into lag, event-bias, context-stream and fusion-head contributions.",
        "",
        "## Matrix",
        "",
