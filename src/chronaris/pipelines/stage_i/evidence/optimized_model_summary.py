"""Stage I optimized model summary package builder."""
...
from chronaris.pipelines.stage_i.evidence.optimized_model_summary_render import (
    render_figures,
    render_report,
)
...
DEFAULT_ARTIFACT_ROOT = "docs/artifacts/assets/stage_i_optimized_model_summary"
...
def run_stage_i_optimized_model_summary(
    ...
    p34_root = _resolve_path(config.p34_root) if config.p34_root else _latest_root("stage_i_task_heads_optimization")
    p35_root = _resolve_path(config.p35_root) if config.p35_root else _latest_root("stage_i_stream_role_fusion")
    p36_root = _resolve_path(config.p36_root) if config.p36_root else _latest_root("stage_i_optimized_reevaluation")
    ...
    report_path = _resolve_path(config.report_root) / f"stage-i-optimized-model-summary-{config.run_id}.md"
...
def _resume_commands(
    ...
    commands.append(
        " ".join(
            [
                PYTHON,
                str(REPO_ROOT / "scripts/stage_i/evidence/build_optimized_model_summary.py"),
                "--run-id",
                config.run_id,
                "--p30-root",
                str(p30_root),
...
def _stage_boundary(stage: str) -> str:
    return {
        "P30": "Private proxy task third-party comparison; mixed result, not final human truth.",
    ...
    }.get(stage, "Stage I evidence artifact.")
