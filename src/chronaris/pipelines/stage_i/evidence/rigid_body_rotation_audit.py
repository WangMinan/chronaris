"""Rotation-field audit for Stage I rigid-body diagnostics."""
...
DEFAULT_ARTIFACT_ROOT = "docs/artifacts/assets/stage_i_rotation_audit"
...
def run_stage_i_rigid_body_rotation_audit(
    ...
    with open_stage_i_run_observer(
        ...
        stage_name="stage_i_rigid_body_rotation_audit",
        logger=LOGGER,
        initial_progress={"artifact_root": str(run_root), "sortie_id": config.sortie_id},
    ) as progress:
        return _run_stage_i_rigid_body_rotation_audit_observed(
            config=config,
            run_root=run_root,
            progress=progress,
        )
...
def _run_stage_i_rigid_body_rotation_audit_observed(
    *,
    config: StageIRigidBodyRotationAuditConfig,
    run_root: Path,
    ...
    summary = {
        ...
        "stage_h_feature_rotation_groups": _serialize_mapping_groups(refreshed_diagnostics.groups),
        "stage_h_missing_requirements": {
            key: list(value)
    ...
    report_path = report_root / f"stage-i-rigid-body-rotation-audit-{config.run_id}.md"
    report_path.write_text(
        render_stage_i_rigid_body_rotation_audit_report(summary) + "\n",
        encoding="utf-8",
    )
...
def render_stage_i_rigid_body_rotation_audit_report(summary: Mapping[str, object]) -> str:
    lines = [
        f"# Stage I Rigid-Body Rotation Audit - {summary['run_id']}",
        "",
    ...
    for group_name, rows in summary.get("stage_h_feature_rotation_groups", {}).items():
        matched = ", ".join(
    ...
    lines.extend(
        [
            ...
            f"3. 当前 Stage H 缺口：`{summary['stage_h_missing_requirements']}`。",
        ]
    )
    return "\n".join(lines)
