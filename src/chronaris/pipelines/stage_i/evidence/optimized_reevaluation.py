...
DEFAULT_ARTIFACT_ROOT = "docs/artifacts/assets/stage_i_optimized_reevaluation"
...
def run_stage_i_optimized_reevaluation(
    ...
    report_path = _resolve_path(config.report_root) / f"stage-i-optimized-chronaris-reevaluation-{config.run_id}.md"
...
def _render_report(summary: Mapping[str, object], model_summary: Mapping[str, object]) -> str:
    return "\n".join(
        [
            f"# Stage I Optimized Chronaris Re-evaluation - {summary['run_id']}",
            "",
...
def _resume_command(
    ...
    parts = [
        str(REPO_ROOT / "scripts/stage_i/evidence/build_optimized_chronaris_reevaluation.py"),
        "--run-id",
        config.run_id,
        "--p30-root",
        str(p30_root),
