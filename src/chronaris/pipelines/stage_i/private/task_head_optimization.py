...
DEFAULT_ARTIFACT_ROOT = "docs/artifacts/assets/stage_i_task_heads_optimization"
...
def run_stage_i_task_head_optimization(
    ...
    report_path = _resolve_path(config.report_root) / f"stage-i-task-aware-heads-{config.run_id}.md"
...
def _render_report(summary: Mapping[str, object]) -> str:
    return "\n".join(
        [
            f"# Stage I Task-aware Heads - {summary['run_id']}",
            "",
...
def _resume_command(config: StageITaskHeadOptimizationConfig) -> str:
    return (
        ...
        "scripts/stage_i/private/run_task_head_optimization.py "
