...
class StageIPublicFusionRefreshConfig:
    ...
    artifact_root: str = "docs/artifacts/assets/stage_i_public_fusion_refresh"
...
def run_stage_i_public_fusion_refresh(
    ...
    with open_stage_i_run_observer(
        ...
        stage_name="stage_i_public_fusion_refresh",
        logger=LOGGER,
        initial_progress={
        ...
        try:
            result = _run_stage_i_public_fusion_refresh_core(
                config=config,
            ...
            payload = {
                ...
                "artifact_root": str(artifact_root),
                "resume_command": (
                    "python scripts/stage_i/public/run_public_fusion_refresh.py "
            ...
            if config.allow_partial:
                report_path = (
                    ...
                    / f"stage-i-public-fusion-refresh-{config.run_id}.md"
                )
...
def _run_stage_i_public_fusion_refresh_core(
    *,
    config: StageIPublicFusionRefreshConfig,
    artifact_root: Path,
    progress,
    ...
    if runtime_device != "cuda" and config.require_cuda:
        raise RuntimeError(
            "stage_i_public_fusion_refresh requires CUDA but resolved "
    ...
    manifest_path = artifact_root / "evidence_manifest.json"
    report_path = _resolve_path(config.report_root) / f"stage-i-public-fusion-refresh-{config.run_id}.md"
...
def _render_refresh_report(run_id: str, summary: Mapping[str, object], confirm_frame: pd.DataFrame) -> str:
    ...
    uab = best.get("uab_workload_dataset") or {}
    lines = [
        f"# Stage I Public Fusion Refresh - {run_id}",
        "",
        "## Executive Summary",
        "",
    ]
