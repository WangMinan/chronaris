"""Small run-observer utilities for long task evaluation jobs."""
...
class StageIRunObserver:
    ...
    def __init__(
        ...
        *,
        ...
        self._namespace_logger = logging.getLogger("chronaris.pipelines.task_eval")
...
def open_task_eval_run_observer(
    *,
    run_root: str | Path,
    run_id: str,
    stage_name: str,
...
def configure_task_eval_cli_logging(stream: IO[str]) -> None:
    """Mirror task evaluation INFO logs to a CLI stream without relying on root logging."""

    namespace_logger = logging.getLogger("chronaris.pipelines.task_eval")
