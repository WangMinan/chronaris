...
from chronaris.evaluation.public_datasets.pipelines.opt_reporting import fmt_public_opt_float
...
def render_torch_uab_report(summary: Mapping[str, object]) -> str:
    ...
    lines = [
        "# task evaluation Public Opt UAB Torch Mainline",
        "",
        "## 运行口径",
        "",
    ...
    for subset_id in ("n_back", "heat_the_chair"):
        if subset_id not in selected_result["groups"]:
            lines.append(f"| {subset_id} | not_run | not_run | not_run | not_run |")
            continue
        metrics = selected_result["groups"][subset_id]
        group_selection = selected_result.get("group_selections", {}).get(subset_id, {})
        lines.append(
    ...
    if reference_public_opt or reference_deep:
        ...
        for subset_id in ("n_back", "heat_the_chair"):
            ...
            torch_group = selected_result["groups"].get(subset_id) or {}
