...
from chronaris.evaluation.public_datasets.pipelines.fusion_refresh import (
...
from chronaris.evaluation.public_datasets.pipelines.fusion_gpuopt_reporting import (
    build_optimization_summary,
    build_perf_summary,
    render_plots,
    render_report,
)
from chronaris.evaluation.public_datasets.pipelines.fusion_gpuopt_training import (
    evaluate_workload,
    maybe_compile_model,
    run_profile_batch,
    select_batch_size,
...
class StageIPublicFusionGPUOptConfig:
    run_id: str = "20260701T-task-eval-public-fusion-refresh-r1-gpuopt-r1"
    base_p28_run_id: str = "20260701T-task-eval-public-fusion-refresh-r1"
    base_p28_root: str = (
        "docs/artifacts/runs/"
        "20260701T-task-eval-public-fusion-refresh-r1"
    )
...
def run_task_eval_public_fusion_gpuopt(
    ...
    with open_task_eval_run_observer(
        ...
        stage_name="task_eval_public_fusion_gpuopt",
        logger=LOGGER,
        initial_progress={
...
def _run_gpuopt_core(
    *,
    ...
    report_path = _resolve_path(config.report_root) / f"task-eval-public-fusion-gpu-optimization-{config.run_id}.md"
...
def _resume_command(config: StageIPublicFusionGPUOptConfig) -> str:
    return (
        "python scripts/task_eval/public/run_public_fusion_gpuopt.py "
