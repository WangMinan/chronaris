...
def run_task_eval_public_fusion_screen(
    ...
    with open_task_eval_run_observer(
        ...
        stage_name="task_eval_public_fusion_screen",
        logger=LOGGER,
        initial_progress={
        ...
        return _run_task_eval_public_fusion_screen_observed(
            config=config,
...
def _run_task_eval_public_fusion_screen_observed(
    *,
    config: StageIPublicFusionScreenConfig,
    artifact_root: Path,
    ...
    LOGGER.info(
        "task_eval_public_fusion_screen start run_id=%s requested_device=%s resolved_device=%s datasets=%s",
        config.run_id,
        config.device,
        runtime_device,
    ...
    if runtime_device != "cuda":
        if config.require_cuda:
            raise RuntimeError(
                "task_eval_public_fusion_screen requires CUDA but resolved runtime_device="
                f"{runtime_device}. Use --device cuda on a CUDA host or pass "
                "--allow-cpu-debug for explicit debugging only."
            )
        LOGGER.warning(
            "task_eval_public_fusion_screen using CPU fallback run_id=%s requested_device=%s",
            config.run_id,
            config.device,
        )
    ...
    for dataset_id, prepared_root in config.dataset_prepared_roots.items():
        ...
        LOGGER.info(
            "task_eval_public_fusion_screen dataset start dataset_id=%s candidate_count=%d",
            dataset_id,
            len(candidates),
        )
        ...
        for candidate_index, candidate in enumerate(candidates, start=1):
            LOGGER.info(
                "task_eval_public_fusion_screen dataset=%s candidate %d/%d candidate_id=%s",
                dataset_id,
                candidate_index,
                len(candidates),
            ...
            LOGGER.info(
                "task_eval_public_fusion_screen dataset=%s candidate=%s metric=%s selection_score=%.4f",
                dataset_id,
                candidate.candidate_id,
        ...
        if ordered_rows:
            LOGGER.info(
                "task_eval_public_fusion_screen dataset complete dataset_id=%s best_candidate=%s",
                dataset_id,
    ...
    report_path = report_root / f"task-eval-public-fusion-screen-{config.run_id}.md"
    ...
    LOGGER.info(
        "task_eval_public_fusion_screen finished run_id=%s summary_path=%s report_path=%s",
        config.run_id,
        summary_path,
        report_path,
    )
...
def _render_public_fusion_screen_report(summary: Mapping[str, object]) -> str:
    lines = [
        f"# task evaluation Public Fusion Screen - {summary['run_id']}",
        "",
