...
from chronaris.evaluation.public_datasets.pipelines.opt_postprocess import apply_public_opt_regression_prediction_aggregation
...
from chronaris.evaluation.public_datasets.pipelines.opt_torch_catalog import TorchUABCandidateSpec
from chronaris.evaluation.public_datasets.pipelines.opt_torch_models import (
...
from chronaris.evaluation.public_datasets.pipelines.opt_torch_supervision import (
...
def _run_torch_uab_candidate(
    *,
    ...
    for subset_id in selected_subsets:
        LOGGER.info(
            "task_eval_public_opt_torch candidate=%s subset=%s start",
        ...
        for fold_index, split in enumerate(loso_splits):
            ...
            LOGGER.info(
                "task_eval_public_opt_torch candidate=%s subset=%s fold=%d/%d train=%d test=%d held_out=%s",
                candidate.candidate_id,
                subset_id,
                fold_index + 1,
                len(loso_splits),
        ...
        LOGGER.info(
            "task_eval_public_opt_torch candidate=%s subset=%s done rmse=%.4f mae=%.4f",
            candidate.candidate_id,
            subset_id,
            float(metrics["rmse"]),
            float(metrics["mae"]),
