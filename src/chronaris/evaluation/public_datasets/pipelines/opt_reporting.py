"""Markdown reporting helpers for task evaluation public-opt runs."""
...
def render_task_eval_public_opt_report(
    *,
    ...
    track_title = (
        "task evaluation Public Opt UAB Subjective Run"
        if summary["track"] == "subjective"
        else "task evaluation Public Opt NASA Attention Run"
    )
    lines = [
        f"# {track_title}",
        "",
    ...
    lines.extend(
        [
            ...
            "## 说明",
            "",
            "- `public opt` 只迁移 Chronaris 的公开 sequence-contract 思路，不改写既有 task evaluation 公开 benchmark 历史事实。",
