...
def run_task_eval_stream_role_fusion_eval(
    config: StageIStreamRoleFusionEvalConfig,
    ...
    route_manifest = {
        ...
        "boundary": "public context proxy is not real_vehicle; private feature export second stream is real_vehicle",
    }
    ...
    report_path = _resolve_path(config.report_root) / f"task-eval-stream-role-aware-fusion-{config.run_id}.md"
...
def _run_route_smoke(device: str) -> list[dict[str, object]]:
    ...
    for dataset_id, metadata, variant in (
        ("private_feature_export", private_stream_metadata(), "chronaris_v3_stream_role"),
...
def _resume_command(config: StageIStreamRoleFusionEvalConfig) -> str:
    parts = [
        ...
        "scripts/task_eval/evidence/run_stream_role_fusion_eval.py",
        "--resume",
        "--resume-run-id",
        config.run_id,
        "--device",
...
def _render_report(summary: Mapping[str, object]) -> str:
    return "\n".join(
        [
            f"# task evaluation Stream-role-aware Fusion - {summary['run_id']}",
            "",
