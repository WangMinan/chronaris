"""Runtime helpers for task evaluation deep baseline pipelines."""
...
from chronaris.modeling.common.deep_models import StageIDeepForwardResult
...
if TYPE_CHECKING:
    from chronaris.evaluation.public_datasets.pipelines.deep_baseline import StageIDeepBaselineConfig
...
def _load_prepared_sequence_dataset(artifact_root: str | Path) -> dict[str, object]:
    ...
    entries = load_task_eval_sequence_entries(root / "task_manifest.jsonl")
    bundle = load_task_eval_sequence_bundle(root / "sequence_bundle.npz")
    summary = load_task_eval_sequence_summary(root / "dataset_summary.json")
...
def _load_reference_comparison(
    *,
    ...
    if not root.exists():
        return None
    artifacts = load_task_eval_baseline_artifacts(
        root,
