"""Preparation pipeline for task evaluation deep-baseline sequence assets."""
...
class StageISequencePreparationConfig:
    ...
    feature_export_run_manifest_path: str | None = None
...
def run_task_eval_sequence_preparation(
    ...
    with open_task_eval_run_observer(
        ...
        stage_name="task_eval_sequence_preparation",
        logger=LOGGER,
        initial_progress={
        ...
        return _run_task_eval_sequence_preparation_observed(
            config=config,
...
def _run_task_eval_sequence_preparation_observed(
    *,
    config: StageISequencePreparationConfig,
    artifact_root: Path,
    progress,
    ...
    LOGGER.info(
        "task_eval_sequence_preparation start dataset_id=%s artifact_root=%s dataset_root=%s",
        config.dataset_id,
        artifact_root,
        config.dataset_root,
    )
    ...
    dump_task_eval_sequence_entries(entries, path=manifest_path)
    save_task_eval_sequence_bundle(payload.bundle, path=bundle_path)
    dump_task_eval_sequence_summary(payload.summary, path=summary_path)
    ...
    LOGGER.info(
        "task_eval_sequence_preparation finished dataset_id=%s summary_path=%s diagnostics_path=%s",
...
def _prepare_payload(
    ...
    *,
    ...
    if normalized == STAGE_H_CASE_DATASET_ID:
        if not config.feature_export_run_manifest_path:
            raise ValueError("feature_export_case preparation requires feature_export_run_manifest_path.")
        return prepare_feature_export_case_sequences(
            config.feature_export_run_manifest_path,
    ...
    raise ValueError(f"unsupported task evaluation deep sequence dataset: {config.dataset_id}")
...
def _record_sequence_preparation_progress(
    ...
    **fields: object,
    ...
    LOGGER.info(
        "task_eval_sequence_preparation %s %s",
        event,
