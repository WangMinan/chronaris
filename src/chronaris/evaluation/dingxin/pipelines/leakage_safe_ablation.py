"""Leakage-safe Dingxin task evaluation component-diagnostic ablation protocol."""
...
from chronaris.modeling.common.baseline_models import StageIBaselineLosoSplit, build_loso_splits
from chronaris.modeling.common.run_observer import StageIRunProgress, open_task_eval_run_observer
...
from chronaris.evaluation.dingxin.pipelines.benchmark_feature_helpers import aggregate_field_score
...
def run_task_eval_leakage_safe_ablation(
    ...
    with open_task_eval_run_observer(
        ...
        stage_name="task_eval_private_leakage_safe_ablation",
        logger=LOGGER,
...
def _run_observed(
    *,
    ...
    report_path = report_root / f"task-eval-private-leakage-safe-ablation-{config.run_id}.md"
...
def render_leakage_safe_ablation_report(summary: Mapping[str, object]) -> str:
    lines = [
        f"# task evaluation Leakage-Safe Dingxin Component-Diagnostic Ablation - {summary['run_id']}",
        "",
