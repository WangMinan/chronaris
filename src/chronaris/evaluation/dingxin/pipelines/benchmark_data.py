...
from chronaris.evaluation.dingxin.pipelines.benchmark_feature_helpers import (
...
from chronaris.evaluation.dingxin.pipelines.feature_utils import (
    bucketize_score,
    none_if_empty,
    read_jsonl,
    safe_mean,
...
PRIVATE_DATASET_ID = "private_feature_export_benchmark"
VARIANT_ORDER = (
    "naive_sync",
    "e_baseline",
    "f_full",
...
def load_aligned_private_records(
    *,
    ...
    e_run = load_feature_export_feature_run(e_run_manifest_path)
    f_run = load_feature_export_feature_run(f_run_manifest_path)
    validate_private_feature_export_run_contract(e_run.run_manifest, stage_name="E")
    validate_private_feature_export_run_contract(f_run.run_manifest, stage_name="F")
    ...
    if set(e_views) != set(f_views):
        raise ValueError("E/F feature export runs must export the same view ids.")
...
def validate_private_feature_export_run_contract(
    run_manifest: Mapping[str, object],
    *,
    stage_name: str,
) -> None:
    """Reject feature export runs that cannot support the private E/F comparison."""

    config = run_manifest.get("config", {})
    if not isinstance(config, Mapping):
        raise ValueError(f"{stage_name} feature export run manifest is missing config.")
    run_id = str(run_manifest.get("run_id") or stage_name)
    if config.get("intermediate_partition") != "all":
        raise ValueError(f"{stage_name} feature export run {run_id} must use intermediate_partition='all'.")
    if run_manifest.get("partial_data") is not None:
        raise ValueError(f"{stage_name} feature export run {run_id} must disable partial-data sidecar.")
    if config.get("causal_fusion_enabled") is not False:
        raise ValueError(f"{stage_name} feature export run {run_id} must disable causal fusion.")
    if config.get("physiology_point_limit_per_measurement") is not None:
        raise ValueError(f"{stage_name} feature export run {run_id} must not cap physiology points.")
    if config.get("vehicle_point_limit_per_measurement") is not None:
        raise ValueError(f"{stage_name} feature export run {run_id} must not cap vehicle points.")

    physics_enabled = config.get("physics_constraints_enabled")
    if stage_name == "E":
        if physics_enabled is not False:
            raise ValueError(f"E feature export run {run_id} must set physics_constraints_enabled=false.")
        return
    if stage_name == "F":
        if physics_enabled is not True:
            raise ValueError(f"F feature export run {run_id} must set physics_constraints_enabled=true.")
        if config.get("physics_constraint_family") != "full":
            raise ValueError(f"F feature export run {run_id} must use physics_constraint_family='full'.")
        return
    raise ValueError(f"unsupported private feature export stage name: {stage_name}")
...
def build_variant_feature_frames(
    ...
    *,
    ...
    if enable_optimized_chronaris:
        from chronaris.evaluation.dingxin.pipelines.optimization import (
