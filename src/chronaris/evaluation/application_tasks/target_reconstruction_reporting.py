...
def _report_text(*, protocol, allowance, best, frames):
    ...
    conclusion = (
        "任务 1C 的全部训练内门禁通过，可以进入安全基座与 Chronaris 残差训练。"
        if allowance["allow_safe_fusion"]
        else "任务 1C 未通过全部训练内门禁，按协议不启动主干优化或教师蒸馏。"
    )
    return f"""# 鼎新目标重构、时间捷径抑制与同合同基线报告
...
- 未来机动连续分数：最佳方法为 `{METHOD_LABELS[score['method']]}`，6 个主选模单元的中位 Spearman 为 `{score['median']:.4f}`。
- 未来机动趋势：最佳方法为 `{METHOD_LABELS[trend['method']]}`，外层训练池平衡平均 Macro-F1 为 `{trend['mean']:.4f}`，最差单元为 `{trend['worst']:.4f}`。
- 生理惯性残差：最佳方法为 `{METHOD_LABELS[residual['method']]}`，中位 RMSE 比率为 `{residual.get('median_rmse_ratio', float('nan')):.4f}`，平均技能为 `{residual.get('mean_skill', float('nan')):.4f}`，正技能单元数为 `{residual.get('positive_skill_split_count', 0)}/6`。
...
- 决策：`{allowance['decision']}`。
- `allow_safe_fusion={str(allowance['allow_safe_fusion']).lower()}`。
- `allow_task_aware_research={str(allowance['allow_task_aware_research']).lower()}`。
- 未通过项：`{', '.join(failed) if failed else '无'}`。
- 首要阻断：`{allowance['primary_blocker']}`。

## 证据边界

- 本轮新任务属于鼎新真实双流上的弱监督任务重构，用于检查连续动力学与生理惯性之外的可预测增量，不是新增人工专家真值。
- `legacy panel` 只保留历史连续性；本报告的 `matched-clean panel` 采用新的统一输入和目标合同，两者不混算。
...
def _configure_chinese_fonts():
    preferred = ("Noto Sans CJK SC", "Source Han Sans SC", "WenQuanYi Micro Hei", "Microsoft YaHei")
