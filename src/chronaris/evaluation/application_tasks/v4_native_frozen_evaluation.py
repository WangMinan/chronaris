...
def run_native_frozen_evaluation(*, domain, fold_index, checkpoint, checkpoint_sha256, route, output_root,
                                 ...
                                 device="cuda", engineering_only=False, minirocket_kernels=10_000):
    ...
    if not engineering_only:
        if device != "cuda" or minirocket_kernels != 10_000:
    ...
    if payload["seed"] not in (17, 29, 43) or (not engineering_only and payload["config"]["device"] != "cuda"):
    ...
    source = {"format": "chronaris.v4_native_frozen_evaluation.v1", "domain": domain, "route": route,
        ...
        "encoder_checkpoint_selection_uses_validation_labels": route == "task_guided",
        "configuration_selection_uses_validation_labels": True}
    ...
    with _periodic_training_heartbeat("native_frozen_evaluation", 30., root=root) as progress:
        ...
        state["consumers"] = run_native_method_consumers(outputs=outputs, targets=targets, definitions=definitions,
            context=context, output_root=root / "consumers", label_used_for_encoder_training=route == "task_guided",
