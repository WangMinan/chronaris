"""task evaluation LLM preprocessing comparison over existing evidence assets."""
...
from chronaris.dataset import load_task_eval_private_task_entries
from chronaris.modeling.common.run_observer import (
    StageIRunProgress,
    open_task_eval_run_observer,
)
from chronaris.llm_preprocessing.comparison_builders import (
...
from chronaris.llm_preprocessing.comparison_reporting import (
    render_task_eval_llm_comparison_report,
    render_task_eval_llm_midterm_summary,
)
...
DEFAULT_LLM_COMPARISON_ARTIFACT_ROOT = "docs/artifacts/runs"
DEFAULT_LLM_COMPARISON_REPORT_ROOT = "docs/artifacts/runs"
DEFAULT_LLM_COMPARISON_MIDTERM_ROOT = "docs/midterm"
DEFAULT_LLM_CONTEXT_PATH = (
    "docs/artifacts/runs/"
    "20260614T-task-eval-p20-deepseek-llm-preprocessing-r3-sliced/"
    "llm_preprocessing_context.json"
)
DEFAULT_LLM_COMPARISON_RUN_ID = "20260614T-task-eval-p21-llm-comparison-r1"
...
def run_task_eval_llm_comparison(
    ...
    with open_task_eval_run_observer(
        ...
        stage_name="task_eval_llm_comparison",
        logger=LOGGER,
        initial_progress={
...
def _load_sources(config: StageILLMComparisonConfig) -> dict[str, object]:
    ...
    return {
        "llm_context": context,
        "task_entries": load_task_eval_private_task_entries(config.task_manifest_path),
...
def _write_outputs(
    *,
    ...
    report_path = report_root / f"task-eval-llm-comparison-{config.run_id}.md"
    ...
    midterm_summary_path.write_text(
        render_task_eval_llm_midterm_summary(summary=summary, condition_manifest=condition_manifest)
        + "\n",
        encoding="utf-8",
    )
    return {
