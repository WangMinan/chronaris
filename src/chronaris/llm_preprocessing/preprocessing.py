"""task evaluation DeepSeek/LLM preprocessing context builder."""
...
from chronaris.llm_preprocessing.harness import (
    build_harness_summary,
    run_preprocessing_llm_task,
)
from chronaris.llm_preprocessing.slicing import (
...
from chronaris.llm_preprocessing.reporting import (
    render_task_eval_llm_preprocessing_report,
)
...
DEFAULT_MULTITASK_SUMMARY_PATH = (
    "docs/artifacts/runs/"
    "20260607T-task-eval-multitask-real-closure-r2/multitask_summary.json"
)
DEFAULT_TASK_MANIFEST_PATH = (
    "docs/artifacts/runs/"
    "20260607T-task-eval-multitask-real-closure-r2/thesis_task_manifest.jsonl"
)
DEFAULT_LIVE_SWEEP_SUMMARY_PATH = (
    "docs/artifacts/runs/"
    "20260613T-task-eval-p11-live-influx-r3-resume/multitask_sweep_summary.json"
)
DEFAULT_SUPPORT_SUMMARY_PATH = (
    "docs/artifacts/runs/"
    "20260607T-task-eval-support-semantic-r2/support_summary.json"
)
DEFAULT_RUNTIME_SCHEMA_CONTRACT_PATH = (
    "docs/artifacts/runs/"
    "20260613T-task-eval-runtime-service-smoke-r2-contract/runtime_schema_contract.json"
)
DEFAULT_RUNTIME_SERVICE_SUMMARY_PATH = (
    "docs/artifacts/runs/"
    "20260613T-task-eval-runtime-service-smoke-r2-contract/runtime_service_smoke_summary.json"
)
DEFAULT_RUNTIME_CASE_TABLE_PATH = (
    "docs/artifacts/runs/"
    "20260613T-task-eval-thesis-materials-r2-p18/runtime_semantic_case.csv"
)
...
class StageILLMPreprocessingConfig:
    """Configuration for one task evaluation LLM preprocessing run."""
...
class StageILLMPreprocessingRunResult:
    """Artifacts written by the task evaluation LLM preprocessing pipeline."""
...
def _load_sources(config: StageILLMPreprocessingConfig) -> dict[str, object]:
    entries = load_task_eval_private_task_entries(config.task_manifest_path)
...
def _write_outputs(
    *,
    ...
    report_path = report_root / f"task-eval-llm-preprocessing-{config.run_id}.md"
    ...
    report_path.write_text(
        render_task_eval_llm_preprocessing_report(summary=summary, comparison_rows=comparison_rows)
        + "\n",
        encoding="utf-8",
    )
    return {
