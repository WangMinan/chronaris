"""Sequence export for task evaluation deep baselines."""
...
from chronaris.dataset.nasa_csm_tasks import DATASET_ID as NASA_DATASET_ID
from chronaris.dataset.nasa_csm_tasks import (
    WINDOW_DURATION_SECONDS as NASA_WINDOW_DURATION_SECONDS,
)
from chronaris.dataset.uab_workload_tasks import DATASET_ID as UAB_DATASET_ID
from chronaris.dataset.uab_workload_tasks import (
    WINDOW_DURATION_SECONDS as UAB_WINDOW_DURATION_SECONDS,
)
from chronaris.features.case_study import load_task_eval_case_study_run
from chronaris.features.task_feature_helpers import (
    build_window_specs,
    entry_iso_window_bounds,
    group_entries_by_csv_path,
)
from chronaris.features.sequence_helpers import (
...
STAGE_H_CASE_DATASET_ID = "feature_export_case"
...
def prepare_feature_export_case_sequences(
    ...
    *,
    ...
    modality_schema = _build_bimodal_schema(
        ...
        notes={
            "physiology": "feature export physiology_reference_projection",
            "vehicle": "feature export vehicle_reference_projection",
        },
    )
    ...
    for view in run_input.views:
        ...
        for sample_index, raw_sample_id in enumerate(view.sample_ids):
            ...
            time_values = view.feature_export_view.reference_offsets_s[sample_index].astype(
                np.float32,
            )
            physiology = view.feature_export_view.physiology_reference_projection[
                sample_index
            ].astype(np.float32)
            vehicle = view.feature_export_view.vehicle_reference_projection[
                sample_index
            ].astype(np.float32)
            ...
            payload = {
                ...
                "artifact_paths": dict(
                    view.feature_export_view.view_manifest.get("artifact_paths", {}),
                ),
            }
            metadata_json.append(json.dumps(payload, ensure_ascii=False))
            event_scores.append(
                view.feature_export_view.vehicle_event_scores[sample_index].astype(np.float32),
            )
            attention_weights.append(
                view.feature_export_view.attention_weights[sample_index].astype(np.float32),
            )
            ...
            entries.append(
                StageISequenceEntry(
                    ...
                    source_origin="feature_export_closure_validation",
