"""task evaluation Phase 2 helpers for consuming frozen feature export assets."""
...
from chronaris.feature_export.bundle import StageHFeatureRun, StageHFeatureView, load_feature_export_feature_run


@dataclass(frozen=True, slots=True)
class StageICaseStudyWindowRow:
    """One window-manifest row associated with a feature export view."""
...
class StageICaseStudyViewInput:
    """One Phase 2 view assembled from a feature export bundle plus sidecars."""

    view_id: str
    sortie_id: str
    pilot_id: int
    projection_diagnostics_verdict: str
    feature_export_view: StageHFeatureView
...
class StageICaseStudyRunInput:
    """All Phase 2 views derived from one feature export run."""

    run_manifest_path: str
    feature_export_run: StageHFeatureRun
    views: tuple[StageICaseStudyViewInput, ...]


def load_task_eval_case_study_run(run_manifest_path: str | Path) -> StageICaseStudyRunInput:
    """Load all Phase 2-consumable inputs from one feature export run manifest."""

    feature_export_run = load_feature_export_feature_run(run_manifest_path)
    views = tuple(_load_case_view(view) for view in feature_export_run.views)
    return StageICaseStudyRunInput(
        run_manifest_path=str(Path(run_manifest_path)),
        feature_export_run=feature_export_run,
        views=views,
    )
