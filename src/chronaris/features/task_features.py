"""Feature extraction for task evaluation public-dataset baselines."""
...
from chronaris.dataset.task_contracts import StageITaskEntry
...
def build_uab_feature_table(
    ...
    """Build the task evaluation feature table for the local UAB dataset."""

    active_entries = tuple(entry for entry in entries if entry.dataset_id == UAB_DATASET_ID)
    if not active_entries:
        raise ValueError("no UAB task evaluation task entries were provided.")
...
def build_nasa_csm_feature_table(
    ...
    """Build the task evaluation feature table for the local NASA CSM dataset."""

    active_entries = tuple(entry for entry in entries if entry.dataset_id == NASA_DATASET_ID)
    if not active_entries:
        raise ValueError("no NASA CSM task evaluation task entries were provided.")
    if {entry.sample_granularity for entry in active_entries} != {"window"}:
        raise ValueError("NASA CSM task evaluation currently supports window samples only.")
