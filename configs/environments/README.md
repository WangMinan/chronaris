...
3. `chronaris-task-eval-cpu.yml`
   适用于任务评估的 CPU 基线、公开数据准备与 sklearn 评测
4. `chronaris-task-eval-gpu.yml`
   适用于任务评估后续需要补充 GPU 依赖时的统一环境入口
...
2. `mne` 当前是任务评估公开 EEG/ECG 数据处理的预留依赖，首轮 UAB baseline 本身不强依赖它
...
conda env create -f configs/environments/chronaris-task-eval-cpu.yml
conda env create -f configs/environments/chronaris-task-eval-gpu.yml
```
...
conda env update -n chronaris -f configs/environments/chronaris-task-eval-cpu.yml --prune
conda env update -n chronaris -f configs/environments/chronaris-task-eval-gpu.yml --prune
```
...
如果是任务评估的公开数据准备与 CPU baseline，默认优先使用：
