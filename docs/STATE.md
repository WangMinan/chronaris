...
固定数据下游评估与完整论文实验长程 goal 正在执行，当前分支为 `codex/fixed-data-downstream-evaluation-20260710`。仿真 clean 锁定主表、时间机制恢复和端到端辅助表均已完成：Chronaris 在线性负荷探针、机动状态/片段质量、时钟偏移和响应时延绝对误差上形成优势，MulT 在 MiniRocket 负荷任务、边界定位和时延排序相关性上领先。鼎新正式三随机种子五折重训已完成前两个留一视图折的 30/30 单元，并进入第三折；当前累计 30/75，统一使用 100 ms 方法无关因果时间箱与唯一 GPU，任务目标和 outer-test 保持关闭。Chronaris 消融已完成 10/12 个训练单元；压力 consumer 已完成 59/105 个种子—场景单元，为避免 CPU 超额调度暂挂在可恢复点，消融表示完成后自动续跑。
...
- 鼎新锁定重训：3 seeds × 5 个外层折 × 5 个选定配置共 75 个训练单元；前两个留一视图折均 15/15 完成，累计 30/75，已进入第三个留一视图折。第二折 Chronaris 三个 seed 的最佳 epoch 为 15/10/46，均由公共自监督 validation 决定。全部单元统一使用 100 ms 因果时间箱和唯一 GPU，任务目标、outer-test 表示和指标继续关闭。
...
- Chronaris 机制消融：四个变体 × 三个随机种子的 CPU 锁定重训已完成 10/12 个单元，seed 43 已进入无物理一致性变体。任务真值继续关闭，12 个 checkpoint 完整后才自动进入消融表示导出。
...
- 鼎新 real-only 轨道尚需完成余下 45 个 checkpoint、270 份统一表示和冻结 consumer；表示导出前任务目标和 outer-test 仍关闭。
- 仿真压力 consumer 尚余 46/105 个种子—场景单元；完成后需审计七因素退化斜率与配对统计。
- Chronaris 消融尚余 2 个训练单元、12 个表示单元和统一 consumer；仿真预训练到鼎新适配、论文证据包和图表抽查也尚待自动队列完成。
