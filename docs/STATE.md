...
固定数据下游评估与完整论文实验长程 goal 正在执行，当前分支为 `codex/fixed-data-downstream-evaluation-20260710`。仿真 clean 锁定主表与时间机制恢复均已完成：Chronaris 在线性负荷探针、机动状态/片段质量、时钟偏移和响应时延绝对误差上形成优势，MulT 在 MiniRocket 负荷任务、边界定位和时延排序相关性上领先。鼎新正式三随机种子五折重训已完成首个留一视图折全部 15 单元，并进入第二折；当前累计 18/75，统一使用 100 ms 方法无关因果时间箱与唯一 GPU，任务目标和 outer-test 保持关闭。端到端辅助表仅余 seed 43 Chronaris；压力 consumer 与消融 CPU 队列为保障鼎新吞吐暂挂在可恢复点，辅助表完成后自动恢复。
...
- 仿真压力主线：3 seeds × 35 场景 × 6 方法共 630 份 `[192,96,64]` 表示全部完成，5/5 门禁通过；冻结 consumer 已启动并完成前四个抖动场景的部分 seed，为保障鼎新训练 CPU 带宽暂挂在逐场景恢复点，端到端辅助表完成后自动继续。
...
- 鼎新锁定重训：3 seeds × 5 个外层折 × 5 个选定配置共 75 个训练单元；首个留一视图折 15/15 完成，第二折已完成 seed 17 的生理单流、航电单流和 MulT，累计 18/75。全部单元统一使用 100 ms 因果时间箱和唯一 GPU，任务目标、outer-test 表示和指标继续关闭。
