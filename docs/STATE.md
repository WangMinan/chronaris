...
固定数据下游评估与完整论文实验长程 goal 正在执行，当前分支为 `codex/fixed-data-downstream-evaluation-20260710`。仿真 clean 锁定主表、时间机制恢复、端到端辅助表和七因素压力评估均已完成：Chronaris 在线性负荷探针、机动状态/片段质量、时钟偏移和响应时延绝对误差上形成优势，MulT 在 MiniRocket 负荷任务、边界定位和时延排序相关性上领先。压力结果进一步表明 Chronaris 在 21 个最高单因素压力组合中有 15 个绝对表现位于前二，但随机/连续缺失退化斜率均为第六，不形成全因素鲁棒性优势。鼎新正式重训已完成三个留一视图主折和两个留一架次辅助折的 75/75 单元，8/8 门禁通过；统一使用 100 ms 方法无关因果时间箱与唯一 GPU，任务目标和 outer-test 在训练期间保持关闭。75 个 checkpoint 完整后，270 份六方法三角色表示已开始自动导出。Chronaris 消融的 12 个 checkpoint 和 36 份三角色表示也已完成。
...
- 鼎新锁定重训：3 seeds × 5 个外层折 × 5 个选定配置共 75/75 个训练单元全部完成，8/8 门禁通过。三个留一视图主折和两个留一架次辅助折均为 15/15；15 个 Chronaris 单元最佳 epoch 范围 8–46，均由公共自监督 validation 决定。全部单元统一使用 100 ms 因果时间箱和唯一 GPU，任务目标、outer-test 访问和指标在训练期间均为零。当前正在导出 3 seeds × 5 folds × 6 methods × 3 roles 共 270 份统一表示。
...
- 鼎新 real-only 轨道尚需完成 270 份统一表示和冻结 consumer；表示导出前已确认 75 个 checkpoint 完整，任务目标和 outer-test 指标仍关闭。
