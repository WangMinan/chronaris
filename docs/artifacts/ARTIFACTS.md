...
- `runs/2026-07-08_chronaris-controlled-optimization-dev/`：**Chronaris 受控候选优化开发 run**。在固定 split / labels、固定 MulT / ContiFormer baseline 和固定 E3 evaluator 下注册 8 个 Chronaris 候选，8 个 smoke 通过、8 个 dev run 完成；Pareto 选择 `chr_v2_residual_delta_h64`。包含 `candidate_registry.csv`、`candidate_t1_t2_metrics.csv`、`candidate_e3_metrics.csv`、`candidate_summary.json`、`pareto_selection.md`、`failure_cases.md`、`paper_boundary.md` 和 `evidence_manifest.json`；`confirmed_metrics_changed=false`，未回写论文协议快照。
- `runs/2026-07-08_chronaris-controlled-optimization-confirm/`：**Chronaris locked confirmation run**。锁定候选为 `chr_v2_residual_delta_h64`；回归任务 RMSE 为 347.009106，相对旧 Chronaris 838.121039 明显改善，但仍略弱于 MulT 344.288975 与 ContiFormer 344.335110；分类任务 macro-F1 为 0.166667，balanced accuracy 为 0.333333；E3 只有 `motif_event_consistency` 从 0 到 1 的正向信号。包含 `locked_candidate_config.json`、`locked_candidate_t1_t2_metrics.csv`、`locked_candidate_e3_metrics.csv`、`locked_confirmation_report.md` 和 `evidence_manifest.json`；论文建议位置是补充或附录诊断，不作为正文主优势结论。
- `runs/2026-07-08_chronaris-oof-representation-export/`：**Chronaris OOF 表示导出 run**。本 run `training_invoked=true`，表示族为 `Chronaris_T2_response_lovo_seed17_pooled_embedding`；8 个候选共 972 行 held-out pooled embedding，selected candidate 含 dev 与 locked confirmation 两组输出。包含 `chronaris_oof_embeddings_long.csv`、`chronaris_oof_embeddings_summary.json`、`checkpoint_manifest.csv/json`、`representation_manifest.json`、`fold_status.csv`、`training_curves.csv` 和 `evidence_manifest.json`；不改 confirmed metrics、不回写论文协议快照。
- `runs/2026-07-08_e3-result-review/`：**E3 结果审查与论文可用性判断**。本 run 只做审查、汇总和下一步决策，`training_invoked=false`、`tuning_invoked=false`、`e3_rerun_invoked=false`；读取四方法 Dingxin E3 validation、deep baseline representation export 和分类/回归任务上下文后，结论为 `D. needs_chronaris_optimization`。当前 E3 对 Chronaris 是 mixed、非差异性结果：四方法各 108 行输入完整，但核心方向指标多为并列或零信号，CLaP 在 12 条流上不可用；不建议作为论文正文优势证据，仅作为附录诊断或下一轮受控优化依据。包含 `report.md`、`paper_use_decision.md`、`e3_method_metric_summary.csv`、`e3_status_summary.csv`、`t1_t2_e3_consistency_table.csv`、`figure_inventory.csv`、`figure_candidate_table.csv`、`risk_and_boundary.md`、`next_prompt_for_chronaris_controlled_optimization.md` 和 `evidence_manifest.json`。
