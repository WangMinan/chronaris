...
- `runs/2026-07-12_chronaris-v2-protocol-lock/`：**Chronaris v2 开发边界与晋级协议锁定**。记录 `main@23b968e` 基线、三个既有确认表的 SHA-256、固定数据/标签/外层折/冻结消费者/`[B,96,64]`/v1 checkpoint 六项不可变边界，以及 seeds 17/29/43 的六主指标严格晋级阈值和附加门禁。候选选择只允许任务无关证据；任一主指标未居首即保留 v1，既有确认指标不回写。
- `runs/2026-07-12_chronaris-v1-distortion-diagnostics/`：**Chronaris v1 任务无关表示失真诊断**。seeds 17/29/43 的表示健康、航电/生理保真、模态反事实、注意力/门控和多损失梯度冲突完成，7/7 验收通过。表示有效秩约 22，未发生整体塌缩；航电/生理恢复 R² 均值为 0.9241/0.9319。旧时延注意力近似均匀比例为 0.978–0.995，门控集中在最短档，多损失最小梯度余弦为 -0.5743，定位结果指向航电信息路径、因果注意力和联合优化，而不是池化后的整体秩塌缩。
- `runs/2026-07-12_chronaris-v2-training-smoke-r2/`：**Chronaris v2 G1 observed-only 训练协议验证**。完整 v2 候选完成单 epoch CUDA 训练、版本化 checkpoint 回载和 24 项网格注册，7/7 验收通过。任务标签、仿真任务真值与锁定测试均保持关闭；本 run 只证明训练/恢复协议可执行，不形成候选排名。
- `runs/2026-07-12_chronaris-v2-structure-screen-smoke/`：**Chronaris v2 结构候选训练烟雾验证**。不可变 v1 参考与“语义组编码 + 旧余弦融合”候选均完成，4/4 训练层门禁通过；v2 候选真实输出 `[N,96,64]`，checkpoint 记录结构候选、参数量和禁止访问字段。本 run 只验证逐项结构开关，不形成正式结构结论。
- `runs/2026-07-12_chronaris-v2-structure-diagnostics-smoke/`：**Chronaris v2 结构候选任务无关门禁烟雾验证**。对 v1 与单 epoch 语义组候选实跑单流保真、有效秩、合成时钟偏移/响应时延恢复、未来扰动不变性、lag mask 和无效位置池化；因果/池化门禁均通过，但两个候选的航电保真均未达到 98% 硬门，因此烟雾结果为 `gates_failed`。该结果只证明门禁会拦截不充分候选，不替代正式 50 epoch 结构筛选。
- `runs/2026-07-12_chronaris-v2-direct-residual-repair-seed17-r1/`：**Chronaris v2 直接因果生理残差正式门禁**。在固定 16 维生理专属切片中直接保留归一化、因果前向填充的查询值；参考完整 v2 与修复候选均使用最终任务无关训练状态诊断。修复候选的航电/生理保真比分别为 1.1396/2.3466，有效秩 25.70，时间机制误差比 0.8804，未来因果不变性、无效位置池化和时延掩码均通过，且任务标签、外层测试和封存确认访问均为 false。该候选成为唯一获准进入 24 项超参数筛选的结构，不构成下游任务结果。
- `runs/2026-07-12_chronaris-v2-hyperparameter-screen-seed17-r3/`：**Chronaris v2 24 项任务无关超参数训练**。直接因果生理残差架构按 3 个内部宽度、2 种时延形式、2 种固定求解器和 2 个学习率组成完整网格；24/24 候选均完成 30 epoch，训练层 3/3 验收通过。checkpoint 使用最终三阶段任务无关状态，任务标签、仿真任务真值和锁定测试访问均为 false；该 run 不生成下游任务结果。
- `runs/2026-07-12_chronaris-v2-hyperparameter-diagnostics-seed17-r3/`：**Chronaris v2 超参数任务无关门禁与前三选择**。24/24 候选均通过双模态保真、表示健康、时间机制、未来因果不变性、无效位置池化和时延掩码门禁。词典序前三均为 96 维、学习率 `1e-3`：连续时延 basis + RK4、固定五档 + RK4、连续时延 basis + Euler；公共自监督验证损失为 0.3219、0.3240、0.3247。三类禁止访问标志均为 false；前三只获准进入鼎新 inner-validation 任务无关复核，尚未锁定配置。
- `runs/2026-07-12_chronaris-v2-dingxin-inner-confirmation-r3/`：**Chronaris v2 鼎新三折任务无关配置锁定**。三个 96 维候选各在三个留一视图 inner-validation 折完成 50 epoch，共 9/9 checkpoint；专属切片保真、有效秩、因果不变性、无效位置池化和时延掩码均通过。航电最差折保真比为 1.2661–1.8648，生理最差折保真比均超过 2.05，provider outer-test 请求为 0。按三折平均公共损失、最差保真和参数量锁定连续时延 basis + Euler + `1e-3`，下游标签与 outer-test 均未用于选择。
- `runs/2026-07-12_chronaris-v2-dingxin-inner-confirmation-r3-full-representation-probe/`：**三折完整表示保真探针诊断记录**。该早期探针把 64 维完整表示用于航电专属保真，在 31 个训练样本下受到生理/共享维度的小样本 ridge 过拟合干扰，产生错误的 `gates_failed`。反事实确认同一 checkpoint 的 24 维航电专属切片三折保真比均超过 1.8；因此该根只记录诊断修正过程，不承担配置锁定结论。
- `runs/2026-07-13_chronaris-v2-confirmation-access/`：**独立仿真确认族锁定后访问授权**。单独 unlock record 绑定锁定配置和预封存 payload 的 SHA-256，方法范围固定为生理单流、航电单流、朴素时间同步、MulT、ContiFormer 和 Chronaris；原 seal manifest 保持 `unlocked=false`，开发与选模阶段仍不可读取确认结果。
- `runs/2026-07-13_chronaris-v2-dingxin-locked-pretraining/`：**Chronaris v2 鼎新三随机种子五折锁定重训**。锁定 96 维连续时延 basis + Euler + `lr=1e-3` 配置完成 15/15 checkpoint；任务标签、仿真真值与 outer-test 均未用于训练，provider outer-test 请求为 0，既有确认指标未改动。重型 checkpoint 位于忽略目录，紧凑 inventory、协议、访问审计与 evidence manifest 进入本索引。
- `runs/2026-07-13_chronaris-v2-dingxin-locked-representations/`：**锁定 v2 与既有五种方法的鼎新统一表示**。完成 3 个随机种子 × 5 个外层折 × 6 方法 × 3 角色共 270/270 份 `[N,96,64]` 表示，6/6 门禁通过；共享因果查询有效域、样本顺序、时间轴与来源哈希全部对齐，表示阶段不计算外层测试指标。
- `runs/2026-07-13_chronaris-v2-dingxin-locked-consumers/`：**Chronaris v2 鼎新固定数据同协议复验**。冻结 consumer 完成 90/90 方法—折—随机种子单元、5,040 条指标与 8/8 门禁。三个留一视图主折汇总中，v2 Chronaris 的机动分类 Macro-F1、高响应 AUPRC 和连续响应 RMSE 为 0.7254、0.8410、0.3346，均未越过预声明硬门槛；该结果只进入差距审计，不回流当前配置。
- `runs/2026-07-13_chronaris-v2-sealed-confirmation-consumers/`：**Chronaris v2 独立仿真六方法锁定确认**。三随机种子、35 个封存场景和六方法完成 630/630 个冻结 consumer 评估、20,160 条指标及 4,032 条压力斜率，6/6 门禁通过。无扰动场景下 v2 的负荷分类 Macro-F1、负荷回归 RMSE、机动分段 Macro-F1 为 0.3044、0.2539、0.4123；仅分段均值居首但仍未越过硬门槛。随机缺失斜率排名第 2，连续缺失排名第 4。
- `runs/2026-07-13_chronaris-v2-simulation-ablation-consumers/`：**Chronaris v2 修正物理与缺失课程正式消融**。两个变体 × 三随机种子完成 6/6 冻结 consumer、384 条指标和 192 条完整模型方向归一差值，6/6 门禁通过。修正物理对三项主指标均值均为正；缺失课程对分类/分段为正、对回归为负，且两项均存在随机种子方向不一致。
- `runs/2026-07-13_chronaris-v1-sealed-reference-mechanism/` 与 `runs/2026-07-13_chronaris-v2-mechanism-consumers/`：**v1/v2 同封存族时间机制比较**。v2 生理响应时延 MAE 为 7.1546 秒，优于 v1 的 7.5199 秒；时钟偏移 MAE 为 0.8944 秒，较 v1 的 0.8864 秒轻微上升。两项均满足相对 v1 恶化不超过 10% 的门禁。
- `runs/2026-07-13_chronaris-v2-promotion-audit/`：**Chronaris v2 一次性锁定晋级审计**。六项主指标全部未过预声明硬门，附加门禁中的无损融合、类别召回差距和缺失鲁棒性前三未通过；因果未来不变性、无效查询池化和时间机制通过。结论固定为 `promoted=false`、论文主模型保持 `chronaris_v1`，确认结果不得回流同一开发轮次。
- `runs/2026-07-13_chronaris-v2-public-adapter-confirmation/`：**Chronaris v2 锁定后 NASA/UAB 公开数据适配确认**。只运行一项由锁定配置映射的适配配置，未使用公开标签选择模型；seeds 17/29/43 完成 365 个留一受试者折。NASA 综合分类 Macro-F1 为 0.5664 ± 0.0085，高于公开参考 0.4550；UAB N-back 与椅背加热任务 RMSE 为 5.0962 ± 0.0578、1.4556 ± 0.0202，前者仍落后公开参考与 ContiFormer，后者与两者接近。第二输入流仅作上下文构造，不等同于鼎新航电流；紧凑根保存三随机种子表、配置、报告、7 幅已抽查中文图及恢复命令。
- `runs/2026-07-13_chronaris-v2-final-evidence-pack/`：**Chronaris v2 锁定复验与差距证据包**。汇总六项主指标、七类压力退化、v1/v2 同封存族时间机制、修正物理/缺失课程正式消融和三随机种子公开数据适配，生成 4 幅中文结论图和可追溯表格。结论为 v2 不晋级、论文主模型保持 v1、既有确认指标不变；确认结果不得回流同一开发轮次。
- `runs/2026-07-12_chronaris-v2-confirmation-family-seal/`：**Chronaris v2 独立仿真确认族预生成与封存**。使用与既有开发/锁定仿真分离的 profile、latent 和 observation seed 区间，生成 48 条独立潜在轨迹、35 个统一压力场景、1,680 个观测场景及 3,360 个原始观测/真值文件，约 1.7 GB，6/6 封存门禁通过。六方法共享同一整体 SHA-256；当前配置尚未锁定，manifest 保持 `unlocked=false`，候选训练、表示导出和任务无关排序均不得读取该确认族。
