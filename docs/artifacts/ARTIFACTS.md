...
- `runs/2026-07-13_chronaris-core-task-recovery-confirmation/`：**核心任务恢复一次性确认与支持区间审计**。锁定配置完成 54 个方法—随机种子—折单元和 162 条外层留出指标，唯一授权记录为 `actual_open_count=1`。后置审计发现前两个留一视图折各有 361 个训练—外层留出支持区间重叠配对，训练上下文 31/31 全部受影响，并各有 31 个同架次同锚点配对；产物已标记 `invalid_protocol_overlap`、`claim_eligible=false`。第三折无重叠，Chronaris 三随机种子机动分类宏平均 F1（Macro-F1）0.7778、生理响应均方根误差（RMSE）0.8222、高响应精确率—召回率曲线下面积（AUPRC）0.8127，只作失败诊断。
- `runs/2026-07-13_chronaris-core-task-recovery-development/`：**核心任务恢复六方法等预算开发漏斗**。只读取内层训练与验证数据，比较固定 MiniRocket 时序卷积特征变换、分类与回归消费者；开发锁定 Chronaris 的航电观测保留机动路径、观测—连续拼接回归路径和连续高响应路径。该 run 不含外层留出指标；后续支持审计表明原外层协议不可用于确认，开发结果不升级为论文主张。
- `runs/2026-07-13_chronaris-core-task-recovery/`：**核心任务恢复协议、候选注册、唯一配置和外层授权**。记录 6 方法、开发候选预算、锁定消费者、数据/目标哈希和唯一授权；外层授权已消费，不能复用。
- `runs/2026-07-13_dingxin-core-task-audit/`：**鼎新当前/未来机动与生理响应任务审计**。保留当前 5 秒机动分类；未来 5 秒机动任务因历史增益和双流增益未过门槛不晋升。审计只使用训练内数据，不形成模型排名。
...
- `runs/2026-07-12_downstream-evidence-pack/`：**固定数据下游评估历史论文证据包**。原始 8 层证据、7 幅图、54 项迁移配对和 9/9 工程门禁保持不变。2026-07-13 支持区间审计确认其中鼎新留一视图主表的前两个外层折存在跨视图时间重叠，因此鼎新数值不再具备论文主张资格；仿真 clean、压力、时间机制、消融和公开数据适配部分不受该鼎新划分问题影响。
...
- `runs/2026-07-12_dingxin-locked-consumers-coalesced/`：**鼎新三随机种子五折冻结表示历史下游评估**。90/90 个方法—折—种子 consumer 和原始指标均保持不变。2026-07-13 审计确认三个留一视图主折中前两折的训练与 held-out 支持区间重叠，故 AUPRC 0.8839、机动分类 0.7394、生理响应 RMSE 0.3365 及相应排名不再用于论文主张；两个留一架次辅助折没有在本轮自动提升为主结果。
...
- `runs/2026-07-11_dingxin-inner-splits/`：**鼎新外层折训练内验证划分**。五个外层折形成 inner-train、validation 和 outer-test 角色；当时的共享航电时间区间零重叠只审计 inner-train 与 validation。2026-07-13 新审计补查 inner-train 与 outer held-out 后，确认前两个留一视图折各有 31/31 个训练上下文发生完整支持区间重叠；因此原 11/11 只保留为训练内划分验收，不能推导外层隔离成立。
