...
[阶段 4.5-C 单因素尝试](runs/2026-09-22_v4-stage45c/report.md)：公共自监督项消融六次微调全部完成、未通过采用门，含完整配对结果与来源核验。

[阶段 4.5-B 实现与交接](runs/2026-09-21_v4-stage45b/report.md)：十二次首轮训练已完成、零入围；九个复核槽位跳过，历史主表保留。
...
- [阶段 5 停机与全分支复核包](runs/2026-09-25_v4-stage5-review/report.md)：12/36 完成、24 份收据核验、首折三种子结果、零通道诊断及标准库复算；[远端入口](runs/2026-09-25_v4-stage5-review/pro-review.md)。

- [阶段 5 固定配置复核](runs/2026-09-23_v4-stage5/report.md)：36 单元固定范围，四个历史结果继承与 32 个新增单元；705 项回归通过、22 项跳过，两域执行资格通过，9 月 23 日启动记录；当前已失败停止，见上方 9 月 25 日复核。

- [阶段 4.5 完整结果与远端复核](runs/2026-09-21_v4-stage45-closeout/report.md)：18 个首轮完成、五种候选零入围；含全部主成绩、补充下游、快照、分支诊断及独立复算材料。

- [阶段 4.5 全流程加速与恢复](runs/2026-09-19_v4-stage45-accelerated/report.md)：697 项显卡回归、六组全流程资格、生产批量微调及独立恢复通过，保留九个完整单元和第 75 次状态。

- [阶段 4.5 暂停复核与恢复](runs/2026-09-14_v4-stage45-pause-review/report.md)：第 75 次状态的计算提速、真实入口恢复与早期分支诊断；取消原 72 小时硬上限。

- [阶段 4.5 编码、验证与运行](runs/2026-09-14_v4-stage45/report.md)：参考配置、定向诊断及有界开发队列；工程凭证与研究结果分开记录。

- [完整开发比较收口验收](runs/2026-09-14_v4-stage4-closeout/report.md)：25 单元、43 路线、65 组下游完成；完整成绩、学习诊断、历史成本修正和后续预算已归档。

- [完整开发比较执行迁移与恢复](runs/2026-09-13_v4-stage4-execution-migration/report.md)：第 200 次状态成功迁移，第 225 次新检查点与九个继承单元核验；后续比较现已完成，见上方收口报告。

- [完整开发比较性能试验](runs/2026-09-13_v4-stage4-performance/report.md)：已保留第 200 次检查点和九个完成单元，固定两次更新核心计算加速 11.46 倍，数值容差与独立恢复通过；后续执行迁移与恢复见上方记录。
...
- [阶段 4 完整开发比较与成本实测](runs/2026-09-11_v4-stage4-comparison/report.md)：首折、种子 17，25 个模型单元与 43 条表征路线；后台启动与后续完成证据分开记录。

- [阶段 3 近期模型真实开发接入](runs/2026-09-11_v4-stage3-models/report.md)：两个必做模型与 SensorLLM 开放骨干变体形成 10 组独立下游、554 条验证预测；保留资源修订、梯度、缓存及恢复证据。

- [阶段 2 共同下游合同与真实开发验收](runs/2026-09-10_v4-stage2-contract/report.md)：三域 12 个评价单元、18 组下游模型，984 条预测及恢复一致；完整 CUDA 测试 669 通过、8 跳过。

- [阶段 1 修复与计算保留](runs/2026-09-10_v4-stage1-repair/report.md)：真实失败 256 窗口逐值重放通过；52 份开发检查点、104 份序列与 14,300 次训练更新可保留，自监督汇总及 26 组线性模型需重建。
...
- [v4 核心消融合同](runs/2026-09-08_v4-core-ablation-contract/report.md)：复用已有变体和训练器，保留观测锚定，确认生成前冻结全部主模型和消融模型；615 项完整 CPU 测试通过。

- [v4 正式主表入口](runs/2026-09-08_v4-formal-mainline/report.md)：共享训练和队列、模型冻结后生成确认、35 场景与两种整模态缺失；611 项完整 CPU 测试通过，正式模型尚待冻结选型。
