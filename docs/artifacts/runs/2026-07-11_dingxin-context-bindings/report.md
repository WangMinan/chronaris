...
- 状态：完成；验收 12/12 通过。
- 96 个标签上下文中，93 个拥有完整 30 秒原始输入；3 个末端上下文实际只覆盖 155–180.991 秒，未达到名义 185 秒终点，因此不可用于模型输入。
- 机动分类在五折中绑定 93 个唯一可用上下文；生理响应同时要求完整未来 5 秒，绑定 90 个唯一可用上下文。
- 公共输入 schema 为 12 个生理字段和 955 个航电字段；20 个机动标签源字段在原始映射层即被删除。
- 允许字段稀疏缓存包含 10,255,756 个 float32 值、数组净大小 78.6 MB；完整审计耗时 12.05 秒，峰值内存 760.8 MB。
