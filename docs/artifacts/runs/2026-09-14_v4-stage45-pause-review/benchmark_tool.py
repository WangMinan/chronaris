...
def compare_trials(expected_root, actual_root, *, exact=False):
    ...
    for path in paths:
        ...
        checks = {}
        keys = ['encoder_state_dict', 'head_state_dict', 'optimizer_state_dict', 'rng_state', 'data_cursor']
        if 'explicit_time_shift_head_state_dict' in left or 'explicit_time_shift_head_state_dict' in right:
            keys.append('explicit_time_shift_head_state_dict')
        for key in keys:
...
def _restore(payload, graph):
    ...
    shift = ExplicitTimeShiftHead(64).cuda().train() if payload.get('explicit_time_shift_head_state_dict') is not None else None
    if shift is not None:
        shift.load_state_dict(payload['explicit_time_shift_head_state_dict'], strict=True)
    parameters = tuple(encoder.parameters())+tuple(heads.parameters())+(tuple(shift.parameters()) if shift is not None else ())
...
def trial(*, checkpoint, pipeline_config, output_root, graph, threads=1, updates=2, resume_state=None, domain=None, profile=False):
    ...
    if (payload['method_name'] != 'chronaris'
        ...
        or not 0 < payload['optimizer_updates'] < payload['config']['max_updates']):
        raise ValueError('trial requires an unchanged-source intermediate Chronaris checkpoint')
    if domain is None:
        if payload['optimizer_updates'] != 200 or payload['chronaris_mechanism_enabled'] or payload['explicit_time_shift_head_state_dict'] is not None:
            raise ValueError('legacy trial requires the stage4 update-200 checkpoint')
        domain = 'cogpilot'
    if domain not in ('clare', 'cogpilot', 'dingxin'):
        raise ValueError('trial requires an approved native development domain')
    ...
    with development_gpu_lock() as acquired, _periodic_training_heartbeat('checkpoint_performance', 30., root=root) as progress:
        ...
        with isolated_training_rng(config.seed):
            ...
            provider, _, fold, digest, _, _, _, raw = contract_development_inputs(domain,
            ...
            if resume_state and 'explicit_time_shift_head_state_dict' in state:
                restored['explicit_time_shift_head_state_dict'] = state['explicit_time_shift_head_state_dict']
            encoder, heads, shift, parameters, optimizer = _restore(restored, graph)
            ...
            metadata = dict(parent_sha256=original_hash, source_code_sha256=candidate_source_code_sha256(),
                domain=domain, graph=graph, threads=threads, actual_batch=4, effective_batch=16,
            ...
            for update in range(start_update+1, start_update+updates+1):
                ...
                diagnostic = dict(format='chronaris.performance_diagnostic.v1', parent_sha256=original_hash,
                    ...
                    explicit_time_shift_head_state_dict={k:v.detach().cpu().clone() for k,v in shift.state_dict().items()} if shift is not None else None,
            ...
            del encoder, heads, shift, optimizer, parameters
            gc.collect()
