...
## ECG-HR 富生理探针（20subj seed17，诚实负向）

为测试“更丰富生理信号是否让融合超 vehicle”，将 ECG 导出的瞬时心率（R-R 间期，强唤醒信号）加入生理流（PPG/EDA/resp/HR 4 通道）。结果：safe_lag macro-F1 `0.3829`，反而**低于**无 HR 的 `0.4654`，更低于 vehicle_only `0.5045`。

**判断**：富生理（含 HR）未能让融合在难度任务上超 vehicle——难度由飞机状态（ILS 偏差/空速等）主导，生理信号（即使强 HR）不增预测力，反增噪声。进一步确认难度任务的车辆主导性。
