...
- leakage-safe Dingxin weak-label ablation r2：`stage-i-private-leakage-safe-ablation-20260619T-stage-i-leakage-safe-ablation-r2.md`
...
- Dingxin third-party comparison GPUOPT r1：`stage-i-private-thirdparty-comparison-20260702T-stage-i-private-thirdparty-comparison-gpuopt-r1.md`
...
- Optimized final polish 最终指标打磨 r1：`stage-i-optimized-final-polish-20260702T-stage-i-optimized-final-polish-r1.md`
- Thesis protocol freeze 论文协议冻结 r1：`stage-i-thesis-protocol-20260703T-stage-i-thesis-protocol-r1.md`
- 中期事实清单：`../../midterm/midterm-fact-sheet-2026-06-13.md`
- 中期边界说明：`../../midterm/boundaries-and-risks-2026-06-13.md`
- LLM 预处理对比 中期结果摘要：`../../midterm/llm-preprocessing-comparison-summary-2026-06-14.md`
...
- 仓库收敛清理 thesis prep cleanup 记录：`../cleanup/20260703-thesis-prep-cleanup.md`
...
- Dingxin-鼎新真实数据第三方模型对比 third-party comparison 资产：
...
- 公开融合消融 fusion ablation 资产：
  ...
  - manifest/log/progress：`evidence_manifest.json`、`run.log`、`progress.json`
- 跨证据矩阵 cross-evidence matrix 资产：
  ...
  - manifest：`evidence_manifest.json`
- 任务感知头优化 task-aware heads confirm 资产：
  ...
  - manifest/log/progress：`evidence_manifest.json`、`run.log`、`progress.json`
- 流角色融合 stream-role-aware fusion completed 资产：
  ...
  - manifest/log/progress：`evidence_manifest.json`、`run.log`、`progress.json`
  - 2026-07-03 仓库收敛清理 cleanup 后，nested public/Dingxin confirm 的 per-candidate byproduct、重复 task manifest、GPU batch 明细和 child run 日志不再作为 git 资产；aggregate metrics、figures、summary 和顶层 logs 保留，外置备份见 `../cleanup/20260703-thesis-prep-cleanup.md`。
- 优化模型再评估 optimized Chronaris re-evaluation completed 资产：
...
- 最终指标打磨 optimized final polish completed 资产：
  ...
  - boundary：鼎新真实数据第三方模型对比、公开融合消融、任务感知头优化、流角色融合与优化模型再评估 是固定 reference；分类任务 与 public route accepted，检索任务 rejected 并沿用 任务感知头优化 retrieval；public 仍是 context-derived second stream。
  - 2026-07-03 仓库收敛清理 cleanup 后，nested screen/confirm 的可再生成训练曲线、child run 日志、GPU batch、逐候选 deep summary 和顶层稠密预测副本已外置备份并从 git 当前树删除；最终指标打磨 顶层 summary/metrics/figures/manifest 保留。
- 论文协议冻结 thesis protocol freeze completed 资产：
  ...
  - manifest/log/progress/resume：`evidence_manifest.json`、`run.log`、`progress.json`、`resume_command.txt`
  - boundary：论文协议冻结 只读聚合 鼎新真实数据第三方模型对比、公开融合消融、跨证据矩阵、任务感知头优化、流角色融合、优化模型再评估与最终指标打磨，不重跑训练、不删除 artifact、不改写历史；public 仍是 context-derived second stream，Dingxin 仍是 weak-label evidence。
- Dingxin component ablation（历史协议）：`stage-i-private-component-ablation-20260607T-stage-i-evidence-closure-r2-private-proxy.md`
...
历史 论文图表材料/运行时服务冒烟验证 图表与 runtime：

- `stage-i-rigid-body-rotation-audit-20260607T-stage-i-rotation-audit-r2.md`
- 旧 论文图表材料/稳定扫描与字段契约收口 thesis materials r2 历史图包、r3/r4 图包已被后续图包接管；r4 runtime case refresh 已从 docs 产物目录清理，`../assets/stage_i_thesis_figures/20260613T-stage-i-thesis-materials-r2-p18/runtime_semantic_case.csv` 仅作为 DeepSeek/LLM preprocessing 历史输入表。
- 旧论文图表刷新 thesis materials r5 图包与报告已由 论文图表报告级重绘 r6 接管，并已按 `../cleanup/20260621-deep-cleanup.md` 从 docs 产物目录清理；当前图表入口使用 r6。
- 早期 public torch/mainline/fusion screen archive-only 迭代已按 `../cleanup/20260701-src-docs-artifact-prune.md` 从 docs 当前产物目录清理；当前 public adapter 与模型对比入口使用 公开模型对比与公开融合刷新、UAB robust-prior、NASA round1、UAB torch auto-cuda confirm 和 fusion screen round2。
- runtime service smoke r1、runtime replay r1、semantic support r1、rigid-body r1、鼎新弱监督任务扫描 live r2 和 DeepSeek 时序预处理 preprocessing r1 仅保留在 git 历史；当前入口使用 r2/r3/r6/r2-contract 产物。

历史 鼎新弱监督任务扫描 live 首轮：
...
- UAB/NASA 第二模态统一写成“上下文构造第二输入流”与公开数据适配/校准证据。
- 鼎新真实数据分支需要区分“分类任务、回归任务和检索任务 = 弱监督构造的组件诊断任务”与 `risk_proxy / workload_proxy / event_replay_tag` thesis weak-label evidence。
- `stage_i_private_leakage_safe_ablation` r2 是 `protocol=leakage_safe_v1` 的新增协议；历史 Dingxin component ablation 只能作为历史弱监督诊断，不与 r2 防泄漏结果混成同一实验结论。
- DeepSeek 时序预处理/LLM 预处理对比 DeepSeek LLM preprocessing 只能写成 preprocessing context、rule review、whitelisted semantic hints、runtime explanation、bounded comparison 和 pending human review packet；不能写成人工真值、OpenAI 默认接入、核心因果证据或人工验证完成。
- 鼎新真实数据第三方模型对比 Dingxin third-party comparison 只能写成 分类任务、回归任务和检索任务 Dingxin weak-label task 的第三方对比；当前结果是混合结果，不能写成 Chronaris 全面胜出。
- 公开融合消融 public fusion ablation 只能写成 public adapter / context-derived second stream 上的组件敏感性诊断；不替代 公开融合刷新 confirmed metrics，也不证明 public 第二模态等价于鼎新真实航电流。
- 跨证据矩阵 cross-evidence matrix 用于 Dingxin/public/weak-label/component 四层证据分工；不要把四层指标混成单一排行榜。
- 任务感知头优化、流角色融合与优化模型再评估 与 optimized model summary 当前为 optimized v2/v3 CUDA confirm / completed aggregation / summary，不替代 鼎新真实数据第三方模型对比、公开融合消融与跨证据矩阵 completed 结果；引用时必须写明 任务感知头优化 是 20-epoch confirm、流角色融合 是 requested Dingxin/public v3 confirm、public 第二流仍是 context-derived second stream，也不得写成 optimized Chronaris 全面超过全部 baseline。
- 最终指标打磨 final polish 当前为固定 鼎新真实数据第三方模型对比、公开融合消融、任务感知头优化、流角色融合与优化模型再评估 reference 上的局部收束：分类任务校准 与 public route calibration accepted，检索任务 rejected 并沿用 任务感知头优化 confirmed retrieval。引用时不要把 public context-derived second stream 写成鼎新真实航电流，也不要把 最终指标打磨 写成全面胜出。
- 论文协议冻结 thesis protocol freeze 是论文协议矩阵入口；论文图表若引用 鼎新真实数据第三方模型对比、公开融合消融、跨证据矩阵、任务感知头优化、流角色融合、优化模型再评估与最终指标打磨，应优先从 论文协议冻结 的 `experiment_registry.csv` 或 `result_matrix_long.csv` 反查原始 artifact path 和 claim boundary。
- 2026-07-02 后，本轮 任务感知头优化、流角色融合与优化模型再评估/summary 柱状图已基于现有 CSV/JSON 重绘并增加短数值标签；future run 默认使用 `checkpoint_policy=last`，checkpoint binary 和 dense prediction CSV 不进入 git。清理/profiling 入口：`../cleanup/20260702-p34-p36-gpu-and-docs-cleanup.md`。
