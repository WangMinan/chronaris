...
4. `docs/planning/implementation-summary-2026-04-15.md`
...
| --- | --- | --- |
| 双流连续潜态建模 | `阶段 E` | 现在直接实现 |
| 物理一致性约束时间对齐 | `阶段 F` | `阶段 E` 先预留接口，不在首轮做满 |
| 因果掩码跨模态融合 | `阶段 G` | 当前不提前实现 |
| 标准化融合特征输出 | `阶段 H` | 当前只保留中间态接口 |
| 下游任务验证 | `阶段 I` | 当前只保留最小评测入口 |

这意味着：

1. 选题和基金中的理论主线仍然成立
2. 但代码实现要按 `E -> F -> G` 的工程顺序分步落地
3. `阶段 E` 的目标是把“连续对齐模型原型”先跑起来，而不是一次性完成整套论文方法
