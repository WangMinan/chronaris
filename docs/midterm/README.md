...
6. 追溯 LLM 预处理对比 设计时读 [llm-preprocessing-comparison-plan-2026-06-14.md](llm-preprocessing-comparison-plan-2026-06-14.md)；引用结论时以 [llm-preprocessing-comparison-summary-2026-06-14.md](llm-preprocessing-comparison-summary-2026-06-14.md) 为准。

## 当前中期核心事实

- 论文题目方向：航空人机异构时序数据连续对齐与语义融合。
- 当前鼎新真实数据主线：`chronaris_opt`，只能写成鼎新真实数据弱监督组件诊断证据。
...
- 当前 LLM 实现：DeepSeek 时序预处理 已完成 DeepSeek 在线时序数据预处理小样本真实切片 run，用于字段语义归一、weak-label 复核、schema gap policy 和 runtime 解释；输出是 preprocessing context，不是人工真值。
- 当前 LLM 对比：LLM 预处理对比 已完成 A0-A4 对比实验，结果为 `333/333` task entries attach 后 `label_unchanged=true`、semantic query coverage `3 -> 7`、runtime explanation 子集 `4/12`、human review packet `15` 条且 `human_review_completed=false`。
- 当前中期图表：r6 report figure polish 已输出 `12` 张 PNG 与 `12` 张 CSV，替换 evidence layer all-one bar、runtime schema、runtime semantic case、rotation audit、weak-label sweep、Dingxin ablation、public transfer、semantic fusion 和 LLM review flow 的旧表达，新增 `runtime_service_flow`，并保留 model backbone / task adapter 严格评价消融图。
...
- LLM 预处理对比 LLM 预处理对比实验计划：[llm-preprocessing-comparison-plan-2026-06-14.md](llm-preprocessing-comparison-plan-2026-06-14.md)
- LLM 预处理对比 LLM 预处理对比实验摘要：[llm-preprocessing-comparison-summary-2026-06-14.md](llm-preprocessing-comparison-summary-2026-06-14.md)
- LLM 预处理对比 LLM 预处理工程报告：[../artifacts/stage_i/stage-i-llm-comparison-20260614T-stage-i-p21-llm-comparison-r1.md](../artifacts/stage_i/stage-i-llm-comparison-20260614T-stage-i-p21-llm-comparison-r1.md)
- 当前中期图表 r6：[../artifacts/stage_i/stage-i-thesis-materials-20260621T-stage-i-thesis-materials-r6-report-figure-polish.md](../artifacts/stage_i/stage-i-thesis-materials-20260621T-stage-i-thesis-materials-r6-report-figure-polish.md)
