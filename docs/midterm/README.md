...
- 当前中期图表：r6 report figure polish 已输出 `12` 张 PNG 与 `12` 张 CSV，替换 evidence layer all-one bar、runtime schema、runtime semantic case、rotation audit、weak-label sweep、private ablation、public transfer、semantic fusion 和 LLM review flow 的旧表达，新增 `runtime_service_flow`，并保留 model backbone / task adapter 严格评价消融图。
