...
- `Stage I`：
  ...
  - `dingxin weak-label branch`：覆盖 `chronaris_opt`、分类任务、回归任务和检索任务、鼎新真实数据 benchmark 与组件诊断，当前代码入口归入 `src/chronaris/pipelines/stage_i/private/`，诊断报告编排归入 `evidence/`。
  ...
  - `evidence closure`：覆盖 主动证据汇总到运行时契约收口 的 evidence runner、weak-label sweep、support、rotation audit、thesis materials、midterm pack 和 runtime schema 边界说明，当前代码入口归入 `src/chronaris/pipelines/stage_i/evidence/`。
  - `runtime/service`：核心推理服务位于 `src/chronaris/serving/`，Stage I 入口脚本归入 `scripts/stage_i/runtime/`。
  - `LLM preprocessing`：DeepSeek 在线时序数据预处理只作为 preprocessing context / rule review / semantic hints / runtime explanation，当前代码入口归入 `src/chronaris/pipelines/stage_i/llm/` 与 `scripts/stage_i/llm/`。
...
- `src/chronaris/pipelines/stage_i/common/`：跨 public/dingxin/evidence 复用的 observer、baseline split、deep model helper。
...
- `src/chronaris/pipelines/stage_i/private/`：鼎新真实数据 benchmark、`chronaris_opt`、弱监督构造任务和优化包。
- `src/chronaris/pipelines/stage_i/evidence/`：主动证据汇总、weak-label sweep、support、rotation audit、thesis materials、midterm pack 和 runtime schema 边界说明。
- `src/chronaris/pipelines/stage_i/llm/`：DeepSeek/LLM preprocessing pipeline、harness、slicing、reporting。
- `src/chronaris/pipelines/stage_i/legacy/`：历史公开 Phase 0/1/2/3 与 baseline closure。

脚本入口统一放到 `scripts/stage_i/<category>/`。根目录不再保留 `run_stage_i_*.py` / `build_stage_i_*.py` 旧脚本文件；需要执行旧命令时，应改用 `scripts/README.md` 里列出的 canonical 路径。代码瘦身追加清理 后不再保留旧 `chronaris.pipelines.stage_i.stage_i_*` import hook 或大规模 package-level re-export 表；新代码应直接导入真实实现模块。

## 当前工作区与推送状态

当前事实：

- 当前分支为 `main`；仓库收敛清理起始 HEAD 与当时 `origin/main` 均为 `756555c79627237800458cd8420a064441ab0147`（`feat: add stage i thesis protocol freeze`）。公开融合刷新 GPU profiling、鼎新真实数据第三方模型对比、公开融合消融、跨证据矩阵、任务感知头优化、流角色融合、优化模型再评估、最终指标打磨、论文协议冻结与仓库收敛清理均已进入当前历史。仓库收敛清理已完成 inventory、代码入口整理、67 个可再生成 byproduct 外置备份删除、`.gitignore` 防回流、docs 索引回写和本地 `git lfs prune`；代码瘦身追加清理已完成追加 `src/` 激进瘦身，`src/chronaris` 当前为 `187` 个 Python 文件、`64,211` 行，`src/scripts/tests` 当前为 `276` 个 Python 文件、`85,162` 行；尚未启动仿真压力测试、检索任务与公开路线优化、论文级消融统一或论文材料化。本轮不改变既有 confirmed metrics。
...
- 当前时间充足，清理完成后优先继续提升 检索任务与 public route 指标。
- 用户接受先清理仓库再跑新实验；允许删除 tracked 历史 artifact、允许外置备份、允许必要时改写 git history。
- 当前论文协议冻结已按计划先执行；current-tree 仓库收敛清理已完成，删除项均先备份到 `/home/wangminan/projects/chronaris-local-artifacts/cleanup-20260703/backup_manifest.csv`。本轮不做 `filter-repo` 历史改写；下一步优先仿真压力测试或检索任务与公开路线定点优化。

当前已完成：

- `P38` 论文协议冻结：已新增 `scripts/stage_i/evidence/build_thesis_protocol.py` 与 `src/chronaris/pipelines/stage_i/evidence/thesis_protocol.py`，从既有 鼎新真实数据第三方模型对比、公开融合消融、跨证据矩阵、任务感知头优化、流角色融合、优化模型再评估与最终指标打磨 artifact 只读生成 `20260703T-stage-i-thesis-protocol-r1`，不重跑训练、不清理 artifact、不改写历史。
- `P42` 仓库收敛清理：已新增 `docs/artifacts/cleanup/20260703-thesis-prep-cleanup-inventory.md` 与 `docs/artifacts/cleanup/20260703-thesis-prep-cleanup.md`；删除最终指标打磨和流角色融合的 nested byproduct、任务感知头优化未索引 batch/manifest、最终指标打磨顶层稠密预测副本共 67 个文件，备份 manifest 在仓库外；代码入口删除纯 wrapper、统一 stream-role CLI、把 GPU helper 实现移入 common，并通过 compile/unit/CLI/论文协议冻结 path/LFS 初验。
...
2. `P40` 检索任务与公开路线 指标提升：清理后定点优化 检索任务与 public route。
...
1. LLM 预处理对比 已完成 LLM preprocessing 融入 Stage I 数据融合管线的对比实验，并把结果落到 `docs/midterm/llm-preprocessing-comparison-summary-2026-06-14.md`，可供中期报告直接引用。
2. DeepSeek 时序预处理 已完成 DeepSeek 在线时序数据预处理、agent-style prompt/harness v2、切片整合和小样本真实 r3-sliced run；LLM 预处理对比 已在此基础上比较 baseline vs LLM-context、semantic query bank vs LLM hints、runtime report with/without LLM explanation，以及小样本人工复核 packet。
3. 中期报告写作优先从 `docs/midterm/` 的事实清单、边界风险说明和 claims matrix 进入；DeepSeek 时序预处理/LLM 预处理对比 只能写成 LLM preprocessing context 与对比证据，不能写成真值标注、OpenAI 默认接入或核心因果证据。
...
9. 中期图表当前入口为 `docs/artifacts/assets/stage_i_thesis_figures/20260621T-stage-i-thesis-materials-r6-report-figure-polish/figure_manifest.json`；旧 `r2-p18` 图包已清理，仅保留 `runtime_semantic_case.csv` 作为 DeepSeek/LLM preprocessing 历史输入表，r4 runtime case refresh 已由 r5 接管并清理，r5 已由 r6 报告重绘图包接管并在 2026-06-21 深度清理中从 docs 产物目录删除。
10. 公开模型对比与公开融合刷新 已完成 public model comparison 与 chronaris_public_fusion refresh；中期公开模型对比优先引用 `docs/artifacts/stage_i/stage-i-public-model-comparison-20260701T-stage-i-public-model-comparison-r1.md`、`docs/artifacts/assets/stage_i_public_model_comparison/20260701T-stage-i-public-model-comparison-r1/improvement_summary.csv` 和 公开融合刷新 `fusion_refresh_summary.json`。
11. 公开融合刷新-GPUOPT 已完成代表性 fold GPU efficiency profiling；效率证据优先引用 `docs/artifacts/stage_i/stage-i-public-fusion-gpu-optimization-20260701T-stage-i-public-fusion-refresh-r1-gpuopt-r1.md` 与 `docs/artifacts/assets/stage_i_public_fusion_refresh/20260701T-stage-i-public-fusion-refresh-r1/gpu_optimization/optimization_summary.json`。该结果只说明训练吞吐、显存和 resume 状态，不替代 公开融合刷新 full LOSO confirmed metrics。
12. 鼎新真实数据第三方模型对比 已完成 Dingxin real dual-stream Stage H third-party comparison；入口为 `docs/artifacts/stage_i/stage-i-private-thirdparty-comparison-20260702T-stage-i-private-thirdparty-comparison-gpuopt-r1.md` 与 `docs/artifacts/assets/stage_i_private_thirdparty_comparison/20260702T-stage-i-private-thirdparty-comparison-gpuopt-r1/evidence_manifest.json`。该结果是 Dingxin 分类任务、回归任务和检索任务 weak-label constructed task 的第三方对比，不能写成论文最终人工真值任务。
13. 公开融合消融 已完成 public fusion ablation；入口为 `docs/artifacts/stage_i/stage-i-public-fusion-ablation-20260702T-stage-i-public-fusion-ablation-gpuopt-r1.md` 与 `docs/artifacts/assets/stage_i_public_fusion_ablation/20260702T-stage-i-public-fusion-ablation-gpuopt-r1/public_fusion_ablation_summary.json`。该结果属于 public adapter / context-derived second-stream component ablation，不替代 公开融合刷新 confirmed refresh，也不证明 public 第二模态等价于鼎新真实航电流。
14. 跨证据矩阵已完成；入口为 `docs/artifacts/stage_i/stage-i-cross-evidence-matrix-20260702T-stage-i-cross-evidence-matrix-gpuopt-r1.md` 与 `docs/artifacts/assets/stage_i_cross_evidence_matrix/20260702T-stage-i-cross-evidence-matrix-gpuopt-r1/evidence_manifest.json`。中期报告可用它说明鼎新真实数据、公开数据、弱监督任务和组件消融四层证据分工。
15. 任务感知头优化、流角色融合与优化模型再评估 与 optimized model summary 已新增 optimized Chronaris CUDA confirm / aggregation，当前入口为 任务感知头优化 r3 + 流角色融合与优化模型再评估 r4：
   - 任务感知头优化：`docs/artifacts/stage_i/stage-i-task-aware-heads-20260702T-stage-i-task-heads-optimization-r3-confirm20.md`，资产根 `docs/artifacts/assets/stage_i_task_heads_optimization/20260702T-stage-i-task-heads-optimization-r3-confirm20/`。
   - 流角色融合：`docs/artifacts/stage_i/stage-i-stream-role-aware-fusion-20260702T-stage-i-stream-role-fusion-r4-v3-confirm20.md`，资产根 `docs/artifacts/assets/stage_i_stream_role_fusion/20260702T-stage-i-stream-role-fusion-r4-v3-confirm20/`。
   - 优化模型再评估：`docs/artifacts/stage_i/stage-i-optimized-chronaris-reevaluation-20260702T-stage-i-optimized-reevaluation-r4-v3-confirm20.md`，资产根 `docs/artifacts/assets/stage_i_optimized_reevaluation/20260702T-stage-i-optimized-reevaluation-r4-v3-confirm20/`。
   ...
   - 当前状态：任务感知头优化 `status=completed`，为 CUDA 20-epoch confirm（3 seeds / `leave_one_view_out` + `leave_one_sortie_out` / 20 epochs），分类任务两个 split 小幅改善，回归任务明显改善，检索任务混合；流角色融合 `status=completed`，完成 requested dingxin/public v3 confirm，dingxin 分支比较 `chronaris_v3_stream_role_fusion` / `v3_no_role_gate` / `v3_fixed_causal_lag`，public 分支比较 `v3_stream_role` / `v3_no_role_gate` / `v3_force_private_causal` / `v3_context_adapter_only`，public 24 个 dataset/variant/seed 组合无缺失；优化模型再评估与 optimized model summary 为 `completed` aggregation，读取固定鼎新真实数据第三方模型对比、公开融合消融与跨证据矩阵 reference 并汇总任务感知头优化、流角色融合与优化模型再评估的关键指标、GPU runtime、gate 和 claim boundary。后续论文表述仍需保留公开数据上下文构造第二输入流、dingxin 弱监督任务 benchmark、检索任务 mixed 和“不是全面胜出”边界。
16. 2026-07-02 任务感知头优化、流角色融合与优化模型再评估 后处理已完成：基于既有 CSV/JSON 重绘本轮柱状图并加入短数值标签；新增 `checkpoint_policy` future-run 输出策略；独立 CUDA profiling 记录表明当前 public context-derived second-stream 小折 tensor cache 约 `0.115 GB`、auto batch 选 `2048/1024`、memory pressure 低于 `0.21`；清理过渡 run、dense predictions、partial CSV、checkpoint 和大日志，dense prediction CSV / checkpoint 已外置备份，重复 weak-label manifest 副本已回指 canonical。清理与 profiling 边界见 `docs/artifacts/cleanup/20260702-p34-p36-gpu-and-docs-cleanup.md`。本项不改变 任务感知头优化、流角色融合与优化模型再评估 或 鼎新真实数据第三方模型对比、公开融合消融与跨证据矩阵 confirmed metrics。
17. 最终指标打磨 optimized final polish 已完成，入口为 `docs/artifacts/stage_i/stage-i-optimized-final-polish-20260702T-stage-i-optimized-final-polish-r1.md`，资产根 `docs/artifacts/assets/stage_i_optimized_final_polish/20260702T-stage-i-optimized-final-polish-r1/`。
   - 当前状态：`status=completed`，鼎新真实数据第三方模型对比、公开融合消融、任务感知头优化、流角色融合与优化模型再评估 固定为 reference，未重跑或改写；CUDA required，tensor cache `auto`，AMP `bf16`，auto batch，torch compile `default`，heartbeat/progress/resume/skip-completed 已落盘。
   - 检索任务：`p37_t3_info_nce_temp0p05_hardw2` 与 任务感知头优化 在 `top1=0.0315315`、`top3=0.0855856`、`top5=0.139640`、`mrr=0.117939` 上持平，最终指标打磨 检索任务 rejected，后续仍引用 任务感知头优化 confirmed retrieval。
   - 分类任务：accepted；`leave_one_view_out` macro-F1 `0.204614` vs 任务感知头优化 `0.187489`（+`0.017125`），`leave_one_sortie_out` macro-F1 `0.220099` vs 任务感知头优化 `0.216065`（+`0.004034`）。
   - public route：accepted；`p37_public_force_adaptive_context_gate` NASA combined macro-F1 `0.443919` vs 流角色融合 `0.432193`（+`0.011726`），UAB mean RMSE `3.191811` vs 流角色融合 `3.374779`（改善 `0.182968`）。该分支仍是 public adapter / context-derived second-stream evidence。
18. 论文协议冻结 thesis protocol freeze 已完成，入口为 `docs/artifacts/stage_i/stage-i-thesis-protocol-20260703T-stage-i-thesis-protocol-r1.md`，资产根 `docs/artifacts/assets/stage_i_thesis_protocol/20260703T-stage-i-thesis-protocol-r1/`。
   - 当前状态：`status=completed`，只读聚合 鼎新真实数据第三方模型对比、公开融合消融、跨证据矩阵、任务感知头优化、流角色融合、优化模型再评估与最终指标打磨 artifact，未重跑训练、未删除 artifact、未改写 git history。
   ...
   - 引用边界：论文协议冻结 是论文协议冻结层，不替代原始 source artifact；public 仍是 context-derived second stream，Dingxin 分类任务、回归任务和检索任务仍是 weak-label evidence，仿真压力测试 synthetic 只能作为未来附录型 stress-test。
...
引用边界：论文协议冻结 是 鼎新真实数据第三方模型对比、公开融合消融、跨证据矩阵、任务感知头优化、流角色融合、优化模型再评估与最终指标打磨 的论文协议冻结层；最终指标打磨 final polish 只在固定 鼎新真实数据第三方模型对比、公开融合消融、任务感知头优化、流角色融合与优化模型再评估 reference 上做局部收束；分类任务校准 与 public route calibration 可写成 accepted improvement，检索任务仍沿用 任务感知头优化 confirmed retrieval，public 分支继续写成 public adapter / context-derived second-stream evidence。
...
## 已完成 阶段冻结：冻结 Phase D/E/F 工作区并提交
...
## 已完成 真实资产联合训练：补真实资产上的 Stage I multitask 联合训练证据
...
- 已修正 workload weak-label 的归一化尺度，并把 event replay pair 调整为优先近邻配对，避免验证/测试分区丢失 retrieval 监督。
...
## 已完成 鼎新真实数据 benchmark 刷新：刷新 Dingxin benchmark 分层资产

结果：已用当前 Phase C 代码重跑 Dingxin benchmark，使历史 分类任务、回归任务和检索任务 weak-label task evidence 与 thesis weak-label evidence 在资产层明确拆开。
...
## 已完成编码 物理一致性补强：补 Stage F 刚体运动物理约束
...
## 已完成编码 运行时推理管线补强：补 Stage G 语义事件融合
...
## 已完成编码 运行时推理补强：补 runtime inference
...
## 已完成主体 Phase D/E/F 产物闭环：把代码完成推进到产物闭环
...
- 已完成 runtime replay：
  ...
  - 当前 runtime task heads：risk_weak_label 分类、workload_weak_label 回归、事件回放标签 检索。
...
2. 深挖现有鼎新 Stage H 数据与论文本体模型：围绕 `risk_proxy / workload_proxy / event_replay_tag` 做小网格和消融表。
3. 拆解 `chronaris_opt` 机制贡献：把它从“Dingxin weak-label 最优结果”推进成“表示学习、对齐、因果掩码贡献可诊断”。
4. 有界补做 CPU-heavy `sklearn` / UAB torch 候选：只作为 public adapter baseline / calibration baseline，不扩写成本体闭环。
5. 整理 NASA/UAB 公开适配器迁移边界：用表格说明公开弱监督数据与鼎新真实双流数据的模态、标签、任务粒度和时间基准差异。
6. 核验 `rigid_body rotation` 字段：能找到成对角速度字段就补 rotation ablation；找不到则固化为真实字段缺口诊断。

## 已完成 刚体字段语义覆盖：扩充 rigid_body 的字段语义覆盖
...
## 已完成 语义事件支撑放大：放大 semantic event support 证据
...
- support 报告已经能回答：
  ...
  - dominant query：risk_weak_label
  - top offset：`0.626197s`

## 已完成 运行时服务化补强：runtime inference 服务化补强
...
- 串联 Stage I multitask checkpoint、rigid_body ablation、semantic support、runtime replay、Dingxin weak-label diagnostics、public adapter calibration summary。
...
- `skip-heavy` 已切到 bounded 路线：
  - `multitask` 使用 stage_h_window_stats_weak_label_input
  - `rigid_body / semantic / runtime` 复用稳定资产

## 已完成 鼎新弱监督任务扫描：鼎新 Stage H 论文本体模型深挖

目标：围绕现有鼎新 Stage H 双流数据和 Phase C multitask checkpoint，把 `risk_proxy / workload_proxy / event_replay_tag` 做成更充分的 thesis weak-label evidence，而不是只保留一轮训练结果。
...
## 已完成 鼎新弱监督任务扫描+：补一轮 `live_influx` thesis weak-label sweep

目标：在当前 bounded stage_h_window_stats_weak_label_input sweep 之外，使用本地 `127.0.0.1` 的 MySQL / InfluxDB CLI 形成 `live_influx` sample collection 证据，并与 weak-label 路线并排展示。
...
- 当前 blocker 已保留但未伪造成稳定证据：
  ...
  - `20260613T-stage-i-p11-live-influx-r1` 的 `4` 组合尝试在 `run_index=3/4` 处因 runtime cost 手动中断；稳定 `r2` 只复用其中已完成的两个 live child run，与 weak-label 的 `2` 组合口径对齐。

## 已完成 Chronaris 组件消融：`chronaris_opt` 机制诊断
...
- 把 `chronaris_opt` 从“Dingxin weak-label benchmark 最优候选”推进成“机制贡献可诊断”的证据。
- 在 分类任务、回归任务和检索任务 上拆解：
  - 去掉因果掩码。
...
## 已完成 公开数据适配校准：public adapter 有界校准 baseline

目标：中期前补做有限预算的 CPU-heavy `sklearn` / UAB torch 候选，用于公开弱监督数据的 adapter baseline / calibration baseline，而不是改写成双流本体闭环。
...
## 已完成 公开数据迁移边界：NASA/UAB 迁移与校准边界报告

目标：把 NASA/UAB 公开适配器结果整理成论文可引用的“迁移边界”证据，说明公开弱监督数据和鼎新真实双流数据之间的差异。
...
- 数据边界表：
  - 鼎新 Stage H：真实生理流 + 真实飞机时序流 + sortie/pilot/view。
  - UAB：公开 physiology + task/context-derived second stream + subjective workload。
  - NASA：公开 physiology + scenario/context-derived second stream + attention state。
- 任务边界表：
  - `risk_proxy / workload_proxy / event_replay_tag` 属于 thesis weak-label。
  - 分类任务、回归任务和检索任务 属于 Dingxin weak-label。
...
- 性能表：
  ...
  - 引用新的 公开数据适配校准 calibration summary。
...
## 已完成 刚体旋转审计：`rigid_body rotation` 字段核验与消融
...
## 已完成 论文图表材料：论文案例、消融表与说明图稳定化

目标：把现有和 弱监督扫描、组件诊断、公开数据校准、迁移边界与旋转审计 新增 evidence 转成论文可直接引用的表格、案例材料和说明图件，减少后期靠手工复制指标或临时画图。
...
- 首轮图包已被图表质量刷新 r3、runtime 语义案例刷新 r4、旧论文图表刷新 r5 和论文图表报告级重绘 r6 逐步替代，并已从 docs 产物目录清理。
- 当前中期图表入口只使用：
...
## 已完成 稳定扫描与字段契约收口：鼎新弱监督任务扫描/运行时服务冒烟验证 风险收口优化

目标：把 鼎新弱监督任务扫描 的“部分完成但不中断证据链”和 运行时服务冒烟验证 的“aligned 但还不是 exact schema”变成可复现、可解释、可验收的工程能力，而不是靠人工口头说明。

### 已完成 稳定扫描与字段契约收口-A：鼎新弱监督任务扫描 live_influx sweep partial/resume
...
### 已完成 稳定扫描与字段契约收口-B：运行时服务冒烟验证 runtime schema contract 与 exact 边界

当前事实：

- 运行时服务冒烟验证 真实 smoke 成功，但 `feature_schema_status=aligned`，`feature_schema_source=input_normalization_stats`。
...
- 新增 `src/chronaris/serving/runtime_schema_contract.py`
  ...
  - 输出 `runtime_schema_contract.json`，作为 运行时服务冒烟验证 后续部署契约事实源。
...
### 已完成 稳定扫描与字段契约收口-C：论文图表材料 图表同步刷新

目标：稳定扫描与字段契约收口 完成后，刷新 论文图表材料 thesis materials，让论文图表同步反映 鼎新弱监督任务扫描/运行时服务冒烟验证 的真实边界。

本轮结果：

- 刷新后的 thesis materials 曾被 旧论文图表刷新/r5 图包接管，当前入口已由 论文图表报告级重绘/r6 接管：
  - `docs/artifacts/assets/stage_i_thesis_figures/20260621T-stage-i-thesis-materials-r6-report-figure-polish/`
  - `docs/artifacts/stage_i/stage-i-thesis-materials-20260621T-stage-i-thesis-materials-r6-report-figure-polish.md`
- 旧 r2 历史图包、manifest 和报告已清理；仅保留 LLM preprocessing 历史输入表：
  - `docs/artifacts/assets/stage_i_thesis_figures/20260613T-stage-i-thesis-materials-r2-p18/runtime_semantic_case.csv`
...
## 已完成 中期报告材料冻结：中期报告材料冻结与 docs 入口清理
...
- 更新导航与状态：
  ...
  - `docs/artifacts/stage_i/README.md` 已把 论文图表材料/运行时服务冒烟验证 当前入口改为 `r2-p18 / r2-contract`，首轮 r1 降为历史入口。
  - `docs/STATE.md` 已把 论文图表材料/运行时服务冒烟验证 r1 标为首轮历史，并补入 稳定扫描与字段契约收口/p18 和中期写作材料入口。
...
## 已完成 DeepSeek 时序预处理：DeepSeek 在线时序数据预处理

目标：在现有 MySQL / InfluxDB 鼎新真实数据链路和 Stage H / Stage I 资产之上，接入 DeepSeek 在线大模型，形成中期可写的 LLM 辅助时序数据预处理能力。
...
## 已完成 LLM 预处理对比：LLM preprocessing 融入管线的对比实验与中期结果落地

目标：在 DeepSeek 时序预处理 `llm_preprocessing_context` 已真实生成的基础上，用 A0-A4 对比实验说明 LLM 接入现有 Stage I 数据融合管线的增量价值，并把结果写成中期报告可直接引用的 summary。

结果：

- A1 `llm_context`：`333/333` 条 Stage I weak-label task entries 已 attach DeepSeek 时序预处理 context，代码逐 entry 检查 `label_changed_count=0`、`label_unchanged=true`。
- A2 `llm_semantic_hints`：内置 semantic query bank 从 `3` 条扩展到 `7` 条，新增 `4` 条 DeepSeek 时序预处理 hints 均通过 recipe whitelist；本轮没有从现有 summary 伪造 view ranking/top attribution 重算。
- A3 `llm_runtime_explanation`：`12` 条 runtime semantic cases 中 `4` 条有 DeepSeek 时序预处理 LLM explanation，解释子集四项完整性 `model_prediction / semantic_attribution / schema_gap_note / weak_label_boundary = 1.0`。
...
- `A0 baseline`：不接 LLM context；复用当前 Stage I task entries 与内置 semantic query bank。
- `A1 llm_context`：attach DeepSeek 时序预处理 context 到 Stage I task entries；已验证 `label_unchanged=true`。
...
- 已更新 `docs/midterm/README.md`，加入 LLM 预处理对比 result summary。
...
## 已完成 图表质量刷新：中期 thesis materials r3 图表质量刷新

目标：修复 论文图表材料/稳定扫描与字段契约收口 thesis materials 中低信息或易误读图表，先生成 r3 图表包验证新版构图，同时保持 evidence layer、runtime schema、rotation、LLM 边界清楚；该图包后续由 r4/r5 接管，当前已由 论文图表报告级重绘/r6 接管。
...
- r3/r4 run 已由 r5 接管并从 docs 产物目录清理；当前 thesis materials 入口见 论文图表报告级重绘 的 `20260621T-stage-i-thesis-materials-r6-report-figure-polish`。
- 本轮输出 `8` 张 PNG 与 `8` 张 CSV：
  ...
  - `weak_label_sweep_ablation`：改为 weak-label/live best metrics、completed/partial 状态和小网格 lag heatmap。
  - `chronaris_opt_component_ablation`：改为 分类任务、回归任务和检索任务 分任务面板 + normalized delta 贡献。
  - `public_transfer_boundary`：完全中文化，改为公开适配、鼎新弱标注主线、鼎新真实数据弱监督任务消融的正向分工图。
  ...
  - `llm_comparison_a0_a4`：纳入 DeepSeek 时序预处理/LLM 预处理对比 A0-A4 对比，明确 LLM 仅作为 preprocessing context / whitelisted hints / runtime explanation / pending review packet。
...
- Dingxin component ablation 使用已有 `27` 行 r2 artifact 重绘，未重跑。
- weak-label sweep 使用已有 stable/partial artifacts 重绘，未扩大 live_influx 网格。
- semantic 与 LLM 使用已有 Stage I semantic support、DeepSeek 时序预处理/LLM 预处理对比 artifacts 重绘；未从 summary 倒推 attribution 改善，也未声称人工复核完成。
...
- `build_thesis_materials.py` 已用 r3 run_id 实跑；r3/r4 输出已由 旧论文图表刷新/r5 图包替代并清理。
- PIL/pandas 验证已确认 `8` 个 PNG 非空且可读取、`8` 个 CSV 有行且关键字段存在、`figure_manifest.json/table_manifest.json` 均为 `8` 项、报告中图表路径可解析。
- 2026-06-19 复核后，旧 r1、r2 thesis-materials 历史图包 PNG/manifest/report 与 r3 thesis-materials 图包已清理，避免中期写作误用；`r2-p18/runtime_semantic_case.csv` 因仍被 DeepSeek/LLM preprocessing 复现实验引用而保留。

## 已完成 runtime 语义案例刷新：runtime semantic case r4 图表质量刷新

目标：把 r2 历史图包中仅保留为 DeepSeek 时序预处理/LLM 预处理对比历史输入的 `runtime_semantic_case.csv` 重新纳入当前 thesis materials 生成链路，并替换旧 runtime/semantic case 中近乎平坦折线、重复归因柱和大号 965 vs 1930 字段数对比。

结果：

- r4 run 已完成验证，随后被 旧论文图表刷新/r5 接管并从 docs 产物目录清理；当前 run 见 论文图表报告级重绘/r6：
  - `docs/artifacts/assets/stage_i_thesis_figures/20260621T-stage-i-thesis-materials-r6-report-figure-polish/`
  - `docs/artifacts/stage_i/stage-i-thesis-materials-20260621T-stage-i-thesis-materials-r6-report-figure-polish.md`
...
- r2 历史图包的 `runtime_semantic_case.csv` 继续保留为 DeepSeek/LLM preprocessing 历史输入表；r6 只是把该表作为当前中期图表生成输入，不改写历史 LLM summary。
...
## 已完成 防泄漏鼎新组件消融：leakage-safe Dingxin component ablation

目标：在不改写历史 Dingxin weak-label 结果的前提下，新增 `protocol=leakage_safe_v1` 的 分类任务、回归任务和检索任务 组件消融协议，审计标签来源字段与输入特征同源风险，并把模型骨干结构消融和任务适配层消融拆分为独立产物。
...
- 分类任务、回归任务和检索任务 seed rows 均完成；检索任务 对单飞行员视图记录 skipped fold，不生成伪正样本。
- 完整防泄漏任务输入：
  - 分类任务 macro-F1=`0.17333333333333334`，balanced accuracy=`0.3333333333333333`。
  - 回归任务 RMSE=`862.6941748579226`，NRMSE=`0.3695127256456088`，persistence RMSE=`201.4895651832178`，相对持久性基线改进率=`-3.281582393973904`。
  - 检索任务 Top-1=`0.0`，Top-3=`0.0`，Top-5=`0.0`，MRR=`0.0114187054292705`，valid query=`74`，candidate=`8140`。
- `t3_similarity_distribution.csv` 对原始 `488400` 行进行确定性压缩，保留全部正样本并抽样负样本，写出 `20000` 行。

边界：

- 历史 Dingxin component ablation r2 仍保留为历史 Dingxin weak-label 诊断；论文实验章节优先引用本轮 `leakage_safe_v1` 结果。
- 检索任务 严格协议下已完成评价，但当前安全向量 Top-1/Top-3/Top-5 均为 `0.0`，不能写成已解决配对检索。

## 已完成 旧论文图表刷新：thesis materials r5 leakage-safe refresh

目标：在 r4 runtime case refresh 基础上，把论文图件全部改为 A4 中期报告可用的中文 300dpi 图，并接入 防泄漏鼎新组件消融 的 leakage-safe 消融结果。

结果：

- r5 曾输出 `11` 张 PNG 与 `11` 张 CSV，并已在 2026-06-21 深度清理中由 论文图表报告级重绘/r6 接管后从 docs 产物目录删除；如需追溯 r5，请从 git 历史读取。
- r5 输出内容包括：
...
- `figure_quality_audit.csv` 曾确认 11 张 PNG 均存在、非空、DPI 约 `300`，并与 table/figure manifest 数量匹配；当前图表 QA 以 论文图表报告级重绘/r6 的 `figure_quality_audit.csv` 为准。
...
## 已完成 论文图表报告级重绘：thesis materials r6 report figure polish
...
## 已完成维护 公开模型对比：源码、脚本与 docs/artifacts 深度清理
...
- 已删除被 论文图表报告级重绘/r6 接管的 旧论文图表刷新/r5 thesis figure 图包与报告；当前中期图表入口只保留 r6。
- 已删除 鼎新弱监督任务扫描 stable/partial 目录下两个空 `runs/` 子目录。
...
- `20260613T-stage-i-thesis-materials-r2-p18/runtime_semantic_case.csv` 是 DeepSeek/LLM preprocessing 历史输入表，保留。
...
## 已完成 公开模型对比与公开融合刷新：public model comparison and fusion refresh

目标：面向中期报告补齐公开 UAB/NASA 模型对比、chronaris_public_fusion 长程 refresh、resume/partial/log/progress contract，并在 公开融合刷新 后重新运行 公开模型对比 生成最终图表。

代码入口：

- 公开模型对比 builder：`src/chronaris/pipelines/stage_i/public/model_comparison.py`
- 公开模型对比 CLI：`scripts/stage_i/public/build_public_model_comparison.py`
- 公开融合刷新 refresh：`src/chronaris/pipelines/stage_i/public/fusion_refresh.py`
- 公开融合刷新 CLI：`scripts/stage_i/public/run_public_fusion_refresh.py`
...
- 公开融合刷新 `missing_figures=[]`，公开模型对比 `missing_metrics=[]`。
- public 数据集仍按公开数据适配、公开数据校准和上下文构造第二输入流证据引用，不改标签、不改 split、不使用 test fold 统计量做训练归一化或调参。
...
- `chronaris_opt` 与 分类任务、回归任务和检索任务：中期前要补机制诊断；仍只能写成 Dingxin weak-label benchmark evidence，不能写成人工真值 thesis task fully closed。
- `risk_proxy / workload_proxy / event_replay_tag`：中期前要补小网格与消融；仍只能写成 thesis weak-label evidence。
- DeepSeek 在线 LLM 预处理：DeepSeek 时序预处理 已接入字段语义归一、weak-label 复核、schema gap policy、runtime 解释和切片整合；仍不能写成 OpenAI 默认接入、人工真值替代、原始全量数据外发或核心因果证据。
