...
在不依赖新增鼎新数据或人工标签、不读取外层测试的前提下，首次使用原始鼎新核心任务监督训练任务感知 Chronaris：先建立强任务安全基座，再验证近零门控的连续因果残差，按门禁进入部分解冻和选择性训练外教师蒸馏。

当前分支：`codex/dingxin-task-aware-safe-residual-20260715`。

## 当前里程碑：任务感知安全残差阶段 2 已按门禁停止

阶段 1、1B、1C 已经合入 `main@719c47c`。阶段 2 首次使用原始鼎新任务监督训练安全残差头，并在冻结无损门禁通过后按预注册范围部分解冻 Chronaris 观测编码、观测更新、融合输出和时延门控。冻结安全残差通过，部分解冻安全性也通过，但两种主干学习率比例均只改善一项任务，未达到“至少两项稳定改善”的研究门禁。当前按 [任务感知安全残差融合预注册](notes/dingxin-task-aware-safe-residual-2026-07-15.md) 停止，不启动训练外教师蒸馏，不打开外层测试。

### 当前门禁

- 冻结安全融合：机动相对基座不低超过 `0.005`，响应 RMSE 不恶化超过 `1%`，高响应 AUPRC 不低超过 `0.005`，每项至少 `4/6` 支持无明显伤害。
- 部分解冻：至少两项稳定改善、第三项满足无损门禁；机动、响应、高响应的训练内参考分别为 `0.88/0.92/0.72`、RMSE ratio `<=0.95`、总体 normalized AP `>=0.20`。
- 配置锁定：三项均不差于同预算最强基线，每项至少 `4/6` 支持方向为正；本轮不打开外层测试。
