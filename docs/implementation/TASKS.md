...
G4.1–G4.2、G5 seed 17 正式筛选、G6 仿真三随机种子五方法锁定重训和三随机种子六方法统一表示已经完成；仿真 15/15 checkpoint、54/54 表示及两级门禁全部通过。首次 consumer 已在方法比较前定位并修正高维 `lbfgs` 运行时瓶颈，旧单方法运行隔离为 runtime benchmark；35 场景 G2 压力表示正在导出，clean consumer 随后从空根按统一 OvR `liblinear` 重跑。鼎新正式重训新增 100 ms 方法无关因果时间箱：首个上下文航电/生理事件由 1779/150 点压缩为 300/120 点，Chronaris GPU 单 epoch为 40.07 秒，旧未合并 checkpoint 已由协议门禁隔离。
