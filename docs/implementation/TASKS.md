...
在不依赖新增鼎新数据或人工标签的前提下，完成 Chronaris 核心任务恢复的编码、开发漏斗、一次性确认和协议审计；出现外层支持重叠后停止晋级与仿真次级确认，保留原始产物并回写结论边界。

当前分支：`codex/chronaris-core-task-recovery-20260713`。

## 当前里程碑：核心任务恢复收口

本轮开发和一次性确认均已执行。外层支持审计发现前两个留一视图折的内层训练集与外层留出集使用同架次重叠时间窗，导致树模型和直接观测路径可记忆跨视图同锚点样本；本轮确认状态固定为 `invalid_protocol_overlap`，不形成论文指标或模型晋级。唯一授权已经消费，不重跑、不调参，不运行仿真次级确认。

### 本轮新增进度

- 当前 5 秒机动任务继续保留；未来 5 秒机动诊断因历史增益与双流增益均未过门槛，不提升为主任务。1 秒和 2.5 秒稠密训练上下文未形成可复用材料，结构化记为 unavailable。
- 已实现观测信息保留残差、航电专用机动锚点、连续融合修正、真实有效掩码、任务路由和任务感知头；六方法共享 1,000/2,500/5,000/10,000 个 MiniRocket 时序卷积核预算及相同下游候选族。
- 三折开发锁定 Chronaris 路由为：机动分类使用航电观测保留序列，生理响应使用观测—连续拼接，高响应识别使用连续序列。开发门禁通过，但只用于唯一配置锁定。
- 一次性确认完成 3 随机种子 × 3 留一视图折 × 6 方法，共 54 个单元和 162 条指标；授权记录为 `actual_open_count=1`、`consumed=true`。
- 外层支持审计发现第一、二折各有 361 个训练—held-out 支持区间重叠配对，训练上下文 31/31 全部受影响，并各有 31 个同架次同锚点配对；第三折零重叠。
- 仅作失败诊断的第三折三随机种子均值为：机动分类宏平均 F1（Macro-F1）0.7778、生理响应均方根误差（RMSE）0.8222、高响应精确率—召回率曲线下面积（AUPRC）0.8127。不得把受污染三折原始汇总写入论文。
- 现有 2026-07-12 鼎新留一视图主表使用相同外层划分，原始产物不删除、不改数值，但相关读者结论降级；仿真、公开数据适配、时间机制与压力证据不受此项划分审计影响。
...
- 五个外层折的角色样本 ID 互斥；既有“共享航电原始时间区间零重叠”只验证了内层训练集与内层验证集，不覆盖外层留出集。2026-07-13 新审计确认前两个留一视图外层折不满足支持区间隔离。
...
- 任何鼎新候选筛选或正式确认都必须先对 30 秒输入与未来 5 秒目标的完整 35 秒支持区间做跨角色清除；只比较 context ID、view ID 或 pilot ID 不构成隔离。
- 本轮外层授权已经消费；除非另行预注册全新按架次隔离协议，不得重新打开外层留出集，也不得用本轮外层结果调整任务、路由、消费者或阈值。
