...
在不依赖新增鼎新数据或人工标签、不读取外层测试的前提下，审计鼎新三个核心任务的泄漏安全训练内上限，并仅在三项上限全部通过后放行冻结专家安全融合。

当前分支：`codex/dingxin-core-feasibility-20260714`。

## 当前里程碑：核心任务可行性门禁已收口

12 个主候选已在三个留一视图外层折各自的净化 inner-train/validation 上完成。机动强度分类、未来生理响应预测和高生理响应识别的最佳训练内均值为 `0.8241/0.8368/0.7875`，三项上限门禁均失败；外层测试未打开，历史确认指标未改变。冻结专家安全融合按协议结构化阻断，下一项任务不得自动启动。

### 本轮结果与退出决定

- 完整 30 秒航电统计是机动分类最有价值的允许信息路径，Macro-F1 为 `0.8241`，高于最近 5 秒路径的 `0.7333`；第三折降到 `0.4722`，暴露明显跨视图差异。
- 连续响应中双流直接观测 RMSE 为 `0.8368`，只比航电路径 `0.8475` 略好；高响应识别由 MiniRocket 5,000 kernels 达到 `0.7875`，仍未越过门槛。
- 第三折机动高等级阈值从前两折的 `0.568` 变化为 `1.084`，验证集仅覆盖两个机动类别；其生理响应验证均值比训练均值高 `45.7%`。
- 35 秒完整支持区间审计发现并净化第三折两个边界训练上下文；开发清单不包含任何外层样本标识，标签与阈值只用净化后的 inner-train 重拟合。
- 按预声明退出要求，不修改 Chronaris 主干、不启动安全融合、不打开外层测试、不允许进入任务感知主干开发。若继续研究，必须先形成新的任务稳定性与分布处理方案并单独预注册。
- 紧凑证据入口：[鼎新核心任务可行性与安全融合审计](../artifacts/runs/2026-07-14_dingxin-core-feasibility/gap_report.md)。
