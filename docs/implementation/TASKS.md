...
v1 失真诊断、v2 结构修复、24 项超参数筛选、鼎新三个 inner-validation 折复核和三随机种子锁定确认均已完成。一次性审计确认六项主指标全部未越过硬门槛，v2 不晋级，论文主模型保持 v1。独立仿真中 v2 仅在机动分段取得六方法最高均值，分类与回归仍明显落后；随机缺失退化斜率排名第 2，连续缺失排名第 4。因果未来不变性、无效查询池化和时间机制门禁通过，无损融合、类别召回差距及连续缺失前三门禁未通过。当前只继续锁定后的 NASA/UAB 一次性公开数据适配和最终证据包，不允许确认结果回流当前开发轮次。
...
- 一次性审计记录 `promoted=false`、`paper_main_model=chronaris_v1`、`locked_results_returned_to_development=false`。NASA/UAB 只运行一个由锁定配置映射的公开数据适配候选；NASA 三随机种子已完成，UAB 正在执行完整留一受试者折。
...
- 新增接口后的全量回归为 `421 passed, 8 skipped, 319 warnings`；共享因果查询域修复的表示合同、单流、朴素同步和公共预训练聚焦测试为 `19 passed`。
