...
v1 失真诊断和 8 项结构筛选均已完成。完整 v2 的航电保真、表示健康和时间机制通过，但生理保真比只有 0.5571；三种投影蒸馏候选最高仍只有 0.5476，未获得晋级资格。直接因果生理残差候选把航电/生理保真比提升到 1.1396/2.3466，时间机制误差比为 0.8804，并通过因果不变性、无效位置池化和时延掩码门禁，成为唯一获准进入超参数筛选的结构。当前按锁定的 `hidden={64,96,128} × lag={固定五档,连续 basis} × solver={固定子步 Euler,RK4} × lr={1e-3,3e-4}` 运行 24 个候选；训练和排序只读取 G1 observed-only 双流及任务无关证据。
