...
## 当前里程碑：v2 未晋级后的外部确认与证据收口

v1 失真诊断、v2 结构修复、24 项超参数筛选、鼎新三个 inner-validation 折复核、三随机种子锁定确认、NASA/UAB 一次性公开数据适配和最终差距证据包均已完成。一次性审计确认六项主指标全部未越过硬门槛，v2 不晋级，论文主模型保持 v1。独立仿真中 v2 仅在机动分段取得六方法最高均值，分类与回归仍明显落后；随机缺失退化斜率排名第 2，连续缺失排名第 4。因果未来不变性、无效查询池化和时间机制门禁通过，无损融合、类别召回差距及连续缺失前三门禁未通过。确认结果未回流当前开发轮次，本 goal 已进入测试、文档和产物收口。
