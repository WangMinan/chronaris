...
唯一活动开发分支为 `research/thesis-continuous-semantic-fusion-202609`。当前执行合同为 [论文主线连续对齐与语义融合评价协议 v3.2](../requirements/thesis-continuous-semantic-fusion-evaluation-v3.2.md)，v3/v3.1 保留追溯；现场保护与基线预检见 [2026-09-01_thesis-mainline-preflight](../artifacts/runs/2026-09-01_thesis-mainline-preflight/report.md)。
...
6. **当前：**论文级模型、预算、数据选择、消费者和运行顺序已冻结。受控仿真 15/15 个正式预训练 checkpoint、9/9 验收完成；首次 clean 表示导出在旧的跨方法有效 mask 相等检查处 fail-fast，未生成任务指标。v3.2.1 仅将跨方法审计改为样本、查询时间和来源严格相同，并把各方法真实 mask 纳入哈希；冻结 checkpoint 逐文件复核后复用。恢复 clean、压力、时间机制和四项消融后，先审计三项机制硬门，再运行 CogPilot、CLARE 和鼎新外层确认。应用增量不足不触发无限调参。
