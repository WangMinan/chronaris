...
集成主线为 `main`，本轮冻结来源分支为 `research/thesis-continuous-semantic-fusion-202609`。用户于 2026-09-05 决定先合并现有研究基线，再从主分支创建后续改进分支；此决定仅更新分支安排。当前实验合同仍为[论文级冻结实验执行清单 v3.2.3](../requirements/thesis-frozen-paper-evaluation-v3.2.3.md)，此前版本保留追溯；集成检查见[主分支集成复核](../review/stage/thesis-mainline/main-integration-2026-09-05.md)。
...
6. **已完成批准范围：**v3.2.3 无观测表示与严格安全门修复验收 6/6，CPU 全量测试 481 项通过、15 项跳过，CUDA 实际窗口重放通过。提交 `6a2393e4` 冻结后，同一运行会话完成 630 份压力表示、20,160 条压力指标、144 份时间机制训练/验证表示和 3,360 条时间机制指标；既有 27 个检查点不变。总审计及恢复重放完成，六份核心结果哈希完全一致，141 份缓存未改。详见[最终复核](../review/stage/thesis-mainline/frozen-simulation-review-2026-09-04.md)。
7. **后续研究待修订范围确认：**最终全量测试 `488 passed, 8 skipped`，但总硬门未通过：显式时移、事件配对、连续演化和物理一致性四项通过，未来信息隔离与安全旁路失败。安全旁路保留十二单元中的三个失败；未来扰动三种子均超阈值，只读定位为事件分数先按全窗最大值归一化、后做历史掩码。拟修复须改变非空模型行为，尚未实施或启动额外调参。所有不利结果保留，公开数据和鼎新外层继续关闭，不声称完整论文任务完成。
8. **当前分支集成：**按用户 2026-09-05 的指示，将全部未提交材料与既有删除一并归档、推送，并将研究分支合入 `main`。保留原始提交历史、冻结检查点和证据；合并不改变研究硬门、模型行为或外层授权。范围与验证见[集成复核](../review/stage/thesis-mainline/main-integration-2026-09-05.md)。

2026 年 7 月公开与研究分支结果保留用于追溯，但受协议缺陷影响的结果均标记 `superseded_due_to_protocol_defects`，不进入毕业论文主表。下方安全滞后感知融合任务作为历史研究记录保留。

## 历史任务：安全滞后感知融合新主线

以下为论文主线建立前的历史快照：两条鼎新真实数据研究线（简化下游评价、任务感知安全残差/残差激活）均已收口退出，当时活动分支为 `research/safe-lag-aware-fusion-20260718`（安全滞后感知融合）。审计与后续研究计划、协议已完成：
