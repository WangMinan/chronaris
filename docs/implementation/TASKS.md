...
## 当前里程碑：v2 结构筛选与任务无关门禁

v1 失真诊断已完成 7/7 验收：表示没有秩塌缩，但航电信息路径不足，时延注意力近似均匀，尺度门控集中在最短档，多损失最小梯度余弦为 -0.5743。当前按预注册顺序执行 v1、语义组编码、可学习因果注意力、私有—共享子空间、滞后目标、修正物理、缺失课程和完整 v2 共 8 项结构候选。训练只读取 G1 observed-only 双流；保真、表示健康、合成时间机制、因果不变性和池化门禁完成前不生成排名。24 项超参数筛选代码入口已实现，但完整 v2 未通过结构门禁时会 fail closed。
...
- Chronaris v2 新增严格 `architecture_version=v2` checkpoint；v1 只走旧实现。航电按六个语义组编码值、有效掩码和观测年龄，内部使用流特异连续动力学，外部固定为航电私有 24 维、生理私有 16 维、因果共享 24 维。
