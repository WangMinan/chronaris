...
## 当前里程碑：鼎新三个 inner-validation 折任务无关复核

v1 失真诊断、8 项结构筛选、直接因果生理残差修复和 24 项超参数筛选均已完成。固定网格为 `hidden={64,96,128} × lag={固定五档,连续 basis} × solver={固定子步 Euler,RK4} × lr={1e-3,3e-4}`；24 个候选均完成 30 epoch，任务标签、outer-test 和封存确认访问均为 false。全部候选通过双模态保真、表示健康、时间机制、因果不变性、无效位置池化和时延掩码门禁；按公共自监督验证损失、最差保真、时间机制误差和参数量的词典序选出三个 96 维、`1e-3` 候选。当前只在鼎新三个 inner-validation 折上复核公共自监督损失与保真，仍不读取下游标签。
