...
2. **部分完成：**确定性训练、真实 validation checkpoint、单次归一化、训练折缩放和正确上下文时长已修复；聚焦测试 `16 passed`、完整测试 `433 passed, 8 skipped`、CUDA 短重放 `2 passed`。当前继续完成原生时间公开数据输入、lazy batch provider 和受控缓存，工程证据见 [可信训练修复](../artifacts/runs/2026-09-01_training-trust-repair/report.md)。
