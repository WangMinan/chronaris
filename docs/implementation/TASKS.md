...
在不依赖新增鼎新数据或人工标签、不读取外层测试的前提下，完成教师辅助残差激活与任务解耦，验证 Chronaris 连续因果支路能否在安全基座之上形成跨验证支持稳定增量；仅在通过时进入三随机种子稳定性确认与唯一配置锁定。

当前分支：`codex/dingxin-residual-activation-20260715`。

## 当前里程碑：教师辅助残差激活已按门禁停止

本轮从阶段 2 安全基座继续，完成 8 个预注册候选 × 6 个唯一训练内验证支持的教师辅助残差激活筛选。模型采用固定或样本条件门控、极小随机残差初始化、显式基座误差学习、任务独立适配器和交叉拟合教师；随后执行一次不扩展候选的实现校正，分别按机动、连续响应和高响应训练内指标选取任务适配器状态，再组合为同一个 Chronaris 检查点。最终安全门禁通过，但激活和研究门禁失败，按 [教师辅助残差激活预注册](notes/dingxin-residual-activation-2026-07-15.md) 停止，不进入三随机种子稳定性确认，不打开外层测试。

### 当前门禁

- 残差激活：至少 `4/6` 验证支持的最优 epoch 大于 0，至少 20% 样本获得可测量修正，中位残差贡献比位于 `[0.02,0.30]`，门控不集中到上下界。
- 安全无损：机动相对基座不低超过 `0.005`，响应 RMSE 不恶化超过 `1%`，高响应 AUPRC 不低超过 `0.005`，每项至少 `4/6` 支持通过。
- 研究增益：至少两项稳定改善，且其中一项必须是机动分类或连续响应；只有通过时才允许三随机种子确认。

### 教师辅助残差激活结果与退出决定

- 8 个候选的 48 个候选—验证支持运行全部完成；教师训练预测均来自未见对应训练样本标签的交叉拟合模型，正式推理不需要教师集成。
- 排名最高的样本条件门控教师辅助候选取得机动平均宏平均 F1 `0.9460`，相对安全基座 `0.9037` 明显提高；最差支持仍为 `0.7500`。
- 同一候选的连续响应中位 RMSE 比率降至 `0.8626`，`4/6` 支持技能为正，但平均技能仍为 `-0.0171`，困难支持的负技能尚未消除。
- 高响应平均归一化平均精确率为 `0.4042`，相对安全基座 `0.3652` 提高，`5/6` 支持为正。
- 候选均满足无损要求，且多个候选达到机动和高响应两项改善判据；但是最佳候选跨支持中位残差贡献比仅 `0.0005`，全部 8 个候选均低于 `0.02` 激活下限，不能证明连续因果支路形成稳定增量。
...
- 按协议不执行三随机种子配置锁定，也不在同轮回流调参。证据入口：[教师辅助残差激活与任务解耦](../artifacts/runs/2026-07-15_dingxin-residual-activation-task-decoupled/gap_report.md)。

### 前一里程碑：任务感知安全残差阶段 2 已收口

#### 阶段 2 结果与退出决定

- 六方法同预算任务基座显示，机动分类由航电单流领先，平均宏平均 F1 为 `0.9037`；连续响应由直接观测基座取得最低中位 RMSE 比率 `1.0071`，但平均技能仍为负；高响应由直接观测基座取得平均归一化平均精确率 `0.3652`。
- 冻结安全残差选用航电机动基座和直接观测响应基座，机动平均/中位/最差宏平均 F1 为 `0.9164/0.9630/0.7500`，连续响应中位 RMSE 比率为 `1.0071`、平均技能为 `-0.2035`，高响应平均/中位归一化平均精确率为 `0.3773/0.4202`；三项无损支持数均为 `6/6`。
- 主干学习率比例 `0.02` 的部分解冻只改善高响应，比例 `0.05` 只轻微改善连续响应；二者的机动平均宏平均 F1 都没有变化。两项候选均保持三任务 `6/6` 安全支持，但改善任务数只有 `1/3`。
- `frozen_safety_passed=true`、`partial_safety_passed=true`、`research_gate_passed=false`、`allow_teacher_distillation=false`、`configuration_locked=false`、`outer_test_opened=false`。
- 当前分支只提交阶段 2 实现、测试、紧凑证据和差距结论；20 MB checkpoint 与训练状态保留在被忽略的重型根。既有训练、外层结果和确认指标没有改变。
- 证据入口：[鼎新任务感知安全残差阶段 2](../artifacts/runs/2026-07-15_dingxin-task-aware-safe-residual/gap_report.md)。

### 前一里程碑：目标重构与统一条件比较已收口
