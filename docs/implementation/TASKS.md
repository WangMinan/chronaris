...
6. **当前：**论文级模型、预算、数据选择、消费者和运行顺序已写入[冻结实验清单](../requirements/thesis-frozen-paper-evaluation-v3.2.md)；G1 CUDA 接线冒烟完成 5/5 方法、9/9 验收，完整测试 `454 passed, 8 skipped`。先运行受控仿真的连续演化、运动学约束和安全单流旁路三项消融硬门；通过后运行 CogPilot、CLARE、鼎新外层确认、其余核心消融、负对照和运行时实验。应用增量不足不触发无限调参。
