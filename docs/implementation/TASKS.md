...
7. **当前等待确认：**最终全量测试 `488 passed, 8 skipped`，但总硬门未通过：显式时移、事件配对、连续演化和物理一致性四项通过，未来信息隔离与安全旁路失败。安全旁路保留十二单元中的三个失败；未来扰动三种子均超阈值，只读定位为事件分数先按全窗最大值归一化、后做历史掩码。拟修复须改变非空模型行为，不在当前批准范围内；未实施、未启动额外调参。所有不利结果保留，公开数据和鼎新外层继续关闭，不合入主分支、不声称完整论文任务完成。
