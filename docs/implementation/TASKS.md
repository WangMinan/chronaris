...
在不新增鼎新数据或人工标签、不修改标签公式、外层划分、冻结 consumer、`[B,96,64]` 表示合同和既有 confirmed metrics 的前提下，完成 Chronaris v2 表示失真诊断、版本化模型骨架、任务无关结构与超参数筛选、三随机种子锁定复验以及锁定后的 NASA/UAB 外部确认。六项预声明主指标必须全部排名第一才允许晋级 v2。

当前分支：`codex/chronaris-v2-mainline-20260712`，基线为 `main@23b968e`。

## 当前里程碑：v2 未晋级后的外部确认与证据收口（已完成）

v1 失真诊断、v2 结构修复、24 项超参数筛选、鼎新三个 inner-validation 折复核、三随机种子锁定确认、NASA/UAB 一次性公开数据适配、最终差距证据包、全量测试和文档产物收口均已完成。一次性审计确认六项主指标全部未越过硬门槛，v2 不晋级，论文主模型保持 v1。独立仿真中 v2 仅在机动分段取得六方法最高均值，分类与回归仍明显落后；随机缺失退化斜率排名第 2，连续缺失排名第 4。因果未来不变性、无效查询池化和时间机制门禁通过，无损融合、类别召回差距及连续缺失前三门禁未通过。确认结果未回流当前开发轮次；下一次继续优化必须建立新的预注册开发轮次和新的独立仿真确认族。

### 本轮新增进度

- 鼎新锁定重训完成 15/15 checkpoint，训练全程未读取任务标签、仿真真值或 outer-test；紧凑证据记录 `outer_test_request_count=0`，既有确认指标未改动。表示导出首次因航电单流的模态专属有效掩码与其他方法的共享有效域不一致而 fail closed；已改为所有方法导出“任一模态具有因果历史”的共享查询域，同时各编码器仍用自身结构化缺失 token 生成序列。首个折三角色六方法对齐哈希已复验通过，失败的局部表示已清理后从 checkpoint 恢复导出。
- 鼎新表示导出完成 15 个随机种子—外层折、270/270 份训练/验证/外层测试表示，六方法在共享因果查询域上三角色全部对齐，表示阶段未打开任务指标；冻结 consumer 随后完成 90/90 单元、5,040 条指标和 8/8 门禁。
- 固定数据同协议复验中，v2 Chronaris 的机动强度分类 Macro-F1 为 0.7254，高生理响应 AUPRC 为 0.8410，连续生理响应 RMSE 为 0.3346；对应硬门槛为高于 0.9409、高于 0.8839、低于 0.2912。三项均未达到方法第一，且高响应识别未保留 v1 的分项优势。本轮配置保持冻结，只形成差距报告，不根据这些结果继续调参。
- 独立仿真确认完成 630/630 个冻结 consumer 评估。v2 的负荷分类 Macro-F1、负荷回归 RMSE 和机动分段 Macro-F1 为 0.3044、0.2539 和 0.4123；机动分段均值在新封存族中居首，且 2/3 随机种子排名第一，但仍未超过预声明硬门槛 0.4687。
- 同一独立封存族上，v2 的生理响应时延 MAE 从 v1 的 7.5199 秒改善到 7.1546 秒，时钟偏移 MAE 从 0.8864 秒轻微变为 0.8944 秒，二者均通过“不恶化超过 10%”门禁。
- 正式消融显示，修正物理对分类、回归和分段的三随机种子均值方向均为正；缺失课程对分类和分段为正、对回归为负。两项机制均存在随机种子方向不一致，只形成分任务机制诊断。
- 一次性审计记录 `promoted=false`、`paper_main_model=chronaris_v1`、`locked_results_returned_to_development=false`。NASA/UAB 只运行一个由锁定配置映射的公开数据适配配置，完成 6 个数据集—随机种子确认单元和 365 个折；NASA 综合分类 Macro-F1 为 0.5664 ± 0.0085，UAB N-back 与椅背加热任务 RMSE 为 5.0962 ± 0.0578、1.4556 ± 0.0202。
- 锁定仿真 clean、独立封存 35 场景、冻结压力 consumer、时间机制、修正物理/缺失课程正式消融、六指标一次性晋级审计和锁定后 NASA/UAB 公开数据适配已全部实现为 fail-closed evidence 队列。公开数据只运行一个由锁定配置映射的适配候选，不使用公开标签选择模型，并继续明确其第二输入流是上下文构造而非鼎新航电流。
- 最终独占 CUDA 全量回归为 `425 passed, 8 skipped, 319 warnings`；Ruff、`compileall` 和 `git diff --check` 均通过。共享因果查询域修复的表示合同、单流、朴素同步和公共预训练聚焦测试为 `19 passed`。

- v1 失真诊断在 seeds 17/29/43 上完成：航电/生理表示恢复 R² 均值为 0.9241/0.9319，有效秩约 22；注意力近似均匀比例为 0.978–0.995，三个旧时延门控均值约为 0.947/0.057/0.011，多损失最小梯度余弦为 -0.5743。
- Chronaris v2 新增严格 `architecture_version=v2` checkpoint；v1 只走旧实现。航电按六个语义组编码值、有效掩码和观测年龄，内部使用流特异连续动力学，外部固定为航电专属 24 维、生理专属 16 维、因果共享 24 维。
- learned Q/K/V、`log1p` 相对时间偏置、五档或连续时延 basis、双模态直接残差、结构化 missing token 和真实有效掩码均已接入；池化统一使用 masked mean，禁止无效查询进入 pooled representation。
- 物理残差在 v2 中先按训练折稳健归一化参数逆变换，再执行严格单位/轴向映射和周期角差；不可靠映射结构化为 unavailable。物理、保真和因果目标按 epoch 10–20 线性启用。
- 缺失课程固定为随机缺失 10/20/30/40%、连续缺口 5/5/10/15 秒、时钟偏移 0.25/0.5/0.5/1 秒；逐训练步骤在共享参数上审计公共、保真、因果和物理四组梯度，低于 -0.1 的冲突步骤超过 20% 时启用 PCGrad。
- v2 早停最早允许点固定为 epoch 30，保证 epoch 11–20 的融合层训练和 epoch 21–30 至少 10 个联合解冻 epoch 均真实执行；30-epoch 微型闭环在 patience 1 条件下仍完成联合解冻，并在冲突步骤率 30% 时自动启用 PCGrad。此前 r1–r3 部分根保留为协议诊断，最终结构筛选使用 `2026-07-12_chronaris-v2-structure-screen-seed17-r4`。
- checkpoint 职责分离：`best.pt` 继续只记录公共自监督验证损失最优 epoch，候选排序使用其最优公共损失数值；保真、表示健康、因果和时间机制门禁统一读取完成三阶段训练后的 `last.pt`。语义组候选按 final state 重算后，航电/生理保真由最佳公共损失 epoch 的 86.2%/48.5% 提高到 90.3%/55.7%，证明后期联合训练状态不能被提前 checkpoint 替代。
- G1 v2 训练烟雾 7/7 通过，结构候选烟雾 4/4 通过；最终三阶段微型闭环走到 epoch 30，进入联合解冻并自动启用 PCGrad。新增协议、missing token、Q/K/V 和图件入口后，全量回归为 `400 passed, 8 skipped`。
- 任务无关选择合同会拒绝任何包含任务标签、outer-test 或封存确认结果的候选证据；六主指标最终晋级审计只要一项未过线即保留 v1。结构候选的正式 seed 17 训练使用独立可恢复运行根，24 候选入口在结构门禁通过前禁止启动。
- seed 17 正式 8 项结构诊断中没有候选直接过门；完整 v2 的公共自监督验证损失为 0.3255，航电/生理保真比分别为 1.1649/0.5571，有效秩为 26.95，时间机制误差比为 0.8436，唯一失败项为生理保真。
- 生理专属切片和生理路径投影蒸馏的正式修复轮次均未达到 98% 生理保真硬门，证明可学习投影会吸收蒸馏目标但不能可靠保存原始生理几何；该轮次按 fail-closed 规则不开放超参数搜索。
- 直接因果生理残差只在生理专属切片中保留归一化、因果前向填充的当前查询值，不改变 24 维航电专属和 24 维因果共享子空间。正式 5-epoch 修复轮次的航电/生理保真比为 1.1396/2.3466，时间机制误差比 0.8804，全部禁止访问标志为 false；24 项搜索入口已改为校验该版本化门禁及 selected candidate 后才运行。
- ODE-RNN 的逐样本 RK4 和 GRU 更新已改为数学等价的 batch 向量化；逐样本 `torchdiffeq`/GRU 输出和参数梯度测试通过。包含 RK4 的三个真实网格候选单 epoch smoke 合计由不可接受的逐样本循环耗时降到 25.5 秒，全量回归为 `411 passed, 8 skipped`。
- 24 项正式训练 24/24 完成且训练层 3/3 门禁通过。任务无关诊断 24/24 通过；排名前三依次为 96 维连续 basis + RK4、96 维固定五档 + RK4、96 维连续 basis + Euler，学习率均为 `1e-3`，公共自监督验证损失分别为 0.3219、0.3240、0.3247。
- 最初三折门禁错误地用完整 64 维表示衡量“专属保真”，共享/生理维度在 31 个训练样本的 ridge 探针中造成强过拟合；该结果以独立诊断根保留但不锁定配置。修正后 v2 航电保真只读 24 维航电专属切片、生理保真只读 16 维生理专属切片，v1 路径不变。当前 checkpoint 的三折航电保真比为 1.2661–2.3750，证明信息已保留在声明子空间而不是依靠共享维度。
- 三折复核最终锁定 96 维连续 basis + Euler + `1e-3`。独立确认访问记录绑定锁定配置 SHA-256 `76ea2962…` 与封存 payload SHA-256 `a7f526ae…`，方法范围固定为六方法；原始 seal manifest 继续保持 `unlocked=false`。
- 新的独立仿真确认族已在配置锁定前生成并封存：48 条潜在轨迹覆盖 35 个统一压力场景，共 1,680 个观测场景和 3,360 个原始观测/真值文件，约 1.7 GB；profile、latent 和 observation seed 区间均与既有开发/确认数据分离。六方法共享同一 payload SHA-256，当前 `unlocked=false`，结构/超参数选择代码无法访问。
...
当前长程 goal 的实验、证据与图表门禁均已通过；最终仓库验证结果如下：

- 完整 pytest：`372 passed, 8 skipped, 319 warnings`。
- `compileall src scripts tests`、本轮改动文件 Ruff、`git diff --check`、读者术语、密钥模式和重型产物忽略检查：通过。
- 全仓 Ruff 仍有 252 项历史基线，集中在未修改的归档脚本、旧模块与合并测试；本轮不做跨目标清理。
- 两个代表案例图已重绘并复核，其余五图也已逐图检查字体、图例、长标签和数值标注。
