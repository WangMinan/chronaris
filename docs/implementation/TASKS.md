...
在不依赖新增鼎新数据或人工标签、不读取外层测试的前提下，修复鼎新训练内任务协议，构造唯一且隔离的验证支持单元，并审计三个核心任务跨验证支持单元的相对可学习性。

当前分支：`codex/dingxin-task-stability-20260714`。

## 当前里程碑：任务协议修复与跨视图稳定化已收口

新协议形成 6 个唯一、三类完整的主选模验证单元，完整 35 秒支持区间互不重叠，并额外保留至少 35 秒间隔。22 个预注册轻量候选完成 132 个候选—验证单元运行；第三个外层训练池无法在不降低约束时形成合法单元，已作为分布压力诊断隔离。三项相对可学习性门禁均未完整通过，冻结专家安全融合和任务感知主干研究均不得自动启动。

### 本轮结果与退出决定

- 机动强度分类最佳候选为双流 MiniRocket（1000 核），平均/中位/最差 Macro-F1 为 `0.8286/0.9111/0.6627`，最低类别召回为 `0.5000`；平均表现和最低类别召回未通过门禁。
- 连续生理响应最佳候选的中位 RMSE 比率为 `1.0000`、平均相对技能为 `0.0000`，正式门禁与近失门禁均失败；没有字段级或双流方案形成跨验证单元稳定增益。
- 高生理响应最佳候选的平均/中位归一化平均精确率为 `0.2089/0.0687`，六个验证单元中五个为正，但中位表现和平均 AUPRC 增益仍未通过门禁。
- 仅使用时间块位置的诊断模型最高 Macro-F1 达 `0.9153`，说明存在时间位置捷径风险；正式轻量候选已统一移除 30 个实际存在的时间类航电通道，并且不使用视图、飞行员、架次或显式查询位置。
- 标签源直接重叠和确定性派生重叠均为 0，开发运行只请求 inner-train/validation；外层测试、历史逐样本外层结果和 held-out 预测均未读取。
- 唯一首要阻断项归纳为跨验证支持单元的泛化不稳定。`allow_safe_fusion=false`、`allow_task_aware_research=false`；若继续研究，必须预注册新的稳定化方案，不能在本轮回流搜索。
- 紧凑证据入口：[鼎新任务协议修复与跨视图稳定化](../artifacts/runs/2026-07-14_dingxin-task-stability/gap_report.md)。
