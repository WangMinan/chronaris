...
v1 失真诊断、结构修复、24 项超参数筛选和鼎新三个 inner-validation 折复核均已完成。专属切片保真诊断确认三候选全部通过三折门禁，航电最差折保真比为 1.2661–1.8648，生理最差折保真比均超过 2.05，outer-test 请求为 0。词典序最终锁定 96 维、连续时延 basis、Euler、`lr=1e-3`、直接因果生理残差配置。seeds 17/29/43 × 鼎新五折的 15 个 v2 任务无关 checkpoint 已全部完成，outer-test provider 请求为 0；当前重新导出既有五种基线与锁定 v2 的六方法表示，随后按冻结 consumer、仿真 clean、封存确认、时间机制、正式消融、一次性晋级审计和公开数据适配顺序自动续跑。
