...
**公开数据（CogPilot）：已运行，诚实修订（小队列结论不稳健）**——见 `runs/2026-07-23_cogpilot-difficulty/`。飞行难度四分类 LOSO，12 epoch：

| 方法 | 10subj seed17 | 10subj seed29 | 20subj seed17 |
| --- | --- | --- | --- |
| Chronaris safe_lag | 0.4122 | 0.3353 | 0.4654 |
| Chronaris multiscale（旧） | 0.4017 | 0.3045 | 0.3543 |
| vehicle_only（飞机单流） | 0.3396 | 0.2060 | **0.5045** |
| physiology_only（生理单流） | 0.1521 | 0.2103 | 0.1833 |

**诚实修订**：10 参与者（3 测试受试者）时 safe_lag 超过 vehicle_only，但扩到 20 参与者（5 测试受试者）后 **vehicle_only（0.5045）反超 safe_lag（0.4654）**——“safe_lag 超最佳单流”在小队列不稳健，难度分类在更大队列也变得可由飞机状态单独预测。**稳健成立的结论**：safe_lag **始终优于旧 multiscale 融合**（10subj 两种子与 20subj 均成立），即安全旁路相对旧融合是稳健改进（晋级门禁 1“超全部融合基线”方向性成立，但门禁 5“超最佳单流”不稳健）。CLARE 与锁定确认未运行。
...
**未达成（诚实修订）。** 公开 CogPilot 难度分类：safe_lag **稳健优于旧 multiscale 融合**（10subj 两种子与 20subj 均成立，晋级门禁 1“超全部融合基线”方向性成立），但**未稳健超过最佳单流**——10 参与者时超 vehicle_only，扩到 20 参与者后 vehicle_only 反超（0.5045 vs 0.4654），门禁 3/5“超最佳单流”不成立。鼎新未来机动三种子（车辆主导）safe_lag 均值 0.224 未稳定超航电单流 0.481（门禁 4/6/8 未满足）；鼎新生理任务六方法 0 正技能字段（门禁 5 鼎新侧未满足）；滞后损失未改善。锁定确认未运行。**核心科学目标（融合稳健超最佳单流）在所测任务上未达成**——车辆/飞机状态在机动与难度任务上都过于强势。架构改进（safe_lag 稳健优于旧融合）真实成立。最低晋级目标与拉伸目标整体未达成。
