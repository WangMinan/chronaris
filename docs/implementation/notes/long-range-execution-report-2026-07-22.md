...
| 方法 | 机动 macro-F1 | balanced acc | Spearman |
| --- | --- | --- | --- |
| Chronaris safe_lag | **0.3387** | 0.4333 | 0.4127 |
| Chronaris multiscale（旧） | 0.1667 | 0.3333 | 0.5471 |
| vehicle_only | 0.4821 | 0.5000 | 0.4554 |

判断：safe_lag 机动 macro-F1 是旧融合的约 2.0 倍（0.3387 vs 0.1667），同口径内超过全部融合基线（门禁 4 方向性满足），但尚未达到航电单流 0.4821（门禁 6 未满足）。**非锁定确认**（单折单种子 30 epoch，非 50 epoch 双折三种子）。
...
**部分进展，未整体达到。** 已有真实证据：鼎新单折单种子上 safe_lag 机动 macro-F1 `0.3387` 是旧融合 `0.1667` 的约 2 倍（同口径超过全部融合基线，门禁 4 方向性满足），但**未达到航电单流** `0.4821`（门禁 6 未满足）；公开数据（门禁 3）、多种子稳定性（门禁 8）、锁定确认均未运行。最低晋级目标整体未达成；拉伸目标（全部鼎新核心任务超基线）未达成。
