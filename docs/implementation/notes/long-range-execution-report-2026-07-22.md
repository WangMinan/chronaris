...
**公开数据（CogPilot）：已运行，两种子稳定正向证据**——见 `runs/2026-07-23_cogpilot-difficulty/`。飞行难度四分类，LOSO（10 参与者训练、3 测试），12 epoch、两种子：

| 方法 | seed17 | seed29 | 均值 |
| --- | --- | --- | --- |
| **Chronaris safe_lag** | **0.4122** | **0.3353** | **0.3738** |
| Chronaris multiscale（旧） | 0.4017 | 0.3045 | 0.3531 |
| vehicle_only（飞机单流） | 0.3396 | 0.2060 | 0.2728 |
| physiology_only（生理单流） | 0.1521 | 0.2103 | 0.1812 |

判断：safe_lag 在 seed17 与 seed29 **均同时高于**旧融合、最佳单流 vehicle_only 与 physiology_only——**公开数据双流增量稳定成立（晋级门禁 3 与门禁 5“双流任务超最佳单流”满足，跨种子一致）**。相对最佳单流 vehicle_only 均值增量 `+0.101`（+37%）。难度同时驱动生理唤醒与飞机操纵/状态，是融合本应占优的任务；与鼎新车辆主导机动任务（融合难超航电单流、且不稳定）对照说明：**任务语义决定融合是否有增量与稳定性**。单 LOSO 折、12 epoch、2 种子，非锁定确认。CLARE 与锁定确认仍未运行。
...
**部分达成（公开数据稳定正向），鼎新未达成。** 公开 CogPilot 飞行难度四分类 LOSO 两种子（seed17/29）safe_lag 均值 `0.3738` 同时高于旧融合 `0.3531`、最佳单流 vehicle_only `0.2728`、physiology_only `0.1812`，**跨种子稳定**——**门禁 3（公开数据超基线）与门禁 5（双流任务超最佳单流）满足**（相对最佳单流均值 +37%，符号一致）。鼎新未来机动三种子（车辆主导任务）safe_lag 均值 `0.224` 未稳定超航电单流 `0.481`，门禁 4/6/8 未满足；鼎新生理任务六方法 0 正技能字段（Dingxin 双流增量门禁未满足）。锁定确认未运行。最低晋级目标部分达成（公开双流增量任务稳定超基线），鼎新核心任务整体未达成；拉伸目标未达成。关键洞见：**任务语义决定融合是否有增量**——车辆主导任务融合难超航电单流，而难度等双流任务融合稳定占优。
