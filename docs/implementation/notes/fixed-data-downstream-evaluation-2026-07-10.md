...
- 该门禁现已完成：270/270 份表示、90/90 个 consumer、6/6 与 8/8 验收全部通过。54 项主指标配对中，Chronaris 的机动分类、高生理响应识别和连续响应预测方向归一平均变化为 -0.0048、-0.0336 和 +0.0062。仿真预训练只对连续响应 RMSE 形成小幅平均改善，且三个 seed 中仅一个改善；高生理响应识别三个 seed 全部退化。因此 real-only 冻结表示仍是鼎新主表，迁移轨道承担跨域适配诊断与负结果证据。

### 2026-07-12 消融与论文证据包收口

- 四项 Chronaris 机制消融完成 12/12 个统一 consumer、768 条指标、384 条完整模型方向归一差异和 72 条以 48 条潜在轨迹为单位的配对统计，6/6 门禁通过。
- 连续演化对负荷分类/回归的平均贡献最明显；因果掩码和多尺度时延对机动状态分段贡献最明显。物理约束在回归与分段上小幅正向，但负荷分类为负向，不写成全任务一致机制增益。
- 最终 `2026-07-12_downstream-evidence-pack` 汇总 8 层证据、7 幅中文图和 54 项迁移配对，9/9 门禁通过。七图逐图抽查后，鼎新案例改用可读分组折名称，仿真案例使用“生成器真值”并隐藏机器轨迹 ID。
- 主结论固定为分项优势与机制解释：鼎新高生理响应识别、仿真机动分段和时间机制绝对误差支持 Chronaris；机动分类、MiniRocket 负荷任务、边界定位、随机/连续缺失斜率和仿真预训练迁移不支持全面领先。
- 最终完整测试为 `372 passed, 8 skipped, 319 warnings`；`compileall`、本轮改动文件 Ruff、差异空白、读者术语、密钥模式和重型产物边界通过。全仓 Ruff 的 252 项历史问题保持独立，不与本次下游评估实现混合清理。
