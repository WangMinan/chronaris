...
2. `Stage H` 导出仍是 `per-view preview training -> export`，不是 `fixed checkpoint inference -> export`。
3. `E/F/G/H` 骨干与 `Stage I` 下游任务仍是“先导出特征，再接外部 head”，还不是联合训练闭环。
...
6. 私有主线任务仍以 `T1/T2/T3 proxy benchmark` 为主，和论文里的风险/负荷/复盘任务本体还没有完全对齐。
...
1. `Phase A`：先纠正论文口径和主线实现不一致。
2. `Phase B`：把导出路径改成 checkpoint inference。
3. `Phase C`：把联合训练和 thesis task builder 打通。
4. `Phase D`：再强化 physics / semantic fusion。
5. `Phase E`：最后补 runtime inference。

原因很直接：

- `Phase A/B` 先解决“现在的证据到底是不是同一条主线”。
- `Phase C/D` 再解决“方法体是否真正落在代码里”。
