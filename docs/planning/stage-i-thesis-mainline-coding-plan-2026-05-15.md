...
## 4. Phase C：联合训练与 thesis task builder
...
1. 先完成 `Phase A` 的命名、边界和文档修正。
2. 再完成 `Phase B` 的 checkpoint backbone 与 `Stage H` inference export。
3. 然后打通 `Phase C` 的 multitask + thesis task builder。
4. 再补 `Phase D/E` 的 physics 与 semantic event 方法体。
5. 最后完成 `Phase F` 的 runtime inference。
