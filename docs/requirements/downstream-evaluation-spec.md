...
日期：2026-07-10  
状态：2026-07-10 历史锁定规格，保留对应版本的任务、划分与判据来源。

2026-09-10 导航更新：当前目标与执行按[融合表示下游评价方案](thesis-downstream-representation-plan-20260909.md)，已有实现来源见 [v4 合同](thesis-v4-development-plan.md)。下文原两架次、目标构造与时间合并规则只在复现该版本时使用，不覆盖九月去重、原生时间输入和双路线修订。
...
鼎新原始长表在 30 秒上下文内包含大量同一字段分散上报的稀疏事件。为避免把“数据库长表行数”误当作“连续时间状态步数”，六种方法在归一化、训练增强和编码之前统一执行 `100 ms` 固定时间箱合并：

- 时间箱由 `floor(timestamp_s / 0.1)` 确定，API 不接收方法名称、任务标签或预测结果。
- 同一时间箱内同一字段的多次真实观测取算术平均；未观测字段保持 feature mask 为 false。
- 每个时间箱的模型时间戳取该箱最后一次真实观测时刻，因此只会把较早观测延迟到箱末，绝不把观测提前到未来。
- 合并后重新计算 point mask、feature mask 和逐字段 observation age；原始 snapshot、原始时间戳和 SHA-256 lineage 保持不变。
- train、validation、outer-test 以及生理单流、航电单流、朴素时间同步、MulT、ContiFormer、Chronaris 必须使用同一实现和同一箱宽。

该步骤属于模型无关的输入组织，不是插值，也不改变 96 点查询轴。首个真实上下文的航电事件由 1779 点合并为 300 点，生理事件由 150 点合并为 120 点；因果末端时间戳保持一致。单折单随机种子实跑中，GPU 上四个深度基线单 epoch 为 8.59–16.46 秒，Chronaris 为 40.07 秒；旧的未合并 Chronaris 路径约 615.6 秒/epoch。因此正式鼎新锁定训练采用该公共合同，并把 `model_input_bin_width_s=0.1` 写入 protocol。
...
该机制任务固定比较朴素时间同步、MulT、ContiFormer 和 Chronaris。编码器锁定后，使用 G1 六个观测场景的融合表示训练两个相同容量的岭回归（Ridge）恢复探针：

- 生理流相对航电流的绝对时钟偏移幅值；
- 第一生理响应字段的真实响应时延。

探针只使用 G1 训练集拟合，`alpha={0.1,1,10,100}` 只由 G1 验证集选择。G2 干净场景、单因素压力和混合重压场景只负责最终评价，不进入拟合或选参。报告平均绝对误差（MAE）、均方根误差（RMSE）、容差命中率和 Spearman 秩相关；配对统计单位为 48 条 G2 潜在轨迹。
...
- 不新增 `hmmlearn`。
- 所有方法共享同一平滑配置。
- 半马尔可夫动态规划允许只对 batch 维做向量化，时间、类别和持续时间的候选遍历顺序保持不变；批量实现必须与逐样本解码逐位一致。
