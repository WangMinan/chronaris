...
日期：2026-07-10  
状态：既有六方法表示接口及 2026-07-10 训练设计来源。

2026-09-10 继承说明：当前研究按[融合表示下游评价方案](../thesis-downstream-representation-plan-20260909.md)。现有代码仍使用 64 维特征和 96 点序列合同；新增方法是否兼容需实测，窗口表示不得复制成假序列。下文固定 30 秒输入、100 毫秒合并、旧训练预算和损失日程为历史设计，现有 v4 的混合窗口、原生时间输入、更新计数及双路线以 [v4 合同](../thesis-v4-development-plan.md)为来源。更新文档不代表这些待验证的新接口已实现。
