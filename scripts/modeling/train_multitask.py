"""Run task evaluation multitask joint training on real feature export all-window assets."""
...
def _default_run_id() -> str:
    return datetime.now(timezone.utc).strftime("%Y%m%dT%H%M%SZ-task-eval-multitask")
...
def main() -> int:
    ...
    result = run_task_eval_multitask_train(
        ...
        source_summary={
            "source_type": "real_feature_export_multitask_train",
            "source_manifests": {
...
def _validate_matched_feature_export_manifests(
    ...
    if not isinstance(e_config, Mapping) or not isinstance(f_config, Mapping):
        raise ValueError("E/F feature export manifests must include config mappings.")
    fields_to_match = (
        "export_profile",
        "window_duration_ms",
        "window_stride_ms",
    ...
    for field_name in fields_to_match:
        if e_config.get(field_name) != f_config.get(field_name):
            raise ValueError(f"E/F feature export manifest config mismatch at {field_name}.")
...
def _write_evidence_report(
    *,
    ...
    lines = [
        f"# task evaluation Thesis Weak-Label Evidence - {run_id}",
        "",
