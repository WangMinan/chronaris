#!/usr/bin/env python3
"""Run frozen CogPilot and CLARE subject-grouped outer evaluation."""
...
def main() -> int:
    ...
    if not SIMULATION_GATES.is_file():
        raise RuntimeError("public outer evaluation requires simulation gate audit")
    gates = json.loads(SIMULATION_GATES.read_text(encoding="utf-8"))
    if gates.get("all_hard_gates_passed") is not True:
        raise RuntimeError("public outer evaluation blocked by simulation hard gate")
    state = run_thesis_native_outer(
        ThesisNativeOuterConfig(
        ...
        json.dumps(
            {
                ...
                "completed_for_requested_tasks": state[
    ...
    return 0 if state["completed_for_requested_tasks"] else 2
...
def _require_clean_source():
    ...
    if status:
        raise RuntimeError("native outer evaluation requires committed source")
