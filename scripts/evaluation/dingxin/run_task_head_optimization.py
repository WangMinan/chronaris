"""Run P34 task-aware head optimization for private feature export proxy tasks."""
...
from chronaris.evaluation.dingxin.pipelines.task_head_optimization import (  # noqa: E402
    P34_MODELS,
    StageITaskHeadOptimizationConfig,
    run_task_eval_task_head_optimization,
)
...
def _default_run_id() -> str:
    return datetime.now(timezone.utc).strftime("%Y%m%dT%H%M%SZ-task-eval-task-heads-optimization")
...
def main() -> int:
    ...
    result = run_task_eval_task_head_optimization(
        StageITaskHeadOptimizationConfig(
            run_id=args.run_id,
