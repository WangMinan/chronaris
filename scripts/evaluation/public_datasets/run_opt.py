"""Run one task evaluation public-opt path over prepared UAB or NASA assets."""
...
DEFAULT_SKLEARN_ARTIFACT_ROOT = "docs/artifacts/runs"
DEFAULT_TORCH_ARTIFACT_ROOT = "docs/artifacts/runs"


def _default_run_id(*, dataset_id: str, backend: str) -> str:
    timestamp = datetime.now(timezone.utc).strftime("%Y%m%dT%H%M%SZ")
    if backend == "torch" and dataset_id == "uab_workload_dataset":
        return f"{timestamp}-task-eval-public-opt-uab-torch"
    return f"{timestamp}-task-eval-public-opt"
...
def _run_torch_backend(
    *,
    ...
    return run_task_eval_public_opt_torch_uab(
        StageIPublicOptTorchUABConfig(
            run_id=run_id,
            prepared_artifact_root=_resolve_path(args.prepared_artifact_root),
            artifact_root=_resolve_path(artifact_root),
