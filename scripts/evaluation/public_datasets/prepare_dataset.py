"""Prepare task evaluation task manifests and feature tables for UAB or NASA CSM."""
...
from chronaris.dataset import build_nasa_csm_task_entries, build_uab_task_entries, dump_task_eval_summary, dump_task_eval_task_entries
from chronaris.features import build_nasa_csm_feature_table, build_uab_feature_table
from chronaris.archive.legacy_public_benchmark.phase3 import build_task_eval_dataset_summary
...
def _default_run_id(dataset: str, profile: str) -> str:
    ...
    return f"{timestamp}-task-eval-{dataset}-{profile}"
...
def main() -> int:
    ...
    dump_task_eval_task_entries(prepared.entries, path=output_dir / "task_manifest.jsonl")
    ...
    summary = build_task_eval_dataset_summary(prepared.entries, feature_result)
    dump_task_eval_summary(summary, path=output_dir / "dataset_summary.json")
