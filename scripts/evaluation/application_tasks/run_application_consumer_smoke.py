#!/usr/bin/env python3
"""Run six frozen fusion methods through fixed application-facing consumers."""
...
def main() -> int:
    ...
    parser.add_argument(
        "--run-id",
        default="2026-07-11_application-consumer-smoke",
    )
    ...
    parser.add_argument(
        "--simulation-root",
        default="artifacts/application_evaluation/2026-07-10_aviation-simulation-formal",
    )
    parser.add_argument(
        "--pretraining-run-id",
        default="2026-07-11_common-pretraining-loop-smoke",
    )
    parser.add_argument("--seed", type=int, default=17)
    parser.add_argument("--resume", action=argparse.BooleanOptionalAction, default=True)
    args = parser.parse_args()
    configure_task_eval_cli_logging(sys.stderr)
    result = run_application_consumer_smoke(
        ApplicationConsumerSmokeConfig(
            run_id=args.run_id,
