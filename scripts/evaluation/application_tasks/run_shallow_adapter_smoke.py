"""Run production smoke for physiology-only, vehicle-only and naive time sync."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--run-id",
        default="2026-07-11_shallow-baseline-adapter-smoke",
    )
...
def main() -> int:
    ...
    args = parse_args()
    result = run_shallow_adapter_smoke(
        ShallowAdapterSmokeConfig(
            run_id=args.run_id,
