"""Run task evaluation baseline suites from prepared artifacts."""
...
from chronaris.archive.legacy_public_benchmark.baseline import (
    render_task_eval_baseline_report,
    run_task_eval_baselines,
...
def main() -> int:
    ...
    report_path = REPO_ROOT / (
        args.report_path
        or f"docs/artifacts/runs/task-eval-{args.dataset or dataset_id}-{profile}-{datetime.now().date().isoformat()}.md"
    )
    report_markdown = render_task_eval_baseline_report(
        artifact_root=artifact_root,
