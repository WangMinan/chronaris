"""Build P36 optimized Chronaris re-evaluation package."""
...
from chronaris.pipelines.stage_i.evidence.optimized_reevaluation import (  # noqa: E402
    StageIOptimizedReevaluationConfig,
    run_stage_i_optimized_reevaluation,
)


def _default_run_id() -> str:
    return datetime.now(timezone.utc).strftime("%Y%m%dT%H%M%SZ-stage-i-optimized-reevaluation")
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--artifact-root", default="docs/artifacts/assets/stage_i_optimized_reevaluation")
...
def main() -> int:
    ...
    result = run_stage_i_optimized_reevaluation(
        StageIOptimizedReevaluationConfig(
            run_id=args.run_id,
