#!/usr/bin/env python3
"""Export Stage I runtime replay samples from a real Stage H run manifest."""
...
def _default_run_id() -> str:
    return datetime.now(timezone.utc).strftime("%Y%m%dT%H%M%SZ-stage-i-runtime-samples")
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--output-root", default="docs/artifacts/assets/stage_i_runtime_inference")
...
def main() -> int:
    ...
    samples, source_summary = collect_stage_i_multitask_samples(
        profiles=profiles,
