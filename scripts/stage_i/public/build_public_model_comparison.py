"""Build Stage I public model-comparison tables, figures, and report."""
...
from chronaris.pipelines.stage_i.public.model_comparison import (  # noqa: E402
...
def _default_run_id() -> str:
    return datetime.now(timezone.utc).strftime("%Y%m%dT%H%M%SZ-stage-i-public-model-comparison")
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--artifact-root",
        default="docs/artifacts/assets/stage_i_public_model_comparison",
    )
...
def main() -> int:
    args = parse_args()
    result = build_stage_i_public_model_comparison(
        StageIPublicModelComparisonConfig(
            run_id=args.run_id,
