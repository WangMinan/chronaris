...
from chronaris.evaluation.public_datasets.pipelines.stream_role_fusion_eval import (  # noqa: E402
    StageIStreamRoleFusionEvalConfig,
    run_task_eval_stream_role_fusion_eval,
)


def _default_run_id() -> str:
    return datetime.now(timezone.utc).strftime("%Y%m%dT%H%M%SZ-task-eval-stream-role-fusion")
...
def main() -> int:
    ...
    result = run_task_eval_stream_role_fusion_eval(
        StageIStreamRoleFusionEvalConfig(
            run_id=args.run_id,
