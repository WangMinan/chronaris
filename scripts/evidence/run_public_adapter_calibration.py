"""Build task evaluation public adapter calibration summary from existing public outputs."""
...
from chronaris.evidence.public_adapter_calibration import (  # noqa: E402
    ...
    resolve_git_commit,
    run_task_eval_public_adapter_calibration,
)


def _default_run_id() -> str:
    return datetime.now(timezone.utc).strftime("%Y%m%dT%H%M%SZ-task-eval-public-adapter-calibration")
...
def main() -> int:
    args = parse_args()
    result = run_task_eval_public_adapter_calibration(
        StageIPublicAdapterCalibrationConfig(
            run_id=args.run_id,
