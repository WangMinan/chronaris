"""Run the task evaluation Phase 2 case study over frozen feature export assets."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--report-path",
        default=f"docs/artifacts/runs/task-eval-case-study-phase2-{datetime.now().date().isoformat()}.md",
    )
...
def _default_run_id() -> str:
    return datetime.now(timezone.utc).strftime("%Y%m%dT%H%M%SZ-task-eval-phase2-case-study")
...
def main() -> int:
    ...
    config = StageICaseStudyConfig(
        run_id=args.run_id,
        feature_export_run_manifest_path=args.feature_export_run_manifest,
        output_root=args.output_root,
        report_path=args.report_path,
        top_k_windows=args.top_k_windows,
        device=args.device,
    )
    result = run_task_eval_case_study(config)
    report_path = write_task_eval_case_study_report(result)
    print(
        json.dumps(
            {
