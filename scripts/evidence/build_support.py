"""Aggregate task evaluation support evidence into alignment and causal reports."""
...
def _default_run_id() -> str:
    return datetime.now(timezone.utc).strftime("%Y%m%dT%H%M%SZ-task-eval-support")
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--feature-export-run-manifest-path", default=DEFAULT_FEATURE_EXPORT_RUN_MANIFEST_PATH)
...
def main() -> int:
    ...
    result = run_task_eval_support(
        StageISupportConfig(
            ...
            feature_export_run_manifest_path=str(REPO_ROOT / args.feature_export_run_manifest_path),
