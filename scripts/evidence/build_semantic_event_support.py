#!/usr/bin/env python3
"""Build multi-view semantic event support from one feature export run manifest."""
...
from chronaris.pipelines.causal_fusion import (  # noqa: E402
    ...
    build_feature_export_semantic_event_support,
)


def _default_run_id() -> str:
    return datetime.now(timezone.utc).strftime("%Y%m%dT%H%M%SZ-task-eval-semantic-support")
...
def main() -> int:
    args = parse_args()
    summary = build_feature_export_semantic_event_support(
        args.run_manifest,
    ...
    report_path = report_root / f"task-eval-semantic-event-support-{args.run_id}.md"
...
def _render_report(run_id: str, summary: dict[str, object]) -> str:
    lines = [
        f"# task evaluation Semantic Event Support - {run_id}",
        "",
