"""Run the minimal thesis-facing task evaluation runtime/demo entry."""
...
from chronaris.serving.runtime_demo import (  # noqa: E402
    ...
    run_task_eval_runtime_demo,
)


def _default_run_id() -> str:
    return datetime.now(timezone.utc).strftime("%Y%m%dT%H%M%SZ-task-eval-runtime-demo")
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--source-type",
        choices=("auto", "feature_export_run_manifest", "optimized_candidate_package"),
        default="auto",
    )
