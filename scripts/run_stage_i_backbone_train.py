"""Train one reusable Stage I alignment backbone from Stage H view samples."""
...
from chronaris.pipelines.stage_i.stage_i_backbone_train import (
...
def _default_run_id() -> str:
    return datetime.now(timezone.utc).strftime("%Y%m%dT%H%M%SZ-stage-i-backbone")
...
def main() -> int:
    ...
    samples, source_summary = collect_stage_i_backbone_samples(
        profiles=profiles,
